  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      <div className="flex items-center justify-between border-b border-border px-5 py-4">
        <div>
          <h2 className="font-bold">All trainings</h2>
          <p className="mt-0.5 text-xs text-muted-foreground">
            Showing {records.length} of 148 trainings
          </p>
        </div>
      </div>
          ...
          <tbody>
            {records.map((item) => (
              <tr key={item.id} className="border-t border-border">
                ...
                <td className="px-5 py-4">{item.trainer}</td>
                <td className="px-5 py-4">{item.department}</td>
                <td className="px-5 py-4">{item.participants}</td>
