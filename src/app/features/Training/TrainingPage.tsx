...
export function TrainingPage() {
  ...
  const canManage =
    hasPermissionKey(P.TRAINING_MANAGE) || hasPermissionKey(P.TRAINING_FULL);
    ...
    <div className="space-y-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {quickActions.map(({ label, icon: Icon, action }) => (
            <button
              key={label}
              onClick={action}
              className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4 text-left shadow-sm transition hover:border-primary/50 hover:shadow"
            >
      ...
      <section className="grid gap-4 xl:grid-cols-[1.8fr_1fr]">
        <div>
          ...
          />
          <div className="mt-4">
            <TrainingTabs records={records} onDetails={() => open("details")} />
          </div>
        </div>
  ...
  const content: Record<TrainingView, React.ReactNode> = {
    ...
    reports: (
      <EmptyWorkspace
        ...
        description="Completion, participation, department, cost, hours, trainer performance, and feedback analysis are available from this workspace."
        icon={BarChart3}
      />
    ),
    settings: (
      <EmptyWorkspace
        ...
        description="Manage categories, types, delivery modes, trainer and skill masters, approval workflow, notifications, feedback templates, and reminders."
        icon={Settings2}
      />
    ),
  ...
  return (
    <div className="w-full space-y-6 px-4 py-6 md:px-8">
      ...
      />
      {content[view]}
      {modal === "create" && <AddCourseModal onClose={close} />}{" "}
      {modal === "assign" && <AssignCourseModal onClose={close} />}{" "}
      {modal === "details" && <CoursePlayerModal onClose={close} />}
    </div>
  );
}
