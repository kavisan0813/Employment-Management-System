...
export function CandidateProcess({
  ...
  const matchedTemplate = templates.find(
    (t) => t.id === employee?.assignedTemplateId,
  );
      ...
      <div className="flex items-center justify-between pb-3 border-b border-border">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            Review and verify candidate-submitted setup data, signed forms, and
            uploads
          </p>
        </div>
        <div className="text-right">
          <span className="text-[13px] font-black text-[#00B87C] block">
            {progressPct}% Setup Complete
          </span>
          ...
          <div className="divide-y divide-border/60">
            {employeeDocs.map((doc) => {
              ...
              return (
                <div
                  key={doc.id}
                  className="py-4 flex items-center justify-between first:pt-0 last:pb-0 gap-4 flex-wrap"
                >
                  <div>
                    <strong className="text-[12px] text-foreground block">
                      {doc.name}
                    </strong>
                    <span className="text-[10px] text-muted-foreground font-semibold uppercase">
                      Mandatory: {doc.mandatory ? "Yes" : "No"} · Max{" "}
                      {doc.maxSize || 5}MB
                    </span>
                  </div>
