...
export function EmployeePortal() {
  ...
  const updateProgress = (
    ...
    const empTasks = nextPhases
      .flatMap((p) => p.tasks)
      .filter((t) => t.owner === "Employee");
  ...
  const submitCandidateProcess = () => {
        ...
        JSON.stringify([
          {
            ...
            deptColor: "#00B87C",
            joiningDate:
              account?.joiningDate || new Date().toISOString().slice(0, 10),
            progress: 0,
    ...
    showToast(
      "Profile Complete",
      "success",
      "Your candidate details have been sent to HR.",
    );
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-4 border-b border-border">
          ...
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-muted/50 border border-border text-[12px] font-bold text-muted-foreground">
            ...
            <span className="text-foreground">
              {hire?.joiningDate || "Awaiting Setup"}
            </span>
          </div>
        </div>

        <Progress
          completedCount={completedChecksCount}
              ...
              <div className="flex items-start gap-4 p-5 bg-[#F59E0B]/10 border border-[#F59E0B]/20 rounded-2xl">
                ...
                <div className="space-y-1">
                  <p className="text-[14px] font-black text-foreground">
                    Awaiting HR Template Assignment
                  </p>
                  <p className="text-[13px] text-muted-foreground leading-relaxed">
                    Once you complete your setup details and HR assigns an
                    onboarding template, your active workflow checklists,
                    policies, and orientations will unlock here.
                  </p>
                </div>
              </div>
            </div>
  ...
  const completedOnbItems =
    completedProfileChecks +
    employeeOwnedTasks.filter((t) => t.status === "done").length +
    documents.filter((d) => d.status === "uploaded").length +
    (hire.completedPolicies || []).length +
    (hire.completedTraining || []).length;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-4 border-b border-border">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[13px] font-bold text-muted-foreground">
              {hire.role} — {hire.dept} Onboarding (v
              {matchedTemplate?.version || 1})
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-muted/50 border border-border text-[12px] font-bold text-muted-foreground">
          <Calendar size={14} className="text-[#00B87C]" />
          Joining Date:{" "}
          <span className="text-foreground">{hire.joiningDate}</span>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground font-semibold mt-0.5">
              Complete your profile information, upload required documents, read
              policy items, and view orientation media.
            </p>
          </div>
      ...
      <div className="flex border-b border-border bg-muted/10 px-4 rounded-xl overflow-x-auto scrollbar-hide shrink-0 gap-2">
        {(
          [
            ...
            {
              key: "documents",
              label: `Required Files (${documents.filter((d) => d.status !== "uploaded").length})`,
            },
            {
              key: "forms",
              label: `Forms (${(matchedTemplate?.forms || []).length - (hire.completedForms || []).length})`,
            },
            {
              key: "policies",
              label: `Policies (${(matchedTemplate?.policies || []).length - (hire.completedPolicies || []).length})`,
            },
            {
              key: "training",
            ...
            {
              key: "assigned-tasks",
              label: `My Handovers (${employeeOwnedTasks.filter((t) => t.status !== "done").length})`,
            },
          ] as const
        ).map((tab) => (
          <button
      ...
      <div className="mt-4">
        ...
        {activeAssignedSubTab === "candidate-process" && (
          <div className="space-y-4">
            ...
            {hire.candidateProcessSubmitted && (
              <p className="text-xs font-bold text-[#00B87C] text-center">
                Candidate information submitted for department review.
              </p>
            )}
          </div>
        )}
        ...
        {activeAssignedSubTab === "documents" && (
          <div className="bg-card border border-border rounded-3xl p-6 space-y-6 shadow-sm">
            <div>
              ...
              <p className="text-xs text-muted-foreground font-semibold mt-0.5">
                Please upload formal files for HR review
              </p>
            </div>
        ...
        {activeAssignedSubTab === "training" && (
          <div className="bg-card border border-border rounded-3xl p-6 space-y-6 shadow-sm">
            <div>
              <h4 className="text-sm font-black text-foreground">
                Welcome Orientations & Training
              </h4>
              <p className="text-xs text-muted-foreground font-semibold mt-0.5">
                Complete allocated orientation streams and webinars
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {activeAssignedSubTab === "assigned-tasks" && (
          <div className="bg-card border border-border rounded-3xl p-6 space-y-6 shadow-sm">
            <div>
              <h4 className="text-sm font-black text-foreground">
                My Allocated Onboarding Checklist
              </h4>
              <p className="text-xs text-muted-foreground font-semibold mt-0.5">
                Check off task objectives allocated specifically to you
              </p>
            </div>
            <div className="divide-y divide-border">
              {employeeOwnedTasks.map((task) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p
                      className={`text-xs font-bold ${task.status === "done" ? "text-muted-foreground line-through" : "text-foreground"}`}
                    >
                      {task.task}
                    </p>
                    {task.description && (
              ...
              {employeeOwnedTasks.length === 0 && (
                <p className="text-xs text-muted-foreground italic text-center py-6">
                  No onboarding tasks configured for employee action.
                </p>
              )}
            </div>
          </div>
