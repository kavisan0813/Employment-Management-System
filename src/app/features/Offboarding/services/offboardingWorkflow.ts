...
export const OFFBOARDING_TEMPLATES_KEY = "viyan_offboarding_templates:v1";
...
export const saveOffboardingTemplate = (template: OffboardingTemplate) => {
  ...
  const next = existing
    ? templates.map((item) =>
        item.id === template.id
          ? { ...template, version: item.version + 1 }
          : item,
      )
    : [
        ...templates,
        {
          ...template,
          id: template.id || `exit-template-${Date.now()}`,
          version: 1,
        },
      ];
...
export const deleteOffboardingTemplate = (id: string) => {
  localStorage.setItem(
    OFFBOARDING_TEMPLATES_KEY,
    JSON.stringify(
      getOffboardingTemplates().filter((template) => template.id !== id),
    ),
  );
  notify();
};
...
export function createOffboardingRecord(input: {
  ...
  return {
    ...
    lwd: input.lwd,
    progress: 0,
    resumptionDate: input.resignationDate || date,
    acceptedDate: date,
    noticePeriodDays: input.noticePeriodDays,
    reason: input.reason,
    createdBy: input.createdBy,
    createdDate,
    workflowStatus: "Offboarding in progress",
    timeline: [
      { label: "Offboarding Record Created", date, status: "done" },
      { label: "Clearances In Progress", date: "Pending", status: "active" },
    ...
    ffStatus: "Pending",
    interviewDone: false,
    assignedTemplateId: input.template?.id,
    assignedTemplateVersion: input.template?.version,
  };
}
...
export function publishClearance(
  ...
  const updated = exits.map((exit) => {
    ...
    const clearance = exit.clearance.map((item) =>
      item.dept === department ? { ...item, status } : item,
    );
    const allCleared = clearance.every((item) => item.status === "cleared");
    const timeline = [
      ...exit.timeline.filter(
        (item) => item.label !== `${department} Clearance Signed Off`,
      ),
      {
        label: `${department} Clearance Signed Off`,
        date,
        status: "done" as const,
      },
    ];
    if (allCleared)
      timeline.push({ label: "Clearances Completed", date, status: "done" });
