      ...
      {templates.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {templates.map((tpl) => (
            <div
              key={tpl.id}
              className="relative p-5 bg-card border border-border rounded-2xl shadow-sm hover:-translate-y-[2px] hover:border-[#00B87C] hover:shadow-[0_0_15px_rgba(0,184,124,0.3)] transition-all flex flex-col justify-between"
            >
              <div>
                ...
                <div className="flex items-center gap-3 mb-3 text-[11px] font-bold text-muted-foreground">
                  ...
                  <span className="w-1 h-1 rounded-full bg-muted-foreground/40" />
                  <span className="capitalize">{tpl.status}</span>
                </div>
                {tpl.description && (
                  <p className="text-[11px] text-muted-foreground leading-normal mb-4 font-semibold line-clamp-2">
                    {tpl.description}
                  </p>
                )}
              </div>
              <button
                ...
                className="w-full py-2 mt-2 rounded-xl border border-border text-[11px] font-black text-foreground uppercase tracking-widest hover:bg-muted/50 transition-all"
              >
                Edit Template
              </button>
            </div>
