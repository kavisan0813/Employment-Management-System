...
export function RequestsTab({ onCountChange }: { onCountChange?: (count: number) => void }) {
  const { user } = useAuth();
  const [requests, setRequests] = useState<ResignationRequest[]>(() => { try { return JSON.parse(localStorage.getItem("viyan_resignation_requests:v1") || "[]"); } catch { return []; } });
  const [selected, setSelected] = useState<ResignationRequest | null>(null);

  useEffect(() => {
    localStorage.setItem("viyan_resignation_requests:v1", JSON.stringify(requests));
  }, [requests]);
  ...
  const update = (id: string, change: (request: ResignationRequest) => ResignationRequest) =>
    setRequests((items) => {
      const next = items.map((request) => (request.id === id ? change(request) : request));
      onCountChange?.(next.filter((request) => request.status.startsWith("pending")).length);
      return next;
    });
