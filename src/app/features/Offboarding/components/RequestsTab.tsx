...
export function RequestsTab({
      ...
      const approved = {
        ...
        timeline: [
          ...request.timeline,
          {
            ...
            comments:
              payload?.comments ||
              "Approved. Offboarding created automatically.",
          },
        ],
      };
      persistOffboardingRecord(
        createOffboardingRecord({
          ...
          type: "Resignation",
          lwd: approved.lwd,
          noticePeriodDays: parseInt(approved.noticePeriod) || 30,
  ...
  const reject = (id: string, role: string, comments: string) =>
    update(id, (request) => ({
      ...request,
      status: "rejected",
      timeline: [
  ...
  const discuss = (id: string, role: string, comments: string) =>
    update(id, (request) => ({
      ...request,
      timeline: [
        ...request.timeline,
  ...
  const sendBack = (id: string, comments: string) =>
    update(id, (request) => ({
      ...request,
        ...
        {requests.map((request) => (
          ...
          >
            <div className="flex justify-between gap-3">
              ...
              <span className="h-fit rounded-full bg-muted px-2.5 py-1 text-[10px] font-bold uppercase">
        ...
        >
          ...
          >
            <RequestDetails
              ...
              onApprove={(...args) => {
                approve(...args);
                showToast("Request approved", "success", "Workflow updated.");
              }}
              onReject={(...args) => {
                reject(...args);
                showToast("Request rejected", "error", "Workflow updated.");
              }}
              onRequestDiscussion={(...args) => {
                discuss(...args);
                showToast(
