...
export default function RoleTemplatesView({
  ...
  const [formScope, setFormScope] =
    useState<RoleTemplate["scope"]>("Organization");
  const [formStatus, setFormStatus] =
    useState<RoleTemplate["status"]>("Active");
  const [permissionMatrix, setPermissionMatrix] = useState<{
    [module: string]: (typeof AVAILABLE_ACTIONS)[number][];
  }>({});
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const formattedPermissions: PermissionRule[] = Object.keys(permissionMatrix)
      .map((m) => ({
        module: m,
    ...
    if (editingRole) {
      ...
      if (formStatus === "Inactive" && editingRole.assignedOrgCount > 0) {
        alert(
          `Cannot deactivate template while it is assigned to ${editingRole.assignedOrgCount} organization(s).`,
        );
        return;
      }

      db.roleTemplates.save(
        currentRoles.map((r) =>
          r.id === editingRole.id
            ? {
                ...r,
      ...
      pushAuditLog(
        "role.template_modified",
        "Admin Action",
        CURRENT_ADMIN_EMAIL,
        "platform_admin",
        null,
        "Active",
        { title_name: formName, scope: formScope, status: formStatus },
      );
    } else {
  ...
  const cloneRoleConfig = (role: RoleTemplate, e: React.MouseEvent) => {
    ...
    pushAuditLog(
      "role.template_cloned",
      "Admin Action",
      CURRENT_ADMIN_EMAIL,
      "platform_admin",
      null,
      "Active",
      { original_role: role.name, cloned_role: newName },
    );
    refreshData();
  };
  ...
  const purgeTemplateElement = (role: RoleTemplate, e: React.MouseEvent) => {
    ...
    if (
      !confirm(
        `Confirm absolute deletion of security role template ${role.name}?`,
      )
    )
      return;

    db.roleTemplates.save(
      db.roleTemplates.get().filter((r) => r.id !== role.id),
    );
    pushAuditLog(
      "role.template_deleted",
      "Admin Action",
      CURRENT_ADMIN_EMAIL,
      "platform_admin",
      null,
      "Active",
      { deleted_title: role.name },
    );
    refreshData();
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-gray-100 gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Establish blueprints and modular RBAC matrices for assignable client
            tenant scopes.
          </p>
        </div>
        <button
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredRoles.length === 0 ? (
                <tr>
                  ...
                  >
                    No system access templates defined inside this node boundary
                    layer.
                  </td>
                </tr>
              ) : (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        {role.isSystemDefault && (
                          <span className="inline-flex px-1.5 py-0.5 rounded bg-indigo-50 border border-indigo-200 text-[9px] text-indigo-700 font-extrabold tracking-wide uppercase">
                            System
                          </span>
                        )}
                      ...
                      <span className="text-[11px] text-gray-400 mt-0.5 block truncate max-w-xs">
                        {role.description}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex px-2 py-0.5 rounded-md border text-[10px] font-semibold uppercase ${planBadgeClass(role.scope)}`}
                      >
                        {role.scope}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          if (role.assignedOrgCount > 0) setDrawerRole(role);
                        }}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1 max-w-sm">
                        {role.permissions.map((p) => (
                          <span
                            key={p.module}
                            className="px-1.5 py-0.5 bg-gray-100 rounded text-[9px] font-mono text-gray-600"
                          >
                    ...
                    >
                      <div className="flex items-center justify-center gap-1.5 bg-gray-50/80 p-1 rounded-lg border border-gray-100 max-w-[250px] mx-auto shadow-2xs">
                        ...
                        <button
                          disabled={
                            role.isSystemDefault || role.assignedOrgCount > 0
                          }
                          ...
                          title={
                            role.isSystemDefault
                              ...
                              : role.assignedOrgCount > 0
                                ? `Cannot delete template while assigned to ${role.assignedOrgCount} organization(s)`
                                : "Delete template"
                          }
                        >
                          Purge
        ...
        <div className="fixed inset-y-0 right-0 w-full sm:max-w-md bg-white shadow-2xl border-l border-gray-200 z-50 flex flex-col overflow-hidden animate-in slide-in-from-right duration-200">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-2 text-xs text-gray-700 bg-white">
            <p className="text-gray-400 mb-3 leading-relaxed">
              Organizations actively consuming this role blueprint definition
              matrix setup config.
            </p>
            <div className="border border-gray-200 bg-gray-50/50 rounded-xl divide-y divide-gray-200 overflow-hidden font-mono text-[11px]">
              <div className="p-3 text-gray-900 flex justify-between">
                ...
                <span className="text-gray-400">ID: org-11</span>
              </div>
              <div className="p-3 text-gray-900 flex justify-between">
                <span>Stark Industries</span>
                <span className="text-gray-400">ID: org-42</span>
              </div>
              <div className="p-3 text-gray-900 flex justify-between">
                <span>Lumina Systems</span>
                <span className="text-gray-400">ID: org-88</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {isModalOpen && (
        <Modal onClose={() => setIsModalOpen(false)}>
          <ModalHeader
            ...
            title={
              editingRole
                ? `Inspect Matrix Blueprint \u2022 ${editingRole.name}`
                : "Initialize Security Access Template"
            }
          ...
          <form onSubmit={handleFormSubmit} className="p-5 space-y-4 text-xs">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-4">
                ...
                <Field label="Explanatory Scope Description">
                  <textarea
                    disabled={editingRole?.isSystemDefault}
                    placeholder="Helper commentary details..."
                ...
                <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1">
                  {AVAILABLE_MODULES.map((moduleName) => {
                    ...
                    return (
                      <div
                        key={moduleName}
                        className="p-2.5 bg-gray-50 rounded-xl border border-gray-200 space-y-1.5"
                      >
                        <span className="font-bold text-gray-900 block font-mono text-[11px]">
                          {moduleName}
                        </span>
                          ...
                          {AVAILABLE_ACTIONS.map((action) => {
                            ...
                            return (
                              <button
                                ...
                                disabled={editingRole?.isSystemDefault}
                                onClick={() =>
                                  handleMatrixToggle(moduleName, action)
                                }
            ...
            <div className="flex justify-end gap-2.5 pt-3 border-t border-gray-200">
              <button
                ...
                className="px-4 py-2 border border-gray-300 rounded-xl font-medium text-gray-700 hover:bg-gray-50 cursor-pointer"
              >
                Cancel
              </button>
...
function Modal({
  ...
  return (
    ...
    >
      <div
        className={`bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-2xl w-full ${maxWidth}`}
        onClick={(e) => e.stopPropagation()}
      >
        {children}
      </div>
...
function ModalHeader({
  ...
  return (
    <div className="p-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
      ...
      <button
        onClick={onClose}
        className="p-1 text-gray-400 hover:text-gray-600 border border-gray-200 bg-white rounded-lg cursor-pointer transition-colors"
      >
  ...
  return (
    <div className="flex items-center gap-2 text-xs text-gray-500 bg-gray-50 px-3 py-2 rounded-xl border border-gray-200/80">
      ...
      <div className="relative flex items-center">
        ...
        >
          {options.map(([val, lbl]) => (
            <option key={val} value={val} className="bg-white text-gray-800">
              {lbl}
            </option>
          ))}
        </select>
