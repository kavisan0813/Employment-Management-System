          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {escalationRules.map((rule) => (
                <tr
                  key={rule.id}
                  className={`hover:bg-gray-50/50 transition-colors ${!rule.active ? "opacity-50" : ""}`}
                >
                  ...
                  <td className="px-5 py-4 font-medium text-gray-700">
                    {rule.noResponseMinutes < 60
                      ? `${rule.noResponseMinutes} min`
                      : rule.noResponseMinutes < 1440
                        ? `${Math.round(rule.noResponseMinutes / 60)} hr`
                        : `${Math.round(rule.noResponseMinutes / 1440)} day`}
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`px-2 py-0.5 rounded text-[10px] font-bold ${levelColor(rule.level)}`}
                    >
                      {rule.level}
                    </span>
                  </td>
                  <td className="px-5 py-4 text-right">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        ...
                        checked={rule.active}
                        onChange={() => actions.toggleEscalationRule(rule.id)}
                      />
