            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  <div
                    className={`p-2 rounded-xl ${issue.severity === "Critical" ? "bg-red-50 text-red-600" : issue.severity === "Major" ? "bg-orange-50 text-orange-600" : "bg-blue-50 text-blue-600"}`}
                  >
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${severityColor(issue.severity)}`}
                      >
                        {issue.severity}
                      </span>
                    </div>
                ...
                <span
                  className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${statusColor(issue.status)}`}
                >
                  {issue.status}
                </span>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-gray-100 flex items-center gap-6 text-xs text-gray-500">
                <span>
                  <strong className="text-gray-700">Developer:</strong>{" "}
                  {issue.assignedDeveloper || "Unassigned"}
                </span>
                <span>
                  <strong className="text-gray-700">Root Cause:</strong>{" "}
                  {issue.rootCause || "Under investigation"}
                </span>
                <span className="flex items-center gap-1">
                  <Link2 className="w-3 h-3" /> {issue.linkedTicketIds.length}{" "}
                  linked ticket(s)
                </span>
              </div>
            </div>
