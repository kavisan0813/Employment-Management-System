...
import { Mail, Edit3, Sparkles, BookOpen, AlertTriangle } from "lucide-react";
import { CommunicationState, EmailTemplate } from "../types/communication.types";
...
export function EmailTemplatesView({ state, handleSaveEmailTemplate }: EmailTemplatesViewProps) {
  const [selectedId, setSelectedId] = useState(state.emailTemplates[0]?.id || "");
  const [subject, setSubject] = useState("");
  const [body, setBody] = useState("");
  const [category, setCategory] = useState<EmailTemplate["category"]>("Authentication");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-xs flex flex-col sm:flex-row gap-4 items-center justify-between">
        <span className="text-xs font-semibold uppercase text-gray-500 tracking-wider">Select Mail Notification Template</span>
        <select
          value={selectedId}
        ...
        >
          {state.emailTemplates.map(temp => (
            <option key={temp.id} value={temp.id}>{temp.template_name} ({temp.version})</option>
          ))}
        </select>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <form onSubmit={handleFormSubmit} className="lg:col-span-7 bg-white border border-gray-200 rounded-2xl p-5 shadow-xs space-y-4">
          <div className="flex justify-between items-center pb-2 border-b border-gray-100">
            <h4 className="text-xs font-semibold uppercase text-gray-500 tracking-wider flex items-center gap-1.5">
              <Edit3 className="w-4 h-4 text-indigo-500" /> Template Structure Editor
            </h4>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Template Name</label>
              <input
                type="text"
                required
                value={templateName}
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Version Release Label</label>
              <input
                type="text"
                required
                value={version}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Email Subject Line</label>
            <input
              type="text"
              required
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Email Body (Plaintext/HTML)</label>
              <span className="text-[9px] font-extrabold text-gray-400 bg-gray-50 px-2 py-0.5 rounded font-mono">UTF-8 Encoding</span>
            </div>
            <textarea
              required
              rows={8}
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-xs space-y-3">
            <h4 className="text-xs font-semibold uppercase text-gray-500 tracking-wider flex items-center gap-1.5">
              <BookOpen className="w-4 h-4 text-indigo-500" /> Supported Template Variable Glossary
            </h4>
            <div className="grid grid-cols-2 gap-2 text-[10px] font-semibold">
              <div className="bg-gray-50 p-2 rounded-lg border border-gray-150">
                <span className="text-indigo-650 font-mono font-bold block mb-0.5">{"{{name}}"}</span>
              ...
              <div className="bg-gray-50 p-2 rounded-lg border border-gray-150">
                <span className="text-indigo-650 font-mono font-bold block mb-0.5">{"{{expiry_date}}"}</span>
                <span className="text-gray-500">Renewal countdown date</span>
              </div>
              <div className="bg-gray-50 p-2 rounded-lg border border-gray-150">
                <span className="text-indigo-650 font-mono font-bold block mb-0.5">{"{{employee_id}}"}</span>
                <span className="text-gray-500">Unique user registration id</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-xs space-y-4">
            ...
            <div className="border border-gray-200 rounded-xl p-4 bg-gray-50/50 space-y-3">
              <div className="pb-2 border-b border-gray-200">
                <span className="text-[10px] font-extrabold text-gray-400 uppercase block mb-1">Subject Output Header</span>
                <div className="text-xs font-bold text-gray-900">{getMappedPreview(subject) || "No subject configured"}</div>
              </div>
              <div>
                <span className="text-[10px] font-extrabold text-gray-400 uppercase block mb-1">Body Text Output</span>
