...
export function BroadcastMessagesView({
  ...
  const nextStep = () => {
    if (
      bcStep === 1 &&
      (!bcTitle.trim() || !bcCampaign.trim() || !bcMessageText.trim())
    )
      return;
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-white border border-gray-200 p-4 rounded-xl text-xs font-medium text-gray-500 shadow-sm">
        {[
          ...
          <div key={item.step} className="flex items-center gap-2">
            <span
              className={`w-6 h-6 rounded-full flex items-center justify-center border text-xs font-semibold ${
                bcStep === item.step
            ...
            <span
              className={
                bcStep === item.step ? "text-gray-900 font-semibold" : ""
              }
            >
              {item.label}
            </span>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm min-h-[300px] flex flex-col justify-between">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1 max-w-sm">
                ...
                <select
                  ...
                  className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm font-medium text-gray-700 outline-none"
                >
                  {audiences.map((aud) => (
              ...
              <div className="flex gap-4">
                {channelsList.map((ch) => (
                  <button
                    ...
                    className={`flex-1 p-4 border rounded-xl text-center text-sm font-semibold transition-all ${bcChannels.includes(ch) ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200 hover:border-gray-400"}`}
                  >
                    {ch}
                  </button>
                ))}
          ...
          {bcStep === 4 && (
            <div className="space-y-4">
              <h5 className="text-sm font-semibold text-gray-700 flex items-center gap-1.5">
                <Eye className="w-4 h-4" /> Review & Dispatch
              </h5>
              <div className="grid grid-cols-2 gap-6 text-sm border border-gray-200 rounded-xl p-4 bg-gray-50/50">
                <div className="space-y-2">
                  <p>
                    <span className="text-gray-500">Campaign:</span>{" "}
                    <span className="font-semibold">{bcCampaign}</span>
                  </p>
                  <p>
                    <span className="text-gray-500">Audience:</span>{" "}
                    <span className="font-semibold">{bcAudience}</span>
                  </p>
                </div>
                <div className="space-y-2">
                  <p>
                  ...
                  <p>
                    ...
                    <span className="font-semibold">{bcTitle}</span>
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="flex justify-between items-center mt-8 pt-4 border-t border-gray-100">
          <button
            onClick={() => setBcStep(bcStep - 1)}
            disabled={bcStep === 1}
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-lg text-sm font-medium border border-gray-200 disabled:opacity-50"
          >
          ...
          {bcStep < 4 ? (
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium transition-all"
            >
