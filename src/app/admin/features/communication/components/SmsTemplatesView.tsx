...
import { Edit3, ShieldCheck, HelpCircle } from "lucide-react";
...
export function SmsTemplatesView({
  ...
  const gateways = [
    {
      name: "Twilio Gateway API (Global)",
      status: "Active Connection",
      ping: "450ms",
      color: "bg-teal-50 text-teal-700 border-teal-200",
    },
    {
      name: "MSG91 Service (India Core)",
      status: "Active Connection",
      ping: "120ms",
      color: "bg-teal-50 text-teal-700 border-teal-200",
    },
    {
      name: "TextLocal API Gateway",
      status: "Suspended",
      ping: "Timeout",
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <form
          ...
          className="lg:col-span-7 bg-white border border-gray-200 rounded-xl p-5 shadow-xs space-y-4"
        >
          <h4 className="text-sm font-semibold text-gray-700 flex items-center gap-1.5 pb-2 border-b border-gray-100">
            <Edit3 className="w-4 h-4 text-indigo-500" /> SMS Text Content
            Editor
          </h4>
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center text-xs font-medium text-gray-500">
              ...
              <span className="font-mono">
                {charCount} / 160 (Segment: {segments})
              </span>
            </div>
            <textarea
              required
          ...
          <div className="flex justify-between items-center pt-1">
            <span className="text-xs text-gray-500 font-medium flex items-center gap-1">
              <HelpCircle className="w-3.5 h-3.5" /> Character length above 160
              divides message into segments.
            </span>
            <button
              type="submit"
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-xs space-y-4">
        <h4 className="text-sm font-semibold text-gray-700 flex items-center gap-1.5">
          <ShieldCheck className="w-4 h-4 text-emerald-500" /> SMS Gateway Core
          Interfaces Status
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {gateways.map((g) => (
            <div
              key={g.name}
              className="p-3 bg-white border border-gray-200 rounded-xl flex items-center justify-between text-xs font-medium"
            >
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded-full border ${g.color}`}
              >
                {g.status}
              </span>
            </div>
