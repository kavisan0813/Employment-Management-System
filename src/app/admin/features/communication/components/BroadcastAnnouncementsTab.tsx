...
export function BroadcastAnnouncementsTab({
  showToast,
}: {
  showToast: (
    text: string,
  ...
  const { data, error, isLoading, mutate } = useSWR(
    ...
    () =>
      communicationService.getAnnouncementHistory({
        status: statusFilter,
        page,
        limit: 10,
      }),
  ...
  const handleCreateDraftAndPreview = async (
    ...
    if (
      (targetType === "plan_based" || targetType === "specific_orgs") &&
      targetCriteria.trim() === ""
    ) {
  ...
  const handleProceedToConfirm = async () => {
    ...
    try {
      const est = await communicationService.getEstimatedRecipientCount(
        targetType,
        targetCriteria,
        channels,
      );
  ...
  const handleSendTest = async () => {
    ...
    try {
      await communicationService.sendTestAnnouncement(
  ...
  const getChannelIcon = (ch: string) => {
    switch (ch) {
      case "in_app":
        return (
          <span title="In-App" className="inline-flex">
            <MonitorSmartphone className="w-4 h-4 text-indigo-600" />
          </span>
        );
      case "email":
        return (
          <span title="Email" className="inline-flex">
            <Mail className="w-4 h-4 text-blue-600" />
          </span>
        );
      case "sms":
        return (
          <span title="SMS" className="inline-flex">
          ...
          <tbody className="divide-y divide-gray-200">
            {isLoading ? (
              ...
              <tr>
                <td
                  colSpan={5}
                  className="px-6 py-12 text-center text-red-500 font-semibold"
                >
                ...
                <tr key={ann.announcement_id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    <div className="font-semibold text-gray-900 text-sm flex items-center gap-2">
                      {ann.title}
                      {ann.urgency === "high" && (
                        <span className="bg-red-50 text-red-700 border border-red-200 text-[10px] px-1.5 rounded uppercase font-bold">
                          Urgent
                        </span>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 bg-gray-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-xl">
            ...
            <form
              ...
              onSubmit={handleCreateDraftAndPreview}
            ...
            {isEstimating ? (
              ...
              <>
                ...
                <div className="p-6 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 text-center">
                      <div className="text-2xl font-bold text-gray-900">
                        {estimates?.orgCount.toLocaleString()}
                      </div>
                      <div className="text-xs text-gray-500 font-semibold uppercase mt-1">
                        Organizations
                      </div>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 text-center">
                      <div className="text-2xl font-bold text-gray-900">
                        {estimates?.userCount.toLocaleString()}
                      </div>
                      <div className="text-xs text-gray-500 font-semibold uppercase mt-1">
                        Recipients
                      </div>
                  ...
                  {channels.includes("sms") &&
                    estimates?.estimatedSmsCost !== undefined && (
