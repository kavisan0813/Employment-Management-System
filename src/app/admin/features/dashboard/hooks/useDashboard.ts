...
export function useDashboard() {
  ...
  const fetchSummary = async (): Promise<{ current: DashboardStatsCache, growth: any }> => {
  ...
  const { data: summaryData, error: summaryError, isLoading: summaryLoading, mutate: mutateSummary } = useSWR('dashboardSummary', fetchSummary);
  ...
  const fetchTrends = async (): Promise<DailySnapshot[]> => {
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        resolve(snapshots.sort((a, b) => new Date(a.snapshot_date).getTime() - new Date(b.snapshot_date).getTime()));
      }, 400);
    });
  };
  ...
  const fetchRecentSignups = async () => {
    return new Promise<any[]>(resolve => {
      setTimeout(() => {
        ...
        const sorted = orgs.sort((a, b) => new Date(b.joinedAt).getTime() - new Date(a.joinedAt).getTime());
        resolve(sorted.slice(0, 5).map(o => ({
          org_id: o.id,
          name: o.name,
          plan_name: o.plan,
  ...
  const fetchAtRisk = async () => {
    return new Promise<any[]>(resolve => {
      setTimeout(() => {
        ...
        resolve(sorted.slice(0, 10).map(o => ({
          org_id: o.id,
          name: o.name,
          plan_name: o.plan,
          days_since_login: o.lastActiveAt 
            ? Math.floor((Date.now() - new Date(o.lastActiveAt).getTime()) / (1000 * 60 * 60 * 24))
            : 'Never'
        })));
      }, 300);
    });
  ...
  const mrrTrend = trendData?.map(s => {
    ...
    return { month: d.toLocaleString('default', { month: 'short' }), mrr_value: s.mrr_total };
  });

  const signupTrend = trendData?.map(s => {
    const d = new Date(s.snapshot_date);
    return { month: d.toLocaleString('default', { month: 'short' }), signup_count: s.new_signups };
  });
