...
export function BillingHistory({
  ...
  org: Organization;
  hook?: {
    actions: { updatePlan: (id: string, plan: Organization["plan"]) => void };
  };
}) {
  ...
  const handleUpgradeConfirm = () => {
    if (hook?.actions?.updatePlan) {
      hook.actions.updatePlan(org.id, selectedPlan as Organization["plan"]);
    }
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-xs flex items-center justify-between">
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-0.5">
                    Billed {currentSub.billingCycle} • ${currentSub.amount}
                  </p>
                </div>
              </div>
              <div className="text-right">
                ...
                <p className="text-sm font-bold text-gray-900">
                  {currentSub.renewalDate || "Not scheduled"}
                </p>
              </div>
            </div>
          ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-gray-200 overflow-hidden flex flex-col">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Select a new plan for {org.name}.
                </p>
              </div>
              <button
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      Change to the {plan} plan.
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-gray-900">
                      ${PLAN_PRICING[plan] || 0} / mo
                    </span>
                    <div
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
              <button
                onClick={() => setIsUpgradeModalOpen(false)}
                className="px-4 py-2 text-gray-600 font-bold hover:bg-gray-200 rounded-xl text-sm transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={handleUpgradeConfirm}
                className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-sm transition-colors cursor-pointer shadow-md"
              >
                Confirm Upgrade
              </button>
            </div>
