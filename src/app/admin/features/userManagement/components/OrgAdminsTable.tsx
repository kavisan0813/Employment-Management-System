      ...
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setIsDrawerOpen(false)}>
        ...
        >
          ...
          <div className="px-8 py-6 border-b flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Org Admin Profile</h2>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8">
            <div className="grid grid-cols-2 gap-6 text-sm">
              ...
              <div>
                ...
                <p className="font-semibold text-indigo-600">{selectedAdmin.organization}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-sm font-bold ${
                selectedAdmin.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
              }`}>
            ...
            <div className="pt-6 border-t">
              <p className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-4">Manage Org Admin</p>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <button onClick={() => handleEditAdmin(selectedAdmin)} className="flex items-center justify-center gap-2 px-3 py-3 bg-white border border-gray-200 rounded-2xl text-xs font-semibold text-gray-700 hover:bg-gray-50 transition-colors">
                  <Edit className="w-4 h-4" /> Edit Admin
                </button>
                <button onClick={() => handleViewOrganization(selectedAdmin)} className="flex items-center justify-center gap-2 px-3 py-3 bg-white border border-gray-200 rounded-2xl text-xs font-semibold text-gray-700 hover:bg-gray-50 transition-colors">
                  <Building2 className="w-4 h-4" /> View Org
                </button>
                <button onClick={() => handleManageEmployees(selectedAdmin)} className="flex items-center justify-center gap-2 px-3 py-3 bg-white border border-gray-200 rounded-2xl text-xs font-semibold text-gray-700 hover:bg-gray-50 transition-colors">
                  <Users className="w-4 h-4" /> Manage Employees
                </button>
                <button onClick={() => handleResetPassword(selectedAdmin)} className="flex items-center justify-center gap-2 px-3 py-3 bg-white border border-gray-200 rounded-2xl text-xs font-semibold text-gray-700 hover:bg-gray-50 transition-colors">
                  <KeyRound className="w-4 h-4" /> Reset Password
                </button>
                <button onClick={() => handleResendInvitation(selectedAdmin)} className="flex items-center justify-center gap-2 px-3 py-3 bg-white border border-gray-200 rounded-2xl text-xs font-semibold text-gray-700 hover:bg-gray-50 transition-colors">
                  <Mail className="w-4 h-4" /> Resend Invite
                </button>
                <button onClick={() => handleManagePermissions(selectedAdmin)} className="flex items-center justify-center gap-2 px-3 py-3 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-2xl text-xs font-semibold hover:bg-indigo-100 transition-colors">
                  <Shield className="w-4 h-4" /> Permissions
                </button>
                {selectedAdmin.status === "Active" ? (
                  <button onClick={() => handleSuspendAdmin(selectedAdmin)} className="flex items-center justify-center gap-2 px-3 py-3 bg-amber-50 text-amber-700 border border-amber-200 rounded-2xl text-xs font-semibold hover:bg-amber-100 transition-colors">
                    <UserX className="w-4 h-4" /> Suspend
                  </button>
                ) : (
                  <button onClick={() => handleActivateAdmin(selectedAdmin)} className="flex items-center justify-center gap-2 px-3 py-3 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-2xl text-xs font-semibold hover:bg-emerald-100 transition-colors">
                    <UserCheck className="w-4 h-4" /> Activate
                  </button>
                )}
                <button onClick={() => handleTransferOwnership(selectedAdmin)} className="flex items-center justify-center gap-2 px-3 py-3 bg-purple-50 text-purple-700 border border-purple-200 rounded-2xl text-xs font-semibold hover:bg-purple-100 transition-colors">
                  <RefreshCw className="w-4 h-4" /> Transfer Owner
                </button>
                <button onClick={() => { handleRemoveAdmin(selectedAdmin); setIsDrawerOpen(false); }} className="flex items-center justify-center gap-2 px-3 py-3 bg-rose-50 text-rose-700 border border-rose-200 rounded-2xl text-xs font-semibold hover:bg-rose-100 transition-colors">
