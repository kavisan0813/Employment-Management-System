...
export function RolesPermissionsTab() {
  ...
  const handleSaveRole = async (e: React.FormEvent) => {
    e.preventDefault();
    dispatchForm({ type: "setField", field: "createError", value: "" });
    if (newRoleName.length < 3 || newRoleName.length > 50) {
      dispatchForm({
        type: "setField",
        field: "createError",
        value: "Role name must be between 3 and 50 characters",
      });
      return;
    }

    // Check if at least 1 permission selected
    const hasPerm = Object.values(newRolePerms).some((mod) =>
      Object.values(mod).some((val) => val),
    );
    if (!hasPerm) {
      dispatchForm({
        type: "setField",
        field: "createError",
        value: "Select at least one permission",
      });
      return;
    }

    dispatchForm({ type: "setField", field: "isSubmitting", value: true });
    try {
      if (editingRole) {
      ...
      dispatchForm({
        type: "setField",
        field: "createError",
        value: err instanceof Error ? err.message : "Failed to save role",
      });
    } finally {
      dispatchForm({ type: "setField", field: "isSubmitting", value: false });
    }
  };
        ...
        <div className="grid grid-cols-2 gap-2">
          {Object.entries(perms).map(([perm, granted]) => (
            <div key={perm} className="flex items-center gap-2 text-sm">
              ...
              <span
                className={
                  granted ? "text-gray-700 font-medium" : "text-gray-400"
                }
              >
                {perm.replace("_", " ")}
              </span>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() =>
            dispatchForm({ type: "setField", field: "isCreating", value: true })
          }
          ...
          >
            <div className="p-5 border-b border-gray-100 flex justify-between items-start">
              <div className="flex items-center gap-3">
                <div
                  className={`p-2 rounded-lg ${role.isSystemRole ? "bg-amber-100 text-amber-700" : "bg-indigo-100 text-indigo-700"}`}
                >
                  {role.isSystemRole ? (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {Object.entries(newRolePerms).map(([mod, perms]) => (
                  ...
                  >
                    <h4 className="font-semibold text-gray-800 capitalize mb-3 border-b border-gray-200 pb-2">
                      {mod}
                    </h4>
                      ...
                      {Object.keys(perms).map((perm) => (
                        ...
                        >
                          <input
                            ...
                            checked={
                              (
                                newRolePerms as unknown as Record<
                                  string,
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl relative">
            ...
            {deletingRole.userCount > 0 ? (
              <>
                <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-800 mb-4 font-medium flex gap-2">
                  ...
                  <p>
                    {deletingRole.userCount} users currently have this role.
                    They will lose access immediately.
                  </p>
                </div>
                <p className="text-sm text-gray-700 mb-2">
                  Please type <strong>DELETE</strong> to confirm deletion of the
                  "{deletingRole.name}" role.
                </p>
                <input
                  type="text"
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button
                ...
                disabled={
                  isDeleting ||
                  (deletingRole.userCount > 0 && deleteConfirmText !== "DELETE")
                }
