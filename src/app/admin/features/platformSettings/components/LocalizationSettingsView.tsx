...
export function LocalizationSettingsView({
  ...
  const updateLoc = (key: keyof SystemConfig["localization"], value: any) => {
    setConfig(prev => ({ ...prev, localization: { ...prev.localization, [key]: value } }));
  };

  const inputClass = "w-full text-sm p-3 border border-gray-200 rounded-xl bg-white outline-none focus:border-indigo-400 transition-colors";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100">
              {["English", "Tamil", "Hindi", "Arabic", "French"].map(lang => (
                <label key={lang} className="flex items-center gap-2 text-sm font-medium text-gray-700 cursor-pointer">
                  <input 
                    type="checkbox" 
                    checked={config.localization.supportedLanguages.includes(lang)}
                    onChange={(e) => {
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select value={config.localization.dateFormat} onChange={e => updateLoc("dateFormat", e.target.value)} className={inputClass}>
            ...
            <div className="space-y-1.5">
              ...
              <select value={config.localization.calendarFormat} onChange={e => updateLoc("calendarFormat", e.target.value)} className={inputClass}>
          ...
          <div className="space-y-1.5">
            ...
            <select value={config.localization.numberFormat} onChange={e => updateLoc("numberFormat", e.target.value)} className={inputClass}>
              <option value="India">Indian (1,00,000)</option>
        ...
        <div className="bg-indigo-50/40 border border-indigo-100 rounded-2xl p-6 flex flex-col justify-between space-y-6">
          ...
          <div className="space-y-3 bg-white p-5 rounded-xl border border-indigo-100 shadow-sm">
            {[
              { label: "Default Language", val: config.localization.defaultLanguage },
              { label: "Date Preview", val: getFormattedDatePreview() },
              { label: "Number Scale", val: getFormattedNumberPreview() },
              { label: "Calendar", val: `${config.localization.calendarFormat} Calendar` }
            ].map(item => (
          ...
          <div className="p-4 bg-white rounded-xl border border-indigo-100 text-xs font-medium text-indigo-800 leading-relaxed">
            <strong>Tamil Language Support:</strong> Enabling regional translations will dynamically adjust UI labels, dates, and number formats for localized users.
          </div>
        </div>
      </div>
    </div>
