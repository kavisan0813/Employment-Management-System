    ...
    <div className="space-y-6 font-semibold">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-white border border-indigo-100 rounded-2xl p-5 space-y-4 shadow-sm">
        <h4 className="text-xs font-semibold text-indigo-900 uppercase tracking-wide flex items-center gap-1.5">
          <Calendar className="w-4 h-4 text-indigo-600" /> Subscription Expiry
          SLA Schedule
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 text-xs text-gray-700">
          {[
            ...
            <div
              key={item.bg}
              className={`${item.bg} border border-black/5 p-3 rounded-xl shadow-inner`}
            >
          ...
          <tbody className="divide-y divide-gray-100 text-gray-700">
            {filtered.map((alert) => (
              ...
              >
                ...
                <td className="px-5 py-4">
                  <span
                    className={`text-[10px] font-semibold px-2 py-0.5 rounded border ${getBadgeStyle(alert.daysLeft)}`}
                  >
                    {alert.daysLeft === 0
                      ? "Expired"
                ...
                <td className="px-5 py-4">
                  <span
                    className={`text-[10px] font-semibold ${alert.status === "notified" ? "text-indigo-600" : alert.daysLeft === 0 ? "text-rose-600" : "text-gray-500"}`}
                  >
                ...
                <td className="px-5 py-4 text-right">
                  {alert.daysLeft > 0 ? (
                    <button
                      ...
                      className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-semibold rounded-lg shadow-sm ml-auto"
                    >
