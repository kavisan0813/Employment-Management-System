...
import { ShieldAlert, CheckCircle2, Search, Globe } from "lucide-react";
...
export function SecurityAlertsView({
  ...
  const filtered = securityAlerts.filter(
    (a) =>
  ...
  return (
    <div className="space-y-6 font-semibold">
                  ...
                  {alert.actionTaken === "none" ? (
                    <div className="flex gap-1.5 justify-end">
                      <button
                        ...
                        className="px-3 py-1.5 bg-white border border-gray-200 hover:bg-gray-50 text-[10px] font-semibold text-gray-700 rounded-lg shadow-sm transition-all"
                      >
                        Warn User
                      </button>
                      <button
                        onClick={() => onSecurityAction(alert.id, "locked")}
                        className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white text-[10px] font-semibold rounded-lg shadow-sm transition-all"
                      >
                        Lock Account
                      </button>
                    </div>
      ...
      <div className="p-4 bg-rose-50/50 border border-rose-100 rounded-xl text-xs text-rose-900 leading-relaxed font-medium flex items-start gap-3">
        ...
        <div>
          <strong>Brute Force & Geo-Location Policy:</strong> Restricting
          accounts blocks logins immediately. Locked users must contact support
          or complete MFA validation resets to recover workspace sessions.
        </div>
      </div>
    </div>
  );
