...
import { 
  Users, ShieldAlert, AlertTriangle, Download, 
  CheckCircle, Globe, Terminal, ShieldAlert as AlertIcon
} from "lucide-react";
import { LoginLog, SecurityEventLog, ErrorLog, LogsStats } from "../types/logs.types";
...
export function LogsDashboard({ stats, loginLogs, securityEvents, errorLogs }: LogsDashboardProps) {
  // Get active sessions
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-2xs relative overflow-hidden transition-all duration-200 hover:shadow-xs">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total Logins</p>
              <h3 className="text-2xl font-bold text-gray-950 mt-1">{stats.totalLogins}</h3>
            </div>
          ...
          <div className="flex items-center gap-1.5 mt-3 text-xs">
            ...
            <span className="text-teal-700 font-semibold">{stats.loginSuccessRate}% success rate</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-2xs relative overflow-hidden transition-all duration-200 hover:shadow-xs">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-950 mt-1">{stats.activeSessions}</h3>
            </div>
        ...
        {/* Security Incidents */}
        <div className={`rounded-xl border p-5 shadow-2xs transition-all duration-200 hover:shadow-xs ${
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className={`text-2xl font-bold mt-1 ${
                stats.criticalSecurityEvents > 0 ? "text-rose-600 animate-pulse" : "text-gray-950"
              }`}>{activeAlerts.length}</h3>
            </div>
            <div className={`p-3 rounded-xl ${
              stats.criticalSecurityEvents > 0 ? "bg-rose-100 text-rose-600" : "bg-gray-50 text-gray-500"
            }`}>
              <ShieldAlert className="w-5 h-5" />
            </div>
          </div>
          <div className="flex items-center gap-1.5 mt-3 text-xs">
            <span className={stats.criticalSecurityEvents > 0 ? "text-rose-700 font-bold" : "text-gray-500"}>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-2xs relative overflow-hidden transition-all duration-200 hover:shadow-xs">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-950 mt-1">{stats.errorRateToday}</h3>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-2xs">
            ...
            <div className="divide-y divide-gray-100 max-h-[300px] overflow-y-auto">
              {activeSessionsList.length === 0 ? (
                <div className="p-6 text-center text-xs text-gray-400">No active user sessions found.</div>
              ) : (
                activeSessionsList.map(session => (
                  <div key={session.id} className="p-4 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-xs font-semibold text-gray-900">{session.user}</div>
                        <div className="text-[10px] text-gray-400 mt-0.5">{session.email} &bull; {session.organization}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-[10px] font-mono text-gray-500">{session.ipAddress}</div>
                      <div className="text-[10px] text-gray-400 mt-0.5">{session.device} &bull; {session.browser}</div>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-2xs">
            ...
            <div className="h-48 flex items-end justify-between gap-1 pt-6 px-2">
              {[35, 48, 62, 51, 80, 95, 70, 60, 45, 90, 110, 85, 75, 98, 120, 115, 80, 92, 110, 125, 140, 105, 90, 95].map((val, idx) => (
                <div key={idx} className="flex-1 flex flex-col items-center group relative cursor-pointer">
                  <div className="w-full bg-indigo-100 hover:bg-indigo-500 rounded-t transition-all duration-200" style={{ height: `${(val / 150) * 100}%` }} />
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-2xs">
            ...
            <div className="divide-y divide-gray-100">
              {activeAlerts.length === 0 ? (
                <div className="p-8 text-center text-xs text-gray-400">No active security concerns detected.</div>
              ) : (
                activeAlerts.slice(0, 3).map(alert => (
                  <div key={alert.id} className="p-4 space-y-2 hover:bg-rose-50/5 transition-colors">
                    <div className="flex items-center justify-between">
                    ...
                    <p className="text-[11px] text-gray-500 leading-relaxed">{alert.details}</p>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-2xs">
            ...
            <div className="divide-y divide-gray-100">
              {recentErrors.length === 0 ? (
                <div className="p-8 text-center text-xs text-gray-400">Zero error logs registered.</div>
              ) : (
                recentErrors.map(err => (
                  <div key={err.id} className="p-4 hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-[10px] font-bold text-rose-600">{err.errorCode}</span>
                      <span className="text-[10px] text-gray-400">
                        {new Date(err.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>
                    <p className="text-xs font-semibold text-gray-800 mt-1 truncate">{err.message}</p>
                    <p className="text-[9px] font-mono text-gray-400 mt-0.5 truncate">{err.path}</p>
                  </div>
                ))
              )}
            </div>
