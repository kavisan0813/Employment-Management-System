...
import { Search, ShieldAlert, ShieldCheck, Clock, RefreshCw } from "lucide-react";
...
export function SecurityEventsTable({
  ...
  const filtered = events.filter(evt => {
    ...
    const matchesOrg = selectedOrg === "ALL" || evt.organization === selectedOrg;
    ...
    return matchesSearch && matchesOrg && matchesSeverity && matchesStatus && matchesDate;
  });
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map(evt => {
            ...
            const getStatusClass = (status: string) => {
              switch (status) {
                case "Resolved": return "bg-teal-50 text-teal-700 border border-teal-200";
                case "Pending": return "bg-amber-50 text-amber-700 border border-amber-200";
                default: return "bg-rose-50 text-rose-700 border border-rose-200";
              }
            };
              ...
              >
                ...
                <div className="flex gap-4 items-start">
                  <div className={`p-3 rounded-xl shrink-0 ${
                    ...
                    {evt.status === "Resolved" ? <ShieldCheck className="w-6 h-6" /> : <ShieldAlert className="w-6 h-6" />}
                  </div>

                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`px-2 py-0.5 border text-[9px] font-bold uppercase rounded ${getSeverityClass(evt.severity)}`}>
                        {evt.severity} Severity
                      </span>
                      <span className={`px-2 py-0.5 border text-[9px] font-bold uppercase rounded ${getStatusClass(evt.status)}`}>
                        {evt.status}
                      </span>
                      <span className="text-[10px] text-gray-400 font-mono flex items-center gap-1">
                        <Clock className="w-3 h-3" />
                        {new Date(evt.detectedAt).toLocaleString().replace(",", "")}
                      </span>
                    </div>

                    <h4 className="text-xs font-bold text-gray-900 mt-1">{evt.type}</h4>
                    <p className="text-xs text-gray-600 leading-relaxed max-w-2xl">{evt.details}</p>

                    <div className="text-[10px] text-gray-400 font-mono pt-1">
                      Target Organization: <strong className="text-gray-600">{evt.organization || "Platform-wide"}</strong> &bull; 
                      Actor: <strong className="text-gray-600">{evt.actor}</strong> &bull; 
                      IP: <strong className="text-gray-600">{evt.ipAddress}</strong>
                    </div>
                  </div>
                </div>
                ...
                <div className="shrink-0 flex items-center gap-3 pt-3 md:pt-0 border-t md:border-t-0 border-gray-100 justify-end">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider hidden md:inline">Action</span>
