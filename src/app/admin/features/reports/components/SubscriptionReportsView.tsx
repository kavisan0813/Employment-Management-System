...
export function SubscriptionReportsView() {
  const plans = [
    {
      name: "Basic Plan",
      count: 200,
      ...
      <div className="p-6 flex-1 overflow-y-auto flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans.map((p) => (
            <div
              key={p.name}
              className={`p-5 border rounded-xl flex flex-col justify-between h-28 ${p.color}`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="border border-gray-200 rounded-xl p-5 bg-white space-y-4">
            <h4 className="text-sm font-semibold text-gray-700 flex items-center gap-1.5">
              <Sparkles className="w-4 h-4 text-indigo-500" /> Trial Conversion
              Funnel
            </h4>
            <div className="grid grid-cols-3 gap-3">
              {[
                { l: "Trial Users", v: "100", c: "bg-gray-50" },
                { l: "Converted", v: "40", c: "bg-teal-50" },
                { l: "Expired", v: "60", c: "bg-rose-50" },
              ].map((item) => (
                <div
                  key={item.l}
                  className={`p-3 ${item.c} border border-gray-100 rounded-lg text-center`}
                >
            ...
            <div className="divide-y divide-gray-100">
              {[
                ...
                >
                  ...
                  <span
                    className={`font-medium px-2 py-0.5 rounded ${item.color}`}
                  >
                    {item.val}
                  </span>
                </div>
