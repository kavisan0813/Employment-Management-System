...
import { 
  ...
  ArrowRight, ShieldAlert, BarChart3, Activity 
} from "lucide-react";
...
export function DashboardOverview({ setActiveTab }: DashboardOverviewProps) {
  ...
  const kpis = [
    { id: "organizations", label: "Total Organizations", value: "500", subText: "Active: 450 | Trial: 30", icon: Building2, color: "text-indigo-600 bg-white", bg: "bg-indigo-50/50", border: "border-indigo-100", tab: "organizations" },
  ...
  const quickLinks = [
    { label: "Organization Growth", desc: "View tenant signup metrics.", icon: Building2, tab: "organizations" },
    { label: "Attrition Analytics", desc: "Headcount and resignation data.", icon: Users, tab: "employees" },
    { label: "MRR & Revenue Ledger", desc: "Income and refund distributions.", icon: CreditCard, tab: "revenue" },
    { label: "System Usage", desc: "DAU/MAU ratios and storage.", icon: Activity, tab: "usage" },
    { label: "Dynamic Reporting", desc: "Custom organization filters.", icon: BarChart3, tab: "custom" },
    { label: "Executive Dashboard", desc: "CLV, CAC, Churn, and renewals.", icon: ShieldAlert, tab: "executive" }
  ] as const;
  ...
  return (
    <div className="bg-slate-50/50 rounded-2xl shadow-sm border border-gray-100 min-h-[600px] overflow-hidden flex flex-col font-medium animate-in fade-in zoom-in-95 duration-200">
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start">
                <span className="text-xs font-semibold text-gray-600 uppercase tracking-wider">{kpi.label}</span>
                <div className={`p-2 rounded-lg ${kpi.color} shadow-sm`}>
                  <Icon className="w-4 h-4" />
                </div>
              </div>
              <div>
                <h4 className="text-xl font-semibold text-gray-800">{kpi.value}</h4>
                <p className="text-xs font-medium text-gray-500 mt-1">{kpi.subText}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        <h3 className="text-xs font-semibold uppercase text-gray-500 tracking-wider mb-5">Quick Link Directory</h3>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h5 className="text-xs font-semibold text-gray-700 group-hover:text-indigo-800">{link.label}</h5>
                    <p className="text-xs font-medium text-gray-500 mt-0.5">{link.desc}</p>
                  </div>
                </div>
