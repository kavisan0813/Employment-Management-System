...
export function ExportCenterView({
  ...
  const handleDownload = (filename: string, ext: string) => {
    triggerAlert(
      `Initiating file transfer. Downloading '${filename}.${ext.toLowerCase()}'.`,
      "success",
    );
  };

  const filteredExports = state.exports.filter(
    (exp) =>
                  ...
                  {filteredExports.map((exp) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-right flex items-center justify-end gap-3">
                        <button
                          onClick={() =>
                            handleDownload(exp.reportName, exp.exportType)
                          }
