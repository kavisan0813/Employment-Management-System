    ...
    <div className="fixed top-4 right-4 z-[9999] flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        ...
        >
          ...
          {t.type === "info"    && <Sparkles     size={18} className="text-sky-400 shrink-0 mt-0.5" />}
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold">{t.title}</p>
            <p className="text-xs opacity-75 mt-0.5 leading-relaxed">{t.message}</p>
          </div>
      ...
      <div className="flex flex-col gap-1.5">
        {segments.map((seg) => (
          <div key={seg.label} className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-muted-foreground font-medium truncate max-w-[80px]">{seg.label}</span>
...
function TransferProgressBar({ progress, status }: { progress: number; status: string }) {
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-1">
        <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">
          {status === "Paid" ? "Transfer Complete" : status === "On Hold" ? "On Hold" : "Awaiting Transfer"}
        </span>
        <span className="text-[9px] font-bold text-emerald-500">{progress}%</span>
      </div>
      <div className="w-full h-1 bg-muted/50 rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-1000 ${
            status === "On Hold" ? "bg-amber-500" : "bg-gradient-to-r from-emerald-500 to-cyan-500"
          }`}
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black text-foreground">Salary Payslip</h3>
...
function AIInsightsPanel({ employees }: { employees: PayrollEmployee[] }) {
  const insights = useMemo(() => {
    ...
    const avgNet  = employees.reduce((s, e) => s + e.net, 0) / (employees.length || 1);
    ...
    return [
      ...
      },
      {
        ...
        text: topEarner ? `Top earner this cycle: ${topEarner.name} (${topEarner.department}) at ₹${topEarner.gross.toLocaleString()} gross.` : "",
        color: "amber",
      },
    ].filter((i) => i.text);
  }, [employees]);
  ...
  return (
    <div className="bg-card border border-border rounded-3xl p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h3 className="text-sm font-black text-foreground">AI Payroll Insights</h3>
          <p className="text-[10px] text-muted-foreground">Powered by NexusAI Engine</p>
        </div>
      ...
      <div className="space-y-2">
        {insights.map((ins, i) => (
          <div key={i} className={`flex items-start gap-2.5 p-2.5 rounded-xl bg-${ins.color}-500/5 border border-${ins.color}-500/10`}>
            <div className={`w-6 h-6 rounded-lg bg-${ins.color}-500/10 flex items-center justify-center shrink-0`}>
              {ins.icon}
            </div>
...
function RunPayrollModal({
  onClose, month, year, employees, onComplete,
}: {
  onClose: () => void;
  month: string;
  year: string;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      i++;
      setProgress(Math.min(100, Math.round((i / Math.max(names.length, 1)) * 100)));
      setCurrentEmp(names[Math.min(i - 1, names.length - 1)] || "");
      if (i >= names.length) {
        clearInterval(interval);
        setTimeout(() => { setStep("success"); onComplete(); }, 600);
      }
    }, 500);
      ...
      >
        ...
        {step === "confirm" && (
          <>
            <div className="p-8">
              ...
              <h3 className="text-2xl font-black text-foreground">Process Payroll</h3>
              <p className="text-sm text-muted-foreground mt-1">
                Initiating disbursement for <span className="text-foreground font-bold">{month} {year}</span>
              </p>

              <div className="mt-6 rounded-2xl border border-border bg-muted/20 divide-y divide-border/50">
                {[
                  { label: "Pending Employees", value: pendingEmps.length.toString() },
                  { label: "Gross Payout",      value: `₹${pendingEmps.reduce((s,e) => s+e.gross,0).toLocaleString()}` },
                  { label: "Net Disbursement",  value: `₹${totalNet.toLocaleString()}`, highlight: true },
                ].map((row) => (
                  <div key={row.label} className="flex items-center justify-between px-5 py-3">
                    <span className="text-xs text-muted-foreground font-medium">{row.label}</span>
                    <span className={`text-sm font-black ${row.highlight ? "text-emerald-500" : "text-foreground"}`}>{row.value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-4 flex items-start gap-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20">
                ...
                <p className="text-xs text-amber-600 leading-relaxed">Disbursement is immediate and irreversible. Ensure all records are verified.</p>
              </div>
            </div>

            <div className="px-8 py-5 flex gap-3 border-t border-border">
              <button onClick={onClose} className="flex-1 py-3 rounded-xl text-sm font-bold border border-border hover:bg-muted transition-colors">Cancel</button>
              <button
                onClick={() => { if (pendingEmps.length === 0) return; setStep("processing"); }}
              ...
              >
                {pendingEmps.length === 0 ? "All Processed" : "Confirm & Disburse"}
              </button>
            </div>
          </>
        )}
        ...
        {step === "processing" && (
          <div className="p-10 flex flex-col items-center text-center">
            ...
            <h3 className="text-xl font-black text-foreground mb-2">Processing Transfers</h3>
            ...
            <div className="flex justify-between w-full text-[10px] text-muted-foreground font-medium">
              <span className="truncate max-w-[200px]">{currentEmp && `Transferring to ${currentEmp}...`}</span>
              <span>{progress}%</span>
            </div>
          </div>
        )}
        ...
        {step === "success" && (
          <div className="p-10 flex flex-col items-center text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2 max-w-xs">
              <span className="text-foreground font-bold">{month} {year}</span> payroll has been processed successfully. ₹{totalNet.toLocaleString()} transferred.
            </p>
            <button
              onClick={onClose}
...
function EditPayrollModal({
  ...
  const handleSave = () => {
    if (!validate()) return;
    onSave({ ...employee, gross: grossNum, deductions: deductNum, net: netPay, bonus: bonusNum, bankAccount, status });
  };
    ...
    <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div className="flex items-center gap-3">
            <img src={employee.avatar} alt={employee.name} className="w-9 h-9 rounded-xl object-cover" />
            <div>
              <h2 className="text-sm font-black text-foreground">{employee.name}</h2>
              <p className="text-xs text-muted-foreground">{employee.id} · {employee.designation}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-muted transition-colors text-muted-foreground"><X size={16} /></button>
        </div>

        <div className="p-6 space-y-4">
          {/* Gross */}
          <div>
            <label className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1.5 block">Gross Salary (₹)</label>
            <input
              type="number" value={gross} min="0"
          ...
          {/* Bonus */}
          <div>
            <label className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1.5 block">Performance Bonus (₹)</label>
            <input
              type="number" value={bonus} min="0"
            ...
            />
            {errors.bonus && <p className="text-xs text-red-500 mt-1">{errors.bonus}</p>}
          </div>

          {/* Deductions */}
          <div>
            <label className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1.5 block">Total Deductions (₹)</label>
            <input
              type="number" value={deductions} min="0"
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/20">
            <span className="text-sm text-muted-foreground font-medium">Calculated Net Pay</span>
            <span className={`text-lg font-black ${netPay >= 0 ? "text-emerald-500" : "text-red-500"}`}>
          ...
          {/* Bank Account */}
          <div>
            <label className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1.5 block">Bank Account (masked)</label>
            <input
          ...
          {/* Status */}
          <div>
            <label className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1.5 block">Payment Status</label>
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value as PayrollEmployee["status"])}
              className="w-full border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500 bg-transparent text-foreground transition-colors"
            >
        ...
        <div className="px-6 py-4 border-t border-border flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 border border-border rounded-xl text-sm font-bold hover:bg-muted transition-colors">Cancel</button>
          <button onClick={handleSave} className="flex-1 py-3 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white font-bold rounded-xl text-sm hover:opacity-90 shadow-lg shadow-emerald-500/20 active:scale-95 transition-all">
            Save Changes
          </button>
        </div>
      </div>
...
/* ─── Stat Card ──────────────────────────── */
function StatCard({ label, value, sub, trend, isUp, sparkData, color, icon }: {
  label: string; value: string; sub: string;
  trend: string; isUp: boolean; sparkData: number[];
  ...
  return (
    <div className="group relative bg-card border border-border rounded-3xl p-6 overflow-hidden hover:shadow-xl transition-all duration-300">
      <div className={`absolute -right-6 -top-6 w-28 h-28 rounded-full bg-${color}-500/5 group-hover:scale-150 transition-transform duration-700`} />
      <div className="relative">
        <div className="flex items-center justify-between mb-3">
          <div className={`w-9 h-9 rounded-xl bg-${color}-500/10 flex items-center justify-center text-${color}-500`}>
            {icon}
          </div>
          <div className={`flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full ${isUp ? `bg-emerald-500/10 text-emerald-500` : `bg-red-500/10 text-red-500`}`}>
        ...
        <div className="mt-4">
          <MiniBarChart data={sparkData} color={`var(--${color === "emerald" ? "emerald" : color === "rose" ? "red" : "sky"}-500, #10b981)`} />
        </div>
      </div>
    </div>
  );
...
export function Payroll() {
  ...
  const [sortField, setSortField]         = useState<"name" | "gross" | "net" | "deductions">("name");
  ...
  useEffect(() => {
    const close = () => { setOpenActionId(null); setShowMonthDropdown(false); setShowYearDropdown(false); };
    document.addEventListener("click", close);
    return () => document.removeEventListener("click", close);
  }, []);

  /* ── Reset page on filter change ── */
  useEffect(() => { setCurrentPage(1); setSelectedIds(new Set()); }, [statusFilter, searchQuery, deptFilter, payRangeFilter]);
  ...
  const departments = useMemo(
    () => ["All", ...Array.from(new Set(employeesData.map((e) => e.department)))],
    [employeesData],
  );
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employeesData
      .filter((emp) => {
        ...
        const matchRange  =
          payRangeFilter === "All" ? true
          : payRangeFilter === "0-50k"   ? emp.net < 50000
          : payRangeFilter === "50-100k" ? emp.net >= 50000 && emp.net < 100000
          : emp.net >= 100000;
        const matchSearch =
  ...
  }, [statusFilter, deptFilter, payRangeFilter, searchQuery, employeesData, sortField, sortDir]);
  ...
  const pendingCount    = employeesData.filter((e) => e.status === "Pending").length;
  const onHoldCount     = employeesData.filter((e) => e.status === "On Hold").length;

  /* ── Department donut data ── */
  const deptDonutData = useMemo(() => {
    const map: Record<string, number> = {};
    employeesData.forEach((e) => { map[e.department] = (map[e.department] || 0) + e.gross; });
    return Object.entries(map).map(([dept, val]) => ({
      label: dept, value: val, color: DEPT_COLORS[dept] || "#6b7280",
    }));
  }, [employeesData]);
  ...
  const getLeaveImpact = (empId: string) => {
    ...
    const leaves  = leaveRequests.filter((lr) => lr.employee.includes(empName) && lr.status === "Approved");
  ...
  const bulkMarkPaid = () => {
    ...
    setEmployeesData((prev) => prev.map((e) =>
      selectedIds.has(e.id) ? { ...e, status: "Paid" as const, transferProgress: 100 } : e,
    ));
    addToast({ type: "success", title: "Bulk Update", message: `${selectedIds.size} employee(s) marked as Paid.` });
  ...
  const bulkHold = () => {
    ...
    addToast({ type: "warning", title: "Payments On Hold", message: `${selectedIds.size} payment(s) placed on hold.` });
    setSelectedIds(new Set());
  };

  /* ── CSV Export ── */
  const exportCSV = () => {
    const headers = ["ID","Name","Designation","Department","Gross","Bonus","Deductions","Net Pay","Status"];
    const rows = filteredEmployees.map((e) => [e.id, e.name, e.designation, e.department, e.gross, e.bonus||0, e.deductions, e.net, e.status]);
    ...
    addToast({ type: "info", title: "Export Ready", message: `Payroll data for ${selectedMonth} ${selectedYear} exported.` });
  };
    ...
    <>
      ...
      <div className="w-full px-4 md:px-8 py-6 pb-20">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 mb-8">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h1 className="text-3xl font-black text-foreground tracking-tight">Payroll Intelligence</h1>
            ...
            <p className="text-muted-foreground text-sm font-medium pl-13">
              Real-time payroll processing · tax management · smart disbursements
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              ...
              >
                ...
                {selectedMonth}
                <ChevronDown size={12} className={`text-muted-foreground transition-transform ${showMonthDropdown ? "rotate-180" : ""}`} />
              </button>
              {showMonthDropdown && (
                <div className="absolute top-full left-0 mt-2 w-44 bg-card border border-border rounded-2xl shadow-2xl z-50 py-2 animate-in slide-in-from-top-2">
                  {MONTHS.map((m) => (
                    <button key={m} onClick={() => { setSelectedMonth(m); setShowMonthDropdown(false); }}
            ...
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              ...
              >
                {selectedYear}
                <ChevronDown size={12} className={`text-muted-foreground transition-transform ${showYearDropdown ? "rotate-180" : ""}`} />
              </button>
              {showYearDropdown && (
        ...
        <div className="flex items-center gap-3 mb-6 overflow-x-auto">
          {[
            { label: `${paidCount} Paid`,      icon: <BadgeCheck size={12} />,    color: "emerald" },
            { label: `${pendingCount} Pending`, icon: <Clock size={12} />,         color: "amber"   },
            { label: `${onHoldCount} On Hold`,  icon: <Lock size={12} />,          color: "red"     },
            { label: "TDS Filing: Apr 15",       icon: <AlertTriangle size={12} />, color: "orange"  },
            { label: "Next Payroll: 8 days",     icon: <Calendar size={12} />,      color: "sky"     },
          ].map((item) => (
            <div key={item.label} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-${item.color}-500/10 border border-${item.color}-500/20 text-${item.color}-600 text-xs font-bold whitespace-nowrap shrink-0`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6 animate-in slide-in-from-top-2 duration-300">
            ...
            <div className="bg-card border border-border rounded-3xl p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-black text-foreground">Dept. Distribution</h3>
              </div>
        ...
        <div className="bg-card border border-border rounded-3xl p-2 mb-4 flex flex-col md:flex-row items-center gap-3">
            ...
            {(["All", "Paid", "Pending", "On Hold"] as const).map((s) => (
              <button
                ...
                className={`flex-1 md:flex-none px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                  statusFilter === s ? "bg-card text-emerald-500 shadow-sm" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {s}
              </button>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto md:ml-auto pr-2">
            ...
            <button onClick={exportCSV} className="p-2.5 rounded-xl bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all" title="Export CSV">
              <Download size={16} />
            </button>
          </div>
        </div>
        ...
        {isFilterOpen && (
          <div className="bg-card border border-border rounded-3xl p-5 mb-4 animate-in slide-in-from-top-2 shadow-sm" onClick={(e) => e.stopPropagation()}>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                <label className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1.5 block">Department</label>
              ...
              <div>
                <label className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1.5 block">Net Pay Range</label>
                <select value={payRangeFilter} onChange={(e) => setPayRangeFilter(e.target.value)}
                  className="w-full bg-muted/30 border border-border rounded-xl px-3 py-2.5 text-sm outline-none focus:border-emerald-500 text-foreground">
              ...
              <div>
                <label className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1.5 block">Sort By</label>
              ...
              <div className="flex items-end gap-2">
                <button onClick={() => { setDeptFilter("All"); setPayRangeFilter("All"); setIsFilterOpen(false); }}
                  className="flex-1 py-2.5 rounded-xl border border-border text-sm font-bold hover:bg-muted transition-colors">Reset</button>
                <button onClick={() => setIsFilterOpen(false)}
                  className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-500 text-white text-sm font-bold hover:opacity-90 transition-all">Apply</button>
              </div>
            </div>
          </div>
        )}
        ...
        {selectedIds.size > 0 && (
          <div className="mb-4 p-3 bg-indigo-950/80 border border-indigo-500/30 rounded-2xl flex items-center gap-3 animate-in slide-in-from-top-2 backdrop-blur-xl">
            ...
            <div className="flex items-center gap-2 ml-auto">
              <button onClick={bulkMarkPaid} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500/20 text-emerald-300 text-xs font-bold hover:bg-emerald-500/30 transition-colors">
                <UserCheck size={12} /> Mark Paid
              </button>
              <button onClick={bulkHold} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500/20 text-amber-300 text-xs font-bold hover:bg-amber-500/30 transition-colors">
                <Lock size={12} /> Hold
              </button>
              <button onClick={() => setSelectedIds(new Set())} className="p-1.5 rounded-xl hover:bg-white/10 text-indigo-300 transition-colors">
                <X size={14} />
              </button>
            </div>
          </div>
            ...
            <table className="w-full text-left min-w-[1100px]">
              <thead>
                <tr className="bg-muted/30 border-b border-border">
                  ...
                  <th className="px-5 py-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Employee</th>
                  <th className="px-5 py-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Department</th>
                  <th
                  ...
                  >
                    <span className="flex items-center justify-end gap-1">Base Salary <SortIcon field="gross" /></span>
                  </th>
                  <th className="px-5 py-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-right">Leave Impact</th>
                  <th
                    className="px-5 py-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-right cursor-pointer hover:text-foreground"
                    onClick={() => handleSort("deductions")}
                  >
                    <span className="flex items-center justify-end gap-1">Deductions <SortIcon field="deductions" /></span>
                  </th>
                  <th
                    className="px-5 py-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-right cursor-pointer hover:text-foreground"
                    onClick={() => handleSort("net")}
                  >
                    <span className="flex items-center justify-end gap-1">Net Pay <SortIcon field="net" /></span>
                  </th>
                  <th className="px-5 py-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Status</th>
                  <th className="px-5 py-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-center">Actions</th>
                </tr>
              </thead>
                  ...
                  <tr>
                    <td colSpan={9} className="py-16 text-center text-muted-foreground">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <button onClick={() => { setSearchQuery(""); setStatusFilter("All"); setDeptFilter("All"); }}
                          className="text-xs text-emerald-500 hover:underline font-bold">Clear Filters</button>
                      </div>
                    </td>
                  </tr>
                )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="relative">
                            <img src={emp.avatar} alt={emp.name} className="w-10 h-10 rounded-xl object-cover border border-border" />
                            <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-card ${
                              emp.status === "Paid" ? "bg-emerald-500" : emp.status === "On Hold" ? "bg-amber-500" : "bg-slate-400"
                            }`} />
                          </div>
                          <div>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5">
                          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: DEPT_COLORS[emp.department] || "#6b7280" }} />
                      ...
                      <td className="px-5 py-4 text-right">
                        ...
                        {emp.bonus && emp.bonus > 0 && (
                          <p className="text-[9px] text-emerald-500 font-bold">+₹{emp.bonus.toLocaleString()} bonus</p>
                        )}
                      </td>
                      ...
                      <td className="px-5 py-4 text-right">
                        {impact.amount > 0 ? (
                          <div>
                            <span className="text-xs font-bold text-rose-500">-₹{impact.amount.toLocaleString()}</span>
                          ...
                          <span className="text-[10px] text-emerald-500 font-bold">No Impact</span>
                        )}
                      </td>
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="flex flex-col items-end gap-1">
                          ...
                          <TransferProgressBar progress={emp.transferProgress || (emp.status === "Paid" ? 100 : 0)} status={emp.status} />
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                          emp.status === "Paid"    ? "bg-emerald-500/10 text-emerald-600"
                          : emp.status === "On Hold" ? "bg-amber-500/10 text-amber-600"
                          : "bg-slate-500/10 text-slate-600"
                        }`}>
                          {emp.status === "Paid"    ? <BadgeCheck size={11} />
                          : emp.status === "On Hold" ? <Lock size={11} />
                          : <Clock size={11} />}
                          {emp.status}
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          {/* More menu */}
                          <div className="relative" ref={openActionId === emp.id ? actionMenuRef : null}>
                            <button
                            ...
                            {openActionId === emp.id && (
                              <div
                                ...
                                onClick={(e) => e.stopPropagation()}
                              >
                                <button onClick={() => { setOpenActionId(null); setEditingEmp(emp); }}
                                  ...
                                  <Edit2 size={14} className="text-indigo-500" /> Edit Entry
                                </button>
                                <button onClick={() => {
                                ...
                                {emp.status !== "Paid" && (
                                  <button onClick={() => {
                                    setOpenActionId(null);
                                    setEmployeesData((prev) => prev.map((e) => e.id === emp.id ? { ...e, status: "Paid" as const, transferProgress: 100 } : e));
                                    addToast({ type: "success", title: "Marked as Paid", message: `${emp.name}'s payment has been disbursed.` });
                                  }}
                                ...
                                <div className="border-t border-border/50">
                                  <button onClick={() => {
                                    setOpenActionId(null);
                                    setEmployeesData((prev) => prev.map((e) => e.id === emp.id ? { ...e, status: "On Hold" as const } : e));
                                    addToast({ type: "warning", title: "Payment On Hold", message: `${emp.name}'s disbursement is now on hold.` });
                                  }}
              ...
              {filteredEmployees.length > 0 && (
                <tfoot>
                  <tr className="bg-gradient-to-r from-emerald-500/5 to-cyan-500/5 border-t-2 border-emerald-500/20">
                    ...
                    <td className="px-5 py-4 text-right font-black text-foreground text-sm">₹{totalGross.toLocaleString()}</td>
                    <td className="px-5 py-4" />
                    <td className="px-5 py-4 text-right font-black text-rose-500 text-sm">₹{totalDeductions.toLocaleString()}</td>
                    <td className="px-5 py-4 text-right">
                      <span className="text-lg font-black text-emerald-500 bg-emerald-500/10 px-3 py-1 rounded-xl">₹{totalNet.toLocaleString()}</span>
                    </td>
                    <td colSpan={2} />
                  </tr>
                </tfoot>
          ...
          {totalPages > 1 && (
            <div className="px-6 py-4 border-t border-border bg-muted/10 flex items-center justify-between">
              <p className="text-xs text-muted-foreground font-medium">
                Showing <span className="text-foreground font-bold">{(currentPage - 1) * itemsPerPage + 1}</span>–
                <span className="text-foreground font-bold">{Math.min(filteredEmployees.length, currentPage * itemsPerPage)}</span>{" "}
                of <span className="text-foreground font-bold">{filteredEmployees.length}</span>
              </p>
              <div className="flex items-center gap-1.5">
                <button
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <button
                    ...
                    className={`w-9 h-9 rounded-xl text-xs font-bold transition-all ${
                      currentPage === page ? "bg-gradient-to-r from-emerald-600 to-emerald-500 text-white shadow-lg shadow-emerald-500/25" : "text-muted-foreground hover:bg-muted"
                    }`}
                  >
                    {page}
                  </button>
        ...
        <div className="mt-6 bg-card border border-border rounded-3xl p-8 shadow-sm overflow-x-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Current cycle status for {selectedMonth} {selectedYear}</p>
            </div>
          ...
          <div className="relative flex justify-between min-w-[700px] px-6">
            ...
            {[
              ...
              <div key={i} className="flex flex-col items-center gap-2 relative z-10 w-28">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center border-2 transition-all ${
                  step.status === "completed" ? "bg-emerald-500 border-emerald-500 shadow-lg shadow-emerald-500/30"
                  : step.status === "active"   ? "bg-card border-emerald-500 ring-4 ring-emerald-500/20"
                  : "bg-card border-border"
                }`}>
                  {step.status === "completed" ? <CheckCircle2 size={16} className="text-white" />
                  : step.status === "active"   ? <Loader2 size={16} className="text-emerald-500 animate-spin" />
                ...
                <div className="text-center">
                  ...
                  <p className="text-[9px] text-muted-foreground font-medium mt-0.5">{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {showRunModal && (
        <RunPayrollModal
          ...
          onComplete={() => {
            setEmployeesData((prev) => prev.map((e) =>
              e.status === "Pending" ? { ...e, status: "Paid" as const, transferProgress: 100 } : e,
            ));
            addToast({ type: "success", title: "Payroll Complete!", message: `All pending disbursements for ${selectedMonth} ${selectedYear} have been processed.` });
          }}
        />
      )}
      ...
      {editingEmp && (
        <EditPayrollModal
          ...
          onSave={(updated) => {
            ...
            setEditingEmp(null);
            addToast({ type: "success", title: "Record Updated", message: `Payroll entry for ${updated.name} has been saved.` });
          }}
        />
      )}
    </>
