          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-extrabold text-foreground flex items-center gap-2">
                {dept.name}
                <span className="text-xs font-black text-slate-400 border border-border px-2 py-0.5 rounded-lg">
                  {dept.code}
                </span>
              </h3>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="p-3 rounded-2xl border border-border bg-background text-center">
              ...
              <p className="text-base font-extrabold text-emerald-600 mt-1">
                {formatCurrency(dept.annualBudget - dept.budgetUsed)}
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredDepts.map((dept) => {
          const budgetPercent = Math.min(
            (dept.budgetUsed / dept.annualBudget) * 100,
            100,
          );
