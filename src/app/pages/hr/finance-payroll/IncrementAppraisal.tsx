...
const appraisalData: AppraisalEmployee[] = employees.map((emp, i) => {
  ...
  return {
    ...
    hrRemarks:
      increment > 0
        ? "Eligible for increment as per appraisal policy."
        : "Recommend review before approval.",
    workflowStep:
      statuses[i] === "Approved" ? 4 : statuses[i] === "Rejected" ? 2 : 2,
  };
});
      ...
      {WORKFLOW_STEPS.map((label, i) => {
        ...
        return (
          ...
          >
              ...
              {i < WORKFLOW_STEPS.length - 1 && (
                <div
                  style={{
                    width: "2px",
                    height: "24px",
                    backgroundColor: done ? "#059669" : "var(--border)",
                    marginTop: "2px",
                  }}
                />
              )}
            ...
            <div
              style={{
                paddingBottom: i < WORKFLOW_STEPS.length - 1 ? "16px" : "0",
                paddingTop: "4px",
              }}
            >
              <p
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
          ...
          <DrawerSection
            ...
            icon={<CalendarCheck size={14} color="var(--primary)" />}
          ...
          >
            ...
            >
              {[
                {
                  label: "Attendance %",
                  value: `${emp.attendancePct}%`,
                  color:
                    emp.attendancePct >= 93
                      ? "#059669"
                      : emp.attendancePct >= 85
                        ? "#D97706"
                        : "#DC2626",
                },
          ...
          <DrawerSection
            title="Increment Recommendation"
            icon={<IndianRupee size={14} color="#059669" />}
          ...
          >
            ...
            >
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <p
                    style={{
                      ...
                      fontWeight: 900,
                      color:
                        emp.recommendedIncrement > 0 ? "#059669" : "#DC2626",
                      margin: 0,
                    }}
                  >
                    {emp.recommendedIncrement > 0
              ...
              >
                ...
                >
                  ...
                  >
                    {(emp.attendancePct * 0.4).toFixed(1)} pts
                  </p>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  >
                    {(emp.performanceScore * 0.6).toFixed(1)} pts
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                >
                  ₹{emp.revisedSalary.toLocaleString()}
                </span>
              </div>
              <div
                style={{
          ...
          <DrawerSection
            ...
            icon={<Info size={14} color="#D97706" />}
          >
          ...
          <DrawerSection
            title="HR Remarks"
            icon={<Star size={14} color="#D97706" />}
          >
            <div
              style={{
                backgroundColor: "var(--secondary)",
...
export function IncrementAppraisal() {
  ...
  const [selectedEmp, setSelectedEmp] = useState<AppraisalEmployee | null>(
    null,
  );
  ...
  const [applyTarget, setApplyTarget] = useState<AppraisalEmployee | null>(
    null,
  );
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (yearDRef.current && !yearDRef.current.contains(e.target as Node))
        setShowYearDrop(false);
      if (deptDRef.current && !deptDRef.current.contains(e.target as Node))
        setShowDeptDrop(false);
      if (statusDRef.current && !statusDRef.current.contains(e.target as Node))
        setShowStatusDrop(false);
      if (bandDRef.current && !bandDRef.current.contains(e.target as Node))
        setShowBandDrop(false);
    };
  ...
  const kpiData = {
    ...
    avgAtt: Math.round(
      empList.reduce((s, e) => s + e.attendancePct, 0) / empList.length,
    ),
    avgPerf: Math.round(
      empList.reduce((s, e) => s + e.performanceScore, 0) / empList.length,
    ),
    payrollImpact: empList
      .filter((e) => e.status === "Approved")
      .reduce((s, e) => s + (e.revisedSalary - e.currentSalary), 0),
  };

  const scatterData = empList.map((e) => ({
    x: e.attendancePct,
    y: e.performanceScore,
    name: e.name,
    dept: e.department,
    increment: e.recommendedIncrement,
  }));
  ...
  function updateEmpStatus(id: string, status: "Approved" | "Rejected") {
    setEmpList((prev) =>
      prev.map((e) =>
        e.id === id
          ? { ...e, status, workflowStep: status === "Approved" ? 4 : 2 }
          : e,
      ),
    );
    setSelectedEmp((prev) =>
      prev?.id === id
        ? { ...prev, status, workflowStep: status === "Approved" ? 4 : 2 }
        : prev,
    );
  }
  ...
  function handleExport() {
    ...
    a.href = url;
    a.download = `increment_appraisal_${selectedYear.replace("–", "-")}.csv`;
    a.click();
  ...
  function confirmApplyPayroll() {
    ...
    setToast({
      msg: `Payroll revised for ${applyTarget.name}. Effective May 1, 2026.`,
      type: "success",
    });
    navigate("/payroll");
  }
    ...
    <div className="w-full px-4 md:px-8 py-6 pb-10">
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        >
          <div>
            ...
            >
              Review yearly attendance, performance score, and salary revision
              recommendations before increment approval.
            </p>
          </div>
      ...
      >
        ...
        >
          ...
          <span style={{ fontSize: "12px", fontWeight: 600, color: "#059669" }}>
            {kpiData.eligible} employees eligible for increment this year
          </span>
        </div>
        <div
          style={{
        ...
        >
          ...
          <span style={{ fontSize: "12px", fontWeight: 600, color: "#D97706" }}>
            {empList.filter((e) => e.attendancePct < 85).length} employees need
            manual review (attendance &lt; 85%)
          </span>
        </div>
        <div
          style={{
        ...
        >
          ...
          <span style={{ fontSize: "12px", fontWeight: 600, color: "#0D9488" }}>
            ...
            {Math.round(
              empList.reduce(
                (s, e) => s + (e.revisedSalary - e.currentSalary),
                0,
              ) / 100000,
            ).toFixed(1)}
            L this cycle
      ...
      >
        {[
          ...
          },
          {
            ...
            sub: "increment approved",
            link: "/payroll",
          },
          {
            label: "Avg. Attendance",
            value: `${kpiData.avgAtt}%`,
            icon: CalendarCheck,
            color: "#14B8A6",
          ...
          },
          {
            ...
            value: `₹${Math.round(kpiData.payrollImpact / 1000)}K`,
            icon: DollarSign,
            color: "#059669",
      ...
      >
        ...
        <div ref={bandDRef} style={{ position: "relative" }}>
          <DropBtn
            label={`Band: ${bandFilter}`}
            open={showBandDrop}
            onClick={() => setShowBandDrop((v) => !v)}
          />
          <DropMenu
            items={BANDS}
            selected={bandFilter}
      ...
      >
        ...
        >
          <div>
            ...
            >
              {filtered.length} employees · Click row to view details · Sort
              columns to reorder
            </p>
          </div>
        ...
        <div
          style={{
            ...
            borderBottom: "1px solid var(--border)",
            gridTemplateColumns:
              "2.2fr 1fr 0.9fr 0.7fr 0.7fr 0.9fr 0.8fr 0.8fr 0.9fr 1.1fr 1.1fr 0.9fr 1fr",
          }}
        >
          {[
            ...
            return (
              ...
              >
                ...
                <span
                  style={{
                    fontSize: "12px",
                    color: emp.leaveDays > 10 ? "#DC2626" : "var(--foreground)",
                  }}
                >
                  {emp.leaveDays}d
                </span>
                ...
                <span
                  style={{
                    fontSize: "13px",
                    fontWeight: 800,
                    color: emp.recommendedIncrement > 0 ? "#059669" : "#DC2626",
                  }}
                >
                  {emp.recommendedIncrement > 0
                    ? `+${emp.recommendedIncrement}%`
                ...
                {/* Current Salary */}
                <span style={{ fontSize: "12px", color: "var(--foreground)" }}>
                  ₹{(emp.currentSalary / 1000).toFixed(0)}K
                </span>
                {/* Revised Salary */}
                <span
                  style={{
                ...
                >
                  ₹{(emp.revisedSalary / 1000).toFixed(0)}K
                </span>
                {/* Status */}
                <StatusBadge status={emp.status} />
                {/* Action */}
                ...
                >
                  {emp.status === "Pending" && (
                    <>
                      <button
                        onClick={() => {
                          updateEmpStatus(emp.id, "Approved");
                          setToast({
                            msg: `${emp.name}'s increment approved.`,
                            type: "success",
                          });
                        }}
                        title="Approve"
                      ...
                      <button
                        onClick={() => {
                          updateEmpStatus(emp.id, "Rejected");
                          setToast({
                            msg: `${emp.name}'s increment rejected.`,
                            type: "warning",
                          });
                        }}
                        title="Reject"
        ...
        >
          <span style={{ fontSize: "12px", color: "var(--muted-foreground)" }}>
            Showing {Math.min(filtered.length, page * PAGE_SIZE + 1)}–
            {Math.min(filtered.length, (page + 1) * PAGE_SIZE)} of{" "}
            {filtered.length}
          </span>
      ...
      >
        ...
        >
          <div style={{ marginBottom: "16px" }}>
            ...
            >
              Correlation scatter • each dot = one employee
            </p>
          </div>
        ...
        >
          <div style={{ marginBottom: "16px" }}>
            ...
            >
              Average recommended increment by department
            </p>
          </div>
          <ResponsiveContainer width="100%" height={220}>
      ...
      >
        ...
        >
          ...
          >
            Increment Recommendation Logic
          </h3>
        </div>
        <div
          style={{
        ...
        >
          {[
            ...
            },
            {
              ...
              msg: "Eligible for 8% with review",
            },
            {
              score: "< 78",
              band: "Poor / Low Att.",
            ...
            >
              ...
              >
                ...
                <span
                  style={{ fontSize: "16px", fontWeight: 900, color: r.color }}
                >
                  {r.increment}
                </span>
              </div>
        ...
        >
          ...
          <span style={{ fontSize: "12px", color: "var(--muted-foreground)" }}>
            Final score = <strong>Attendance × 40%</strong> +{" "}
            <strong>Performance × 60%</strong> · Attendance below 85%
            automatically flags for manual review.
          </span>
        </div>
      </div>
      ...
      {selectedEmp && (
        <DetailDrawer
          ...
          onApprove={() => {
            updateEmpStatus(selectedEmp.id, "Approved");
            setToast({
              msg: `${selectedEmp.name}'s increment approved.`,
              type: "success",
            });
            setSelectedEmp(null);
          }}
          onReject={() => {
            updateEmpStatus(selectedEmp.id, "Rejected");
            setToast({
              msg: `${selectedEmp.name}'s increment rejected.`,
              type: "warning",
            });
            setSelectedEmp(null);
          }}
          onSendReview={() => {
            setToast({
              msg: `${selectedEmp.name}'s review sent to HR.`,
              type: "info",
            });
            setSelectedEmp(null);
          }}
          onApplyPayroll={() => {
            handleApplyPayroll(selectedEmp);
            setSelectedEmp(null);
          }}
        />
      )}
      ...
      {showApplyModal && applyTarget && (
        ...
        >
          ...
          >
            ...
            >
              Confirm salary revision for{" "}
              <strong style={{ color: "var(--foreground)" }}>
                {applyTarget.name}
              </strong>
              . This will update payroll effective May 1, 2026.
            </p>
            <div
              style={{
                backgroundColor: "var(--secondary)",
            ...
            >
              ...
              >
                ...
                >
                  ₹{applyTarget.currentSalary.toLocaleString()}
                </span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span
                ...
                >
                  ₹{applyTarget.revisedSalary.toLocaleString()}
                </span>
              </div>
            </div>
