...
const loadNotifications = (): NotificationItem[] => {
  const local = localStorage.getItem("viyan_notifications:v1");
  if (local) {
    try {
...
const loadAnnouncements = (): AnnouncementItem[] => {
  const local = localStorage.getItem("viyan_announcements:v1");
  if (local) {
    try {
      return JSON.parse(local);
    } catch (e) {
...
export function Notifications() {
  ...
  const handlePostAnnouncement = () => {
    ...
    localStorage.setItem("viyan_announcements:v1", JSON.stringify(updated));
  ...
  const handleDismissNotification = (id: number, e: React.MouseEvent) => {
    ...
    localStorage.setItem("viyan_notifications:v1", JSON.stringify(updated));
...
const loadFinanceNotifications = (): FinanceNotificationItem[] => {
  const local = localStorage.getItem("viyan_finance_notifications:v1");
  if (local) {
    try {
      return JSON.parse(local);
    } catch (e) {
  ...
  localStorage.setItem(
    "viyan_finance_notifications:v1",
