...
const MOCK_CATALOG = [
  { id: "c1", title: "AWS Cloud Architect", provider: "Amazon Web Services", duration: "12 hours", rating: 4.8, category: "Engineering", color: "from-emerald-500 to-emerald-700" },
  { id: "c2", title: "React Performance Masterclass", provider: "Frontend Masters", duration: "8 hours", rating: 4.9, category: "Frontend", color: "from-purple-500 to-purple-700" },
  { id: "c3", title: "Secure Coding Practices", provider: "NexusHR Infosec", duration: "3 hours", rating: 4.5, category: "Security", color: "from-rose-500 to-rose-700" },
  { id: "c4", title: "Engineering Leadership", provider: "Coursera", duration: "16 hours", rating: 4.7, category: "Leadership", color: "from-blue-500 to-blue-700" },
  { id: "c5", title: "Advanced Git & CI/CD", provider: "Pluralsight", duration: "6 hours", rating: 4.6, category: "DevOps", color: "from-teal-500 to-teal-700" },
  { id: "c6", title: "Data Privacy & GDPR", provider: "Internal Legal", duration: "2 hours", rating: 4.2, category: "Compliance", color: "from-amber-500 to-amber-700" },
];

const MOCK_ASSIGNMENTS = [
  { id: "a1", emp: "Arjun Mehta", course: "Advanced Git & CI/CD", assignedOn: "Apr 01, 2026", dueDate: "Apr 30, 2026", progress: 60, status: "In Progress" },
  { id: "a2", emp: "Dev Patel", course: "Secure Coding Practices", assignedOn: "Mar 15, 2026", dueDate: "Mar 31, 2026", progress: 20, status: "Overdue" },
  { id: "a3", emp: "Leo Martinez", course: "Data Privacy & GDPR", assignedOn: "Apr 10, 2026", dueDate: "Apr 25, 2026", progress: 0, status: "Not Started" },
  { id: "a4", emp: "Sneha Rao", course: "AWS Cloud Architect", assignedOn: "Apr 05, 2026", dueDate: "May 15, 2026", progress: 100, status: "Completed" },
];

export function ManagerTeamTraining() {
  const [activeTab, setActiveTab] = useState<"My Team" | "Training Catalog" | "Assignments">("My Team");
  ...
  const getStatusColor = (status: string) => {
    if (status.includes("overdue") || status === "Overdue") return "text-red-600 bg-red-50 border-red-100";
    if (status.includes("complete") || status === "Completed") return "text-emerald-600 bg-emerald-50 border-emerald-100";
    if (status === "In Progress") return "text-teal-600 bg-teal-50 border-teal-100";
  ...
  return (
    <div className="w-full px-4 md:px-8 py-6 pb-10">
      ...
      <div className="flex items-center gap-1 border-b border-border mb-6 overflow-x-auto no-scrollbar">
        {(["My Team", "Training Catalog", "Assignments"] as const).map((tab) => (
          <button
            key={tab}
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-secondary/50 text-muted-foreground text-xs font-bold uppercase tracking-wider">
                <tr>
                  ...
                  <th className="px-6 py-4 border-b border-border">Completed</th>
                  <th className="px-6 py-4 border-b border-border">Mandatory Status</th>
                  <th className="px-6 py-4 border-b border-border">Certifications</th>
                    ...
                    <td className="px-6 py-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] font-semibold text-muted-foreground">{row.designation}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-2 font-bold text-foreground">{row.enrolledCourses}</td>
                    <td className="px-6 py-2 text-muted-foreground font-semibold">{row.completed} done</td>
                    <td className="px-6 py-2">
                      <span className={`px-2 py-1 rounded-md text-[11px] font-bold border flex items-center gap-1 w-max ${getStatusColor(row.mandatoryStatus)}`}>
                        {row.mandatoryStatus.includes("overdue") ? <AlertTriangle size={12}/> : <CheckCircle2 size={12}/>}
                        {row.mandatoryStatus}
                      </span>
                    </td>
                    <td className="px-6 py-2 font-bold text-foreground flex items-center gap-1.5 mt-2">
                      <Award size={16} className="text-purple-500" /> {row.certs} certs
                    </td>
          ...
          {MOCK_CATALOG.map((course) => (
            <div key={course.id} className="bg-card border border-border rounded-2xl overflow-hidden hover:-translate-y-[2px] hover:border-[#00B87C] hover:shadow-[0_0_15px_rgba(0,184,124,0.3)] transition-shadow flex flex-col">
              ...
              <div className="p-5 flex flex-col flex-1">
                <h3 className="text-base font-bold text-foreground mb-1 line-clamp-1">{course.title}</h3>
                <p className="text-xs font-semibold text-muted-foreground mb-4">{course.provider}</p>
                
                <div className="flex items-center gap-4 mt-auto mb-5 text-xs font-bold text-foreground">
                  <div className="flex items-center gap-1.5"><Clock size={14} className="text-muted-foreground" /> {course.duration}</div>
                  <div className="flex items-center gap-1.5"><Star size={14} className="text-amber-400" fill="currentColor"/> {course.rating}</div>
                </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-secondary/50 text-muted-foreground text-xs font-bold uppercase tracking-wider">
                <tr>
                  ...
                  <th className="px-6 py-4 border-b border-border">Assigned On</th>
              ...
              <tbody className="divide-y divide-border">
                {MOCK_ASSIGNMENTS.map((row) => (
                  <tr key={row.id} className="hover:bg-[#00B87C]/[0.08] transition-colors h-[64px]">
                    ...
                    <td className="px-6 py-2">
                      <span className={`px-2 py-1 rounded-md text-[11px] font-bold border ${getStatusColor(row.status)}`}>
                        {row.status}
                      </span>
                    </td>
                    <td className="px-6 py-2 text-right">
        ...
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-[480px] bg-card rounded-2xl shadow-2xl border border-border flex flex-col animate-in zoom-in-95">
            ...
            <div className="p-6 space-y-6">
              ...
              {/* Course Selection */}
              <div>
                <label className="block text-xs font-bold text-foreground mb-1.5">Select Course</label>
              ...
              {/* Assign To */}
              <div>
                <label className="block text-xs font-bold text-foreground mb-1.5">Assign To</label>
                <div className="flex flex-wrap gap-2">
                  <button 
                    onClick={() => toggleAssignee("all")}
                    className={`px-3 py-1.5 text-[11px] font-bold rounded-lg border transition-colors ${
                      selectedAssignees.length === MOCK_MY_TEAM.length ? "bg-primary text-white border-primary" : "bg-secondary text-muted-foreground border-border hover:bg-neutral-200 dark:hover:bg-zinc-800"
                    }`}
                  >
                    All Team
                  </button>
                  {MOCK_MY_TEAM.map(emp => (
                    <button 
                      ...
                      className={`px-3 py-1.5 text-[11px] font-bold rounded-lg border transition-colors ${
                        selectedAssignees.includes(emp.id) ? "bg-primary text-white border-primary" : "bg-background text-foreground border-border hover:bg-secondary"
                      }`}
                    >
                      {emp.name}
                    </button>
              ...
              {/* Note */}
              <div>
                <label className="block text-xs font-bold text-foreground mb-1.5">Note to Team (Optional)</label>
                <textarea 
