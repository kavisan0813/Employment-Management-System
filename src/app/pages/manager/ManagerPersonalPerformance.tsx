    ...
    <div className="w-full px-4 md:px-8 py-6 space-y-6 animate-in fade-in duration-500 min-h-screen bg-[#F0FDF4]/30 dark:bg-transparent">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-emerald-500/10 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-foreground tracking-tight mb-1">My Performance</h1>
            <p className="text-[13px] font-bold text-muted-foreground uppercase tracking-widest">Track your professional development, goals and appraisal status</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20 flex items-center gap-2">
            <Clock size={14} className="text-amber-600" />
            <span className="text-[11px] font-black text-amber-600 uppercase tracking-widest">Self-review due: April 25</span>
          </div>
          <button 
            onClick={() => setActiveTab("Self Review")}
            className="px-5 py-2.5 rounded-xl bg-[#00B87C] text-white font-black text-[12px] uppercase tracking-widest hover:opacity-90 transition-all shadow-lg shadow-[#00B87C]/20"
          >
            Start Self-Review
          </button>
        </div>
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="px-2 py-0.5 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-[10px] font-black text-[#00B87C] uppercase tracking-widest">5 goals</span>
                </div>
                <button 
                  onClick={() => showToast("Info", "info", "Goal requests are sent to HR.")}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl border border-[#00B87C] text-[#00B87C] font-black text-[11px] uppercase tracking-widest hover:bg-[#00B87C]/5 transition-all"
                >
            ...
            >
              ...
              <div className="space-y-8">
                ...
                <button 
                  ...
                  className="w-full py-4 rounded-2xl bg-[#00B87C] text-white font-black text-[14px] uppercase tracking-[2px] hover:opacity-90 transition-all shadow-xl shadow-emerald-500/20"
                >
                  Submit Self Review
                </button>
              </div>
    ...
    <div className="flex items-center justify-between p-6 h-[72px] hover:bg-[#F0FDF4] dark:hover:bg-emerald-500/5 transition-all group border-b border-border last:border-0 cursor-pointer">
      <div className="flex items-center gap-4 flex-1">
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-3">
            ...
            <span className="px-2 py-0.5 rounded-md bg-secondary text-[9px] font-black text-muted-foreground uppercase tracking-widest">{category}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-8 w-[300px]">
        ...
        <div className="flex items-center gap-3 min-w-[120px] justify-end">
          <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black border uppercase tracking-widest ${statusColor}`}>
...
function RatingSection({ label }: { label: string }) {
  ...
  return (
    <div className="space-y-4 bg-secondary/35 border border-border p-6 rounded-[24px]">
...
function HistoryRow({ period, rating, band, reviewer, date }: { period: string, rating: string, band: string, reviewer: string, date: string }) {
  return (
    <tr className="group hover:bg-[#F0FDF4]/30 dark:hover:bg-emerald-500/5 transition-all cursor-pointer">
      <td className="px-8 py-5 text-[14px] font-black text-foreground">{period}</td>
      ...
      <td className="px-8 py-5">
        <span className="px-3 py-1 rounded-lg bg-purple-500/10 text-purple-600 text-xs font-black uppercase">{band}</span>
      </td>
