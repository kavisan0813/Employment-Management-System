...
const ONBOARDING_PHASES = [
  { label: "Pre-Joining", status: "Done", tasks: "6/6", doneDate: "Done Apr 5", tasksTotal: 6, tasksDone: 6 },
  { label: "Day 1", status: "In Progress", tasks: "2/7", doneDate: "In progress — 2 tasks overdue", tasksTotal: 7, tasksDone: 2 },
  { label: "Week 1", status: "Pending", tasks: "0/8", doneDate: "Starts Apr 9", tasksTotal: 8, tasksDone: 0 },
  { label: "Month 1", status: "Pending", tasks: "0/12", doneDate: "Starts Apr 15", tasksTotal: 12, tasksDone: 0 },
  { label: "Complete", status: "Pending", tasks: "0/0", doneDate: "—", tasksTotal: 0, tasksDone: 0 },
];
...
const TASKS_DATA: Task[] = [
  { id: 1, title: "Conduct Welcome 1:1 Meeting", desc: "Schedule and conduct first 1:1 with Priya Sharma", phase: "Day 1", due: "April 8, 2026", dueLabel: "Today", urgency: "amber", estTime: "30 mins", icon: User, iconBg: "#DCFCE7", iconColor: "#00B87C", actionLabel: "Schedule Meeting", actionType: "schedule" },
  { id: 2, title: "Introduce Priya to the Team", desc: "Arrange team introductions and walk through team structure", phase: "Day 1", due: "April 10, 2026", dueLabel: "Apr 10", urgency: "amber", icon: Users, iconBg: "#FEF3C7", iconColor: "#D97706", actionType: "intro" },
  { id: 3, title: "Set 30-Day Goals with Priya", desc: "Define 30-day objectives, success metrics, and growth milestones", phase: "Month 1", due: "April 22, 2026", dueLabel: "Apr 22", urgency: "gray", icon: Target, iconBg: "#E0F2FE", iconColor: "#0284C7", actionType: "goals" },
  { id: 4, title: "Review Knowledge Transfer Plan", desc: "Verified knowledge transfer documentation and handover plan", phase: "Pre-Joining", due: "April 5, 2026", dueLabel: "Completed", urgency: "green", icon: CheckCircle, iconBg: "#DCFCE7", iconColor: "#00B87C" },
];

const COMPLETED_TASKS = [
  { id: 5, title: "Send Offer Letter Confirmation", desc: "Verified candidate acceptance and sent offer package", phase: "Pre-Joining", doneDate: "Apr 1" },
  { id: 6, title: "Set Up IT Workstation", desc: "Configured laptop and software access", phase: "Pre-Joining", doneDate: "Apr 3" },
  { id: 7, title: "Prepare Onboarding Kit", desc: "Welcome kit and document package assembled", phase: "Pre-Joining", doneDate: "Apr 4" },
];

const TEAM_MEMBERS = [
  { name: "Arjun Mehta", initials: "AM", designation: "Senior Frontend Developer" },
...
export function ManagerTeamOnboarding() {
  ...
  const [scheduleTitle, setScheduleTitle] = useState("Welcome 1:1 — Priya Sharma");
  const [scheduleDate, setScheduleDate] = useState("");
  const [scheduleTime, setScheduleTime] = useState("");
  const [scheduleDuration, setScheduleDuration] = useState("30 mins");
  const [scheduleLocation, setScheduleLocation] = useState("Conference Room A / Zoom link");
  const [scheduleNotes, setScheduleNotes] = useState("Agenda: Introduction, Role overview, Q&A");
  ...
  const handleScheduleMeeting = () => {
    ...
    showToast("Meeting Scheduled", "success", `1:1 with Priya scheduled for ${scheduleDate} at ${scheduleTime}`);
  };
  ...
  const handleAssignBuddy = () => {
    ...
    showToast("Buddy Assigned", "success", `${selectedBuddy} assigned as Priya's buddy`);
  };
    ...
    <div className="w-full px-4 md:px-8 py-6 pb-20 flex flex-col gap-6 animate-in fade-in duration-700 min-h-screen bg-[#F0FDF4]/30 dark:bg-transparent relative">
      ...
      <div className="flex flex-wrap items-center gap-4 bg-card px-5 py-3.5 rounded-2xl border border-border shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-bold text-foreground"><span className="text-[#00B87C]">1</span> new team member onboarding — Priya Sharma</span>
        </div>
        <div className="w-px h-4 bg-border" />
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-amber-500" />
          <span className="text-[12px] font-bold text-muted-foreground"><span className="text-amber-500">2</span> tasks assigned to you — due this week</span>
        </div>
        <div className="w-px h-4 bg-border" />
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-cyan-500" />
          <span className="text-[12px] font-bold text-muted-foreground">Priya joining: April 8, 2026 <span className="text-cyan-500">(Today!)</span></span>
        </div>
      </div>
            ...
            <div className="rounded-2xl overflow-hidden bg-card border border-border">
              ...
              <div className="px-6 pb-6 relative">
                ...
                <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-[12px] font-bold text-muted-foreground mb-4">
                  <span className="flex items-center gap-1.5"><Briefcase size={13} className="text-[#00B87C]" /> {NEW_JOINER.department}</span>
                  <span className="flex items-center gap-1.5"><MapPin size={13} className="text-[#00B87C]" /> {NEW_JOINER.location}</span>
                  <span className="flex items-center gap-1.5"><Calendar size={13} className="text-[#00B87C]" /> Joining Today: {NEW_JOINER.joiningDate}</span>
                  <span className="text-muted-foreground/60">Reports to: <span className="text-foreground">{NEW_JOINER.reportsTo}</span></span>
                </div>
            ...
            {!buddyAssigned ? (
              <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[13px] font-bold text-foreground">No buddy assigned yet</p>
                    <p className="text-[11px] font-medium text-muted-foreground">Assign a team member as Priya's buddy</p>
                  </div>
                </div>
                <button
        ...
        <div className="xl:w-[45%] space-y-6">
          ...
          <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      >
                        <task.icon size={14} style={{ color: task.iconColor }} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <h4 className={`text-[14px] ${isDone ? "line-through text-muted-foreground" : "font-bold text-foreground"}`}>{task.title}</h4>
                          {isDone && <CheckCircle size={14} className="text-[#00B87C]" />}
                        </div>
            ...
            <div className="mt-4 pt-3 border-t border-border">
              ...
              >
                {showCompletedTasks ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                  ...
                  >
                    <div className="space-y-2 mt-3">
                      {COMPLETED_TASKS.map((task) => (
                        <div key={task.id} className="flex items-center gap-3 p-3 rounded-xl bg-emerald-500/5 border border-[#00B87C]/10">
          ...
          <div className="bg-card rounded-2xl p-5 border border-border shadow-sm">
            ...
            <div className="space-y-2.5">
              {ONBOARDING_PHASES.filter(p => p.label !== "Complete").map((phase, i) => (
                <div key={i} id={`mgr-phase-${i}`} className="flex items-center justify-between py-2.5 px-3 rounded-xl bg-secondary/30 border border-border/50">
                  <div className="flex items-center gap-3">
                    <div
              ...
              <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <label className="flex items-center gap-2.5 cursor-pointer">
                    ...
                    >
                      <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-all ${notifyEmail ? "left-4" : "left-0.5"}`} />
                    </div>
                    <span className="text-[12px] font-bold text-foreground">Notify Priya via email</span>
                  </label>
                </div>
                ...
                {ONBOARDING_PHASES.filter(p => p.label !== "Complete").map((phase, i) => (
                  <div key={i} className="rounded-2xl border border-border overflow-hidden">
                    <div className={`px-4 py-3 border-b border-border flex items-center justify-between ${
                      phase.status === "Done" ? "bg-emerald-500/5" : phase.status === "In Progress" ? "bg-cyan-500/5" : "bg-secondary/30"
                    }`}>
                      <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[11px] font-bold ${
                        phase.status === "Done" ? "text-[#00B87C]" : phase.status === "In Progress" ? "text-amber-500" : "text-muted-foreground/60"
                      }`}>{phase.tasks} tasks</span>
                    </div>
                    <div className="p-4 space-y-2">
                      {Array.from({ length: phase.tasksTotal }).map((_, j) => (
                        <div key={j} className="flex items-center gap-2.5 py-1.5">
                          <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                            ...
                            {j < phase.tasksDone && <Check size={9} className="text-white" strokeWidth={3} />}
                          </div>
                          <span className={`text-[12px] font-medium ${j < phase.tasksDone ? "text-foreground" : "text-muted-foreground"}`}>
