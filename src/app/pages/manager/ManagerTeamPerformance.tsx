...
const COMPETENCIES = [
  { id: "c1", name: "Technical Skills", desc: "Depth of technical knowledge and application.", self: "4.0" },
  { id: "c2", name: "Problem Solving", desc: "Ability to analyze and resolve complex issues.", self: "4.0" },
  { id: "c3", name: "Communication", desc: "Clarity in written and verbal exchanges.", self: "4.5" },
  { id: "c4", name: "Teamwork & Collaboration", desc: "Working effectively within and across teams.", self: "4.0" },
  { id: "c5", name: "Delivery & Ownership", desc: "Taking responsibility for project outcomes.", self: "3.5" },
  { id: "c6", name: "Leadership Potential", desc: "Mentoring others and taking initiative.", self: "4.0" },
];
...
export function ManagerTeamPerformance() {
  ...
  const [reviewingEmp, setReviewingEmp] = useState<typeof MOCK_TEAM_PERFORMANCE[0] | null>(null);
  ...
  const getStatusChip = (status: string) => {
    if (status === "Completed") return <span className="flex items-center gap-1 text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100"><CheckCircle2 size={12}/> Completed</span>;
    if (status === "In Progress") return <span className="flex items-center gap-1 text-[11px] font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-md border border-amber-100"><Clock size={12}/> In Progress</span>;
    if (status === "Not Started") return <span className="flex items-center gap-1 text-[11px] font-bold text-rose-600 bg-rose-50 px-2 py-1 rounded-md border border-rose-100"><AlertTriangle size={12}/> Not Started</span>;
    return null;
  };

  const getRatingColor = (rating: number, isSelected: boolean) => {
    if (!isSelected) return "bg-[#F3F4F6] text-muted-foreground border-transparent hover:bg-neutral-200 dark:bg-zinc-800 dark:hover:bg-zinc-700";
    if (rating === 1) return "bg-rose-500 text-white border-rose-600 shadow-sm";
    if (rating === 2) return "bg-amber-500 text-white border-amber-600 shadow-sm";
    if (rating === 3) return "bg-yellow-500 text-white border-yellow-600 shadow-sm";
    if (rating === 4) return "bg-teal-500 text-white border-teal-600 shadow-sm";
    if (rating === 5) return "bg-emerald-500 text-white border-emerald-600 shadow-sm";
    return "";
  };
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-secondary/50 text-muted-foreground text-xs font-bold uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4 border-b border-border">Manager Rating</th>
                <th className="px-6 py-4 border-b border-border">Goals Met</th>
                <th className="px-6 py-4 border-b border-border">Final Score</th>
            ...
            <tbody className="divide-y divide-border">
              {MOCK_TEAM_PERFORMANCE.map((row) => (
                <tr key={row.id} className="hover:bg-[#00B87C]/[0.08] transition-colors h-[64px]">
                  ...
                  <td className="px-6 py-2">
                    <span className="flex items-center gap-1 font-bold text-foreground"><Star size={14} className="text-amber-400" fill="#fbbf24" /> {row.selfRating}</span>
                  </td>
                  <td className="px-6 py-2">
                    {row.managerRating.includes("Pending") ? (
                      <span className="text-xs font-semibold text-muted-foreground italic">{row.managerRating}</span>
                    ) : (
                      <span className="flex items-center gap-1 font-bold text-foreground"><Star size={14} className="text-emerald-500" fill="#10b981" /> {row.managerRating}</span>
                    )}
                  </td>
                  <td className="px-6 py-2">
                    <div className="w-32">
                      <div className="flex items-center justify-between text-xs font-bold mb-1">
                        <span>{row.goalsMet}/{row.goalsTotal}</span>
                      </div>
                      <div className="w-full h-1.5 bg-secondary rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full ${row.goalsMet >= 8 ? 'bg-emerald-500' : row.goalsMet >= 7 ? 'bg-amber-500' : 'bg-red-500'}`} 
                          style={{ width: `${(row.goalsMet / row.goalsTotal) * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  </td>
          ...
          <div className="w-full max-w-[560px] max-h-[90vh] bg-card rounded-2xl shadow-2xl border border-border flex flex-col animate-in zoom-in-95">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div className="flex items-center gap-4 bg-secondary/50 p-4 rounded-xl border border-border/50">
                <img src={reviewingEmp.avatar} className="w-12 h-12 rounded-full border-2 border-background shadow-sm" />
                <div>
                  <h4 className="text-base font-bold text-foreground">{reviewingEmp.name}</h4>
                  ...
                  {COMPETENCIES.map((comp) => (
                    <div key={comp.id} className="bg-background border border-border rounded-xl p-4">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <div className="flex items-center gap-1 bg-secondary p-1 rounded-lg">
                            {[1, 2, 3, 4, 5].map((rating) => {
                              const isSelected = compRatings[comp.id] === rating;
                              return (
                                <button
                                  key={rating}
                      ...
                      <div>
                        ...
                        >
                          <MessageSquare size={12} /> {expandedComments[comp.id] ? "Hide Comment" : "Add Comment"}
                        </button>
                        {expandedComments[comp.id] && (
                          <textarea
                            ...
                            onChange={(e) => setCompComments(prev => ({ ...prev, [comp.id]: e.target.value }))}
                          />
                        )}
                      </div>
                    </div>
                ...
                <div className="space-y-3">
                  {GOALS.map((goal) => (
                    <div key={goal.id} className="bg-background border border-border rounded-xl p-4">
                      <p className="text-sm font-bold text-foreground mb-3">{goal.text}</p>
                      <div className="flex flex-wrap gap-2 mb-3">
                        {["Met", "Partially Met", "Not Met"].map((status) => (
                          <button
                            key={status}
                            onClick={() => setGoalStatus(prev => ({ ...prev, [goal.id]: status }))}
                      ...
                      <textarea
                        ...
                        onChange={(e) => setGoalComments(prev => ({ ...prev, [goal.id]: e.target.value }))}
                      />
                    </div>
                  ))}
                </div>
              ...
              <section className="bg-secondary/50 rounded-xl p-5 border border-border">
                ...
                <div className="flex items-center justify-between mb-4 pt-4 border-t border-border">
                  <label className="text-sm font-bold text-foreground">Override Final Rating</label>
                  <button 
                  ...
                  >
                    <div className={`w-4 h-4 rounded-full bg-white absolute top-0.5 transition-all shadow-sm ${overrideRating ? "left-5" : "left-1"}`}></div>
                  </button>
                </div>
              ...
              <section>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-foreground mb-1.5">Key Strengths</label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-xs font-bold text-foreground mb-1.5">Development Areas</label>
                    <textarea
                  ...
                  <div className="bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/50 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-sm font-bold text-indigo-900 dark:text-indigo-200">Promotion Recommendation</label>
