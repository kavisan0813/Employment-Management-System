...
export function ManagerTeamAppraisal() {
  const [members, setMembers] =
    useState<TeamAppraisalMember[]>(INITIAL_TEAM_MEMBERS);
  ...
  const saveRating = () => {
    ...
    const targetName =
      members.find((m) => m.id === ratingTargetId)?.name || "Employee";
  ...
  const filteredMembers = members.filter((m) => {
    ...
    const matchesStatus =
      ...
      (selectedStatus === "Pending Finance" &&
    ...
    const matchesBand =
      ...
      (selectedBand === "Good" &&
        m.performanceScore >= 85 &&
        m.performanceScore < 95) ||
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[13px] text-muted-foreground mt-0.5">
              FY 2025-26 — Increment recommendations for your team
            </p>
          </div>
        </div>
        <button
      ...
      >
        ...
        <div className="flex-1">
          <p className="text-[14px] font-bold text-[#B45309] leading-snug">
            Increment Approval & Rating Policy Notice
          </p>
          <p className="text-[12px] text-muted-foreground mt-1 leading-relaxed">
            Increment approval is handled strictly by{" "}
            <strong className="text-foreground">Finance</strong>. As an
            Engineering Manager, you can:
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1.5 mt-2.5">
            <div className="flex items-center gap-2 text-[12px] text-muted-foreground">
              <span className="text-[#10B981] font-bold">✓</span> View increment
              recommendations for your team
            </div>
            <div className="flex items-center gap-2 text-[12px] text-muted-foreground">
              <span className="text-[#EF4444] font-bold">✗</span> Cannot approve
              or modify final increment amounts
            </div>
            <div className="flex items-center gap-2 text-[12px] text-muted-foreground">
              <span className="text-[#10B981] font-bold">✓</span> Provide
              manager rating inputs (used in calculation)
            </div>
            <div className="flex items-center gap-2 text-[12px] text-muted-foreground">
              <span className="text-[#10B981] font-bold">✓</span> Track Finance
              approval status
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-3 px-4 py-3 bg-card border border-border rounded-2xl mb-6 shadow-sm">
        <div className="flex flex-wrap items-center gap-3 flex-1">
          ...
          <div className="relative">
            ...
            <ChevronRight
              size={14}
              className="absolute right-3.5 top-1/2 -translate-y-1/2 rotate-90 pointer-events-none text-muted-foreground"
            />
          </div>
        ...
        <div className="px-5 py-4 border-b border-border flex items-center justify-between bg-muted/5">
          <div>
            <h2 className="text-base font-bold text-foreground">
              Team Increment Appraisal
            </h2>
            <p className="text-[12px] text-muted-foreground mt-0.5">
              12 team members · Click row for details · Manager ratings editable
            </p>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border bg-muted/10">
                ...
                <th className="px-3 py-3.5 text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
                  Leaves
                </th>
                <th className="px-3 py-3.5 text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
                  Late
                ...
                <th className="px-4 py-3.5 text-[11px] font-bold text-muted-foreground uppercase tracking-wider text-center">
                  Mgr Rating
                </th>
                <th className="px-4 py-3.5 text-[11px] font-bold text-muted-foreground uppercase tracking-wider text-center">
                  Increment %
                </th>
                <th className="px-4 py-3.5 text-[11px] font-bold text-muted-foreground uppercase tracking-wider text-right">
                  Current Salary
                </th>
                <th className="px-4 py-3.5 text-[11px] font-bold text-muted-foreground uppercase tracking-wider text-right">
                  Revised Salary
                ...
                <th className="px-5 py-3.5 text-[11px] font-bold text-muted-foreground uppercase tracking-wider text-center">
                  Action
                </th>
              </tr>
            </thead>
              ...
              {filteredMembers.map((m) => {
                ...
                return (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{
                            ...
                            color:
                              m.status === "Approved" ? "#059669" : "#4B5563",
                          }}
                        >
                          {m.initials}
                        </div>
                    ...
                    {/* Department */}
                    <td className="px-4 py-4 text-xs font-bold text-foreground/80">
                      {m.department}
                    </td>
                    ...
                    {/* Leaves */}
                    <td className="px-3 py-4 text-xs font-bold text-foreground/80">
                      {m.leaveDays}d
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        {m.status === "Manual Review" ? (
                          <button
                            onClick={() =>
                              alert(
                                `A justification report is required for ${m.name} due to attendance (${m.attendancePct}%). Complete this form to submit justification to Finance.`,
                              )
                            }
                          ...
                          >
                            <span>
                              {m.status === "Approved" ? "₹ Payroll" : "View"}
                            </span>
            ...
            <>
              ...
              <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[440px] max-w-[90vw] bg-card border border-border rounded-2xl p-6 shadow-2xl z-[3001] animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center gap-3 p-3 bg-muted/20 border border-border rounded-xl mb-5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      ...
                      <span className="text-[#00B87C] font-semibold">
                ...
                <div className="p-3 bg-muted/10 rounded-xl border border-border/50 text-[11px] text-muted-foreground leading-relaxed mb-5">
                  <div className="font-bold text-foreground uppercase tracking-wider mb-1">
                    Appraisal Rating Scale Guide:
                  </div>
                ...
                <div className="mt-4 p-2.5 bg-[#F0FDF4] border border-[#DCFCE7] rounded-xl flex items-center gap-2 text-[11px] text-[#15803D] font-bold">
                  ...
                  <span>
                    Note: Rating change sends notification to Finance for
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                ...
                <div className="bg-muted/10 border border-border p-3 rounded-xl">
                  ...
                  <span className="text-sm font-bold text-foreground mt-1 block">
                    {selectedMember.managerRating.toFixed(1)}★
                  </span>
                </div>
                <div className="bg-muted/10 border border-border p-3 rounded-xl">
                  <span className="text-[11px] font-bold text-muted-foreground block uppercase">
                  ...
                  <span className="text-sm font-bold text-[#10B981] mt-1 block">
                    {typeof selectedMember.recommendedIncrement === "number"
                      ? `+${selectedMember.recommendedIncrement}%`
                      : selectedMember.recommendedIncrement}
                  </span>
                </div>
              </div>
              ...
              <div className="mb-6">
                <h5 className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mb-4">
                  Approval Journey
                </h5>
                <div className="space-y-4 relative pl-5 border-l-2 border-border/60 ml-2">
                  ...
                  <div className="relative">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        Rating {selectedMember.managerRating} submitted by
                        Suresh Iyer
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        Eligibility confirmed and verified
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        {selectedMember.status === "Approved"
                          ? "Approved by Ananya Das"
                          : "Under review (Ananya Das)"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        Pending final processing by Finance
                      </p>
                    </div>
                  </div>
              ...
              <div className="bg-muted/10 border border-border/80 rounded-xl p-4 mb-6">
                <h5 className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mb-3">
                  Salary Breakdown
                </h5>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="font-bold text-foreground">
                        {selectedMember.goalsMet} / {selectedMember.goalsTotal}
                      </span>
                    </div>
                    <div className="w-full bg-muted h-1.5 rounded-full overflow-hidden">
                      <div
                        className="bg-[#00B87C] h-full rounded-full"
                        style={{
                          width: `${(selectedMember.goalsMet / selectedMember.goalsTotal) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="font-bold text-foreground">
                        {selectedMember.kpiScore}%
                      </span>
                    </div>
                    <div className="w-full bg-muted h-1.5 rounded-full overflow-hidden">
                      <div
                        className="bg-[#14B8A6] h-full rounded-full"
                        style={{ width: `${selectedMember.kpiScore}%` }}
                      />
                    </div>
                  </div>
