...
const initialTasksData: Record<string, TaskItem[]> = {
  NH001: [
    ...
    },
    {
      id: "T6",
      category: "Manager",
      task: "Introductory Team Meeting",
      status: "Overdue",
      due: "2026-04-26",
    },
    {
      id: "T7",
      category: "Manager",
      task: "Provide Team Space Access",
      status: "Pending",
      due: "2026-04-29",
    },
  ],
...
function NewHireFormModal({
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      email:
        form.email || `${form.name.toLowerCase().replace(" ", ".")}@nexus.com`,
      buddy: form.buddy,
      ...
      progress:
        ...
        (form.currentStep === "Completed"
          ? 100
          : form.currentStep === "Offer Accepted"
            ? 14
            : 50),
        ...
        <div className="px-8 py-6 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, currentStep: e.target.value })
              }
            >
...
export function Onboarding() {
  ...
  const kpiData = {
    ...
    joiningWeek: hires.filter(
      (h) => h.joiningDate <= "2026-04-30" && h.status !== "Completed",
    ).length,
    ...
    <div className="w-full px-4 md:px-8 pb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        {[
          ...
          },
          {
            ...
            val: kpiData.pendingDocs,
            icon: FileText,
            color: "text-amber-600",
          },
          {
            label: "Joining This Week",
            val: kpiData.joiningWeek,
            icon: Calendar,
            color: "text-indigo-600",
          },
          {
            label: "Completed Onboarding",
            val: kpiData.completed,
            icon: CheckCircle2,
            color: "text-emerald-600",
          },
          {
            label: "IT Setup Pending",
            val: kpiData.itPending,
            icon: Laptop,
            color: "text-rose-500",
          },
        ].map((kpi, idx) => (
          <div
            key={idx}
            className="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-border shadow-sm flex items-center gap-3"
          >
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center bg-neutral-50 dark:bg-zinc-800/50 ${kpi.color}`}
            >
              <kpi.icon size={18} />
            </div>
            <div>
      ...
      <div className="bg-rose-50/60 dark:bg-rose-950/10 border border-rose-100 dark:border-rose-900/30 rounded-2xl p-3 mb-8 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-xs font-bold text-rose-800 dark:text-rose-400">
          ...
          <span>
            System Alert: Suresh G. background verification documents remain
            overdue by 3 days.
          </span>
        </div>
        <button className="text-[10px] font-black uppercase tracking-wider text-rose-800 dark:text-rose-400 hover:underline">
          View Task
        </button>
      </div>
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-full ${hire.avatarColor} text-white font-black text-xs flex items-center justify-center flex-shrink-0 shadow-sm`}
                      >
                        {hire.name
                          .split(" ")
                          .map((n) => n[0])
                  ...
                  <td className="px-4 py-4">
                    <span
                      className={`px-2 py-0.5 text-[9px] font-black rounded-full uppercase tracking-wider border ${getStatusStyle(hire.status)}`}
                    >
                      {hire.status}
                    </span>
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <div className="relative">
                        <button
                          ...
                          onClick={() =>
                            setShowMenu(showMenu === hire.id ? null : hire.id)
                          }
                        >
                          <MoreIcon size={16} />
                        </button>
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-border bg-neutral-50 dark:bg-zinc-800/40 flex-shrink-0">
              <div className="flex items-center gap-4">
                <div
                  className={`w-12 h-12 rounded-full ${selectedHire.avatarColor} text-white flex items-center justify-center font-black text-sm shadow-sm`}
                >
                  {selectedHire.name
                    .split(" ")
                    .map((n) => n[0])
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3 text-center p-3 bg-neutral-50 dark:bg-zinc-800/40 border border-border rounded-2xl">
                ...
                <div>
                  ...
                  <p className="text-xs font-extrabold text-[#00B87C] mt-1">
                    {selectedHire.buddy || "Unassigned"}
                  </p>
                </div>
                <div>
                  <span className="text-[9px] font-black text-muted-foreground uppercase tracking-wider">
                  ...
                  <p className="text-xs font-extrabold text-foreground mt-1">
                    {selectedHire.probationDays} Days
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative flex items-center justify-between">
                  ...
                  <div
                    ...
                    style={{
                      width: `${(getStepIndex(selectedHire.currentStep) / (onboardingSteps.length - 1)) * 100}%`,
                    }}
                  />
                  {onboardingSteps.map((step, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-5 h-5 rounded-full border-2 flex items-center justify-center text-[10px] font-bold transition-all ${
                            isDone
                              ...
                              : isCurrent
                                ? "bg-white dark:bg-zinc-900 border-[#00B87C] text-[#00B87C] ring-4 ring-[#00B87C]/20"
                        ...
                        <span
                          className={`text-[8px] font-black uppercase tracking-wider mt-2 hidden sm:block text-center max-w-[50px] ${
              ...
              <div className="border-t border-border pt-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {(
                    initialDocsData[selectedHire.id] || initialDocsData["NH001"]
                  ).map((doc) => (
                    <div
                      key={doc.id}
                      className="p-3 rounded-xl border border-border bg-neutral-50 dark:bg-zinc-800/40 flex items-center justify-between"
                    >
              ...
              <div className="border-t border-border pt-5">
                ...
                <div className="space-y-2">
                  {(
                    initialTasksData[selectedHire.id] ||
                    initialTasksData["NH001"]
                  ).map((task) => (
                    <div
                      key={task.id}
                      className="flex items-center justify-between p-3 rounded-2xl border border-border bg-white dark:bg-zinc-900"
                    >
                      <div className="flex items-center gap-3">
                        <div
            ...
            <div className="p-4 border-t border-border bg-neutral-50 dark:bg-zinc-800/40 flex items-center justify-end gap-3 flex-shrink-0">
              <button
                onClick={() =>
                  setReminderPrompt(
                    `Reminder successfully dispatched for ${selectedHire.name}.`,
                  )
                }
