...
export function ITClearance() {
  ...
  const pending = exits.filter(
    (exit) =>
      exit.clearance.find((item) => item.dept === "IT")?.status !== "cleared",
  );
  return (
    <div className="w-full max-w-5xl mx-auto px-4 md:px-8 py-8 space-y-6">
      <div className="flex items-center gap-4">
        <div className="w-11 h-11 rounded-xl bg-sky-100 flex items-center justify-center">
          <Laptop className="text-sky-600" />
        </div>
        <div>
          <h1 className="text-2xl font-bold">IT Clearance</h1>
          <p className="text-sm text-muted-foreground">
            Complete asset return and access-revocation sign-off.
          </p>
        </div>
      </div>
      {pending.length === 0 ? (
        <div className="rounded-2xl border p-8 text-center text-muted-foreground">
          No IT clearances are pending.
        </div>
      ) : (
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Confirm laptop/assets returned, email disabled, VPN disabled,
                  and access removed.
                </p>
              </div>
              {canClear && (
                <button
                  onClick={() => publishClearance(exit.name, "IT")}
                  className="px-4 py-2.5 rounded-xl bg-[#00B87C] text-white text-sm font-bold flex items-center gap-2"
                >
                  <ShieldCheck size={16} /> Approve IT Clearance
                </button>
              )}
            </div>
          ))}
