...
const SUB_NAV_ITEMS: SubNavItem[] = [
  { key: "security", icon: Lock, label: "Account & Security", group: "ACCOUNT" },
  { key: "privacy", icon: ShieldCheck, label: "Privacy", group: "ACCOUNT" },
  { key: "notifications", icon: Bell, label: "Notifications", group: "PREFERENCES" },
  { key: "appearance", icon: Moon, label: "Appearance", group: "PREFERENCES" },
  { key: "language", icon: Globe, label: "Language & Region", group: "PREFERENCES" },
  { key: "devices", icon: Laptop, label: "Connected Devices", group: "DEVICES & DATA" },
  { key: "data", icon: Download, label: "My Data & Downloads", group: "DEVICES & DATA" },
  { key: "help", icon: HelpCircle, label: "Help & FAQ", group: "SUPPORT" },
  { key: "contact", icon: Headphones, label: "Contact HR / Support", group: "SUPPORT" },
];
...
function Toggle({ on, onChange, disabled }: { on: boolean; onChange: (v: boolean) => void; disabled?: boolean }) {
  return (
    ...
    >
      <div className={`absolute top-1/2 -translate-y-1/2 w-3.5 h-3.5 rounded-full transition-all ${on ? "right-1 bg-primary" : "left-1 bg-muted-foreground/40"}`} />
    </button>
  );
}
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-700 w-full px-4 md:px-8 py-6 pb-20">
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <div>
          <h1 className="text-[26px] font-black text-foreground leading-none">Settings</h1>
          <p className="text-[14px] font-bold text-muted-foreground mt-1">Manage your account preferences and security</p>
        </div>
      </div>
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-6">
          <div className="bg-card rounded-2xl border border-border shadow-sm p-8">
            ...
            {activeSection === "language" && <LanguageRegion onModal={setActiveModal} />}
            {activeSection === "devices" && <ConnectedDevices onModal={setActiveModal} />}
            {activeSection === "data" && <DataDownloads onModal={setActiveModal} />}
            {activeSection === "help" && <HelpFAQ navigate={navigate} />}
            {activeSection === "contact" && <ContactSupport onModal={setActiveModal} navigate={navigate} />}
          </div>
        </div>
      </div>
    </div>
...
function AccountSecurity({ onModal }: { onModal: (m: string | null) => void }) {
  ...
  const sessions = [
    { id: 1, device: "MacBook Pro · Chrome", location: "Chennai, India", time: "Today 9:02 AM", icon: Laptop, color: "var(--primary)", bg: "var(--secondary)", current: true },
    { id: 2, device: "iPhone 14 Pro · NexusHR App", location: "Chennai, India", time: "Apr 5, 2026", icon: Smartphone, color: "#0EA5E9", bg: "#E0F2FE", current: false },
    { id: 3, device: "Windows PC · Chrome", location: "Mumbai, India", time: "Mar 28, 2026", icon: Laptop, color: "#8B5CF6", bg: "#EDE9FE", current: false },
  ];

  const pwStrength = (pw: string) => {
    if (!pw) return { level: 0, label: "Enter a password", color: "var(--muted-foreground)" };
    let s = 0;
    if (pw.length >= 8) s++;
    if (/[A-Z]/.test(pw)) s++;
    if (/\d/.test(pw)) s++;
    ...
    <div>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8 mb-8">
        ...
        <div className="space-y-5 max-w-lg">
          ...
          <div>
            <PasswordField label="New Password" value={newPw} onChange={setNewPw} show={showPw.new} onToggle={() => setShowPw({ ...showPw, new: !showPw.new })} />
            <div className="flex items-center gap-2 mt-3">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-1.5 flex-1 rounded-full transition-colors" style={{ backgroundColor: i <= strength.level ? strength.color : "var(--border)" }} />
              ))}
              <span className="text-[11px] font-bold ml-1" style={{ color: strength.color }}>{strength.label}</span>
            </div>
          </div>
          <div>
            <PasswordField label="Confirm New Password" value={confirmPw} onChange={setConfirmPw} show={showPw.confirm} onToggle={() => setShowPw({ ...showPw, confirm: !showPw.confirm })} />
            {pwMatch !== null && (
              <p className={`text-[12px] font-bold mt-1.5 ${pwMatch ? "text-primary" : "text-rose-500"}`}>
        ...
        {twoFA && (
          <div className="p-5 rounded-2xl bg-primary/5 border border-primary/10 mb-6">
            <div className="flex items-center gap-2 text-[13px] font-black text-primary mb-1">
              <ShieldCheck size={16} /> 2FA is active — your account is protected
            </div>
            <p className="text-[13px] font-bold text-muted-foreground">● Authenticator App (Google Authenticator)</p>
            <p className="text-[12px] font-bold text-muted-foreground/60">Last used: Apr 5, 2026 at 9:15 AM</p>
          </div>
        )}
        <div className="space-y-5 max-w-lg">
          <ToggleRow label="Enable Two-Factor Authentication" desc="Requires a second verification step on login" on={twoFA} onChange={setTwoFA} />
          <div className="flex items-center justify-between py-2 px-4 rounded-2xl bg-background border border-border">
            <div>
              <p className="text-[14px] font-bold text-foreground">Backup Codes</p>
              <p className="text-[12px] font-bold text-muted-foreground">8 backup codes available</p>
            </div>
            <div className="flex items-center gap-3">
              <button className="px-4 py-2 rounded-xl border border-primary text-primary text-[11px] font-black hover:bg-primary/10 transition-all">View Codes</button>
              <button className="text-[12px] font-bold text-muted-foreground hover:text-foreground transition-all">Regenerate</button>
            </div>
          </div>
          <ToggleRow label="Remember this device for 30 days" desc="Skip 2FA on trusted devices" on={rememberDevice} onChange={setRememberDevice} />
        </div>
      </div>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8">
        ...
        <div className="space-y-1">
          {sessions.map((s) => {
            const Icon = s.icon;
            return (
              <div key={s.id} className="flex items-center gap-4 py-3.5 px-3 rounded-2xl hover:bg-secondary transition-colors border-b border-border/50 last:border-b-0">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: s.bg }}>
                  <Icon size={16} style={{ color: s.color }} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-black text-foreground truncate">{s.device}</p>
                ...
                {s.current ? (
                  ...
                  <button className="text-[12px] font-black text-rose-500 hover:underline shrink-0">Revoke</button>
                )}
              </div>
            );
          })}
...
function PrivacySettings({ onModal }: { onModal: (m: string | null) => void }) {
  const [toggles, setToggles] = useState({
    showPhone: false, showEmail: true, showDob: false, showLinkedin: true,
    showBlood: false, showSchedule: true, showOnline: true, allowPerformanceData: true,
    receiveBirthdayWishes: true, salaryManager: true, salaryFinance: true, loginActivity: true,
  });
  const toggle = (key: keyof typeof toggles) => {
    if (["salaryManager", "salaryFinance", "loginActivity"].includes(key)) return;
    ...
    <div>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8 mb-6">
        ...
        <div className="space-y-1 max-w-lg">
          <ToggleRow label="Show my phone number to colleagues" desc="Team Directory will hide your mobile number" on={toggles.showPhone} onChange={() => toggle("showPhone")} />
          <ToggleRow label="Show my email to colleagues" desc="Work email always visible. Personal email follows this setting" on={toggles.showEmail} onChange={() => toggle("showEmail")} />
          <ToggleRow label="Show my date of birth" desc="Birthday reminders depend on this setting" on={toggles.showDob} onChange={() => toggle("showDob")} />
          <ToggleRow label="Show my LinkedIn profile" on={toggles.showLinkedin} onChange={() => toggle("showLinkedin")} />
          <ToggleRow label="Show my blood group" desc="Only HR and emergency contacts can see this" on={toggles.showBlood} onChange={() => toggle("showBlood")} />
          <ToggleRow label="Allow colleagues to see my work schedule" desc="Team members can see your shift timings" on={toggles.showSchedule} onChange={() => toggle("showSchedule")} />
        </div>
      </div>
      ...
      <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-8 mb-6">
        ...
        <div className="space-y-3 max-w-lg">
          ...
          <p className="text-[12px] font-bold text-amber-500 flex items-center gap-1"><AlertTriangle size={12} /> This setting is controlled by HR policy</p>
          <ToggleRow label="Show my salary details to Finance team" desc="Finance requires this for payroll processing" on={toggles.salaryFinance} onChange={() => {}} disabled />
          <p className="text-[12px] font-bold text-amber-500 flex items-center gap-1"><AlertTriangle size={12} /> Required for payroll — cannot be disabled</p>
        </div>
      </div>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8 mb-6">
        ...
        <div className="space-y-1 max-w-lg">
          ...
          <ToggleRow label="Allow HR to see my login activity" desc="HR can view login times for attendance purposes" on={toggles.loginActivity} onChange={() => {}} disabled />
          <p className="text-[12px] font-bold text-amber-500 flex items-center gap-1"><AlertTriangle size={12} /> Required by company policy</p>
          <ToggleRow label="Allow performance data to be used in team reports" desc="Your anonymized performance contributes to team analytics" on={toggles.allowPerformanceData} onChange={() => toggle("allowPerformanceData")} />
          <ToggleRow label="Receive birthday wishes from colleagues" desc="Colleagues are notified on your birthday" on={toggles.receiveBirthdayWishes} onChange={() => toggle("receiveBirthdayWishes")} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-card rounded-2xl border border-border shadow-sm p-8">
          ...
          <button className="px-5 py-2.5 rounded-xl border border-primary text-primary text-[12px] font-black hover:bg-primary/10 transition-all active:scale-95"
            onClick={() => showToast("Export Requested", "success", "Your data export request has been submitted. You'll receive an email within 48 hours.")}>
            Request Download
          </button>
        </div>
...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
function NotificationSettings({ onModal }: { onModal: (m: string | null) => void }) {
  ...
  const toggleChannel = (cat: keyof typeof channels, ch: "email" | "push" | "sms") => {
    setChannels({ ...channels, [cat]: { ...channels[cat], [ch]: !channels[cat][ch] } });
  };

  const channelRows = [
    { key: "leave" as const, label: "Leave Requests & Approvals", desc: "When your leave is approved or rejected" },
    { key: "payroll" as const, label: "Salary & Payslips", desc: "When salary is credited or payslip is ready" },
    { key: "attendance" as const, label: "Attendance Alerts", desc: "Late marks, regularization approvals" },
    { key: "performance" as const, label: "Performance Reviews", desc: "Review deadlines, feedback received" },
    { key: "expenses" as const, label: "Expense Claims", desc: "When expense is approved, rejected" },
    { key: "schedule" as const, label: "Shift & Schedule", desc: "Shift changes, swap approvals" },
    { key: "announcements" as const, label: "Company Announcements", desc: "Urgent company-wide updates" },
    { key: "training" as const, label: "Goals & Learning", desc: "Goal deadlines, training due dates" },
  ];
  ...
  return (
    <div>
            ...
            <div key={row.key} className="grid grid-cols-[1fr_52px_52px_52px] gap-2 px-5 py-4 border-b last:border-b-0 border-border/50 hover:bg-[#00B87C]/[0.08] transition-colors items-center">
              ...
              {(["email", "push", "sms"] as const).map((ch) => (
                <div key={ch} className="flex justify-center">
                  <Toggle on={channels[row.key][ch]} onChange={() => toggleChannel(row.key, ch)} />
                </div>
              ))}
            </div>
          ))}
        ...
        <div className="space-y-5 max-w-lg">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <p className="text-[14px] font-black text-foreground">Daily Digest Email</p>
              <p className="text-[12px] font-bold text-muted-foreground">Receive a summary email at 9 AM instead of individual emails</p>
            </div>
          ...
          <ToggleRow label="Quiet Hours" desc="No push notifications during off-hours" on={quietHours} onChange={setQuietHours} />
          {quietHours && (
            <div className="flex items-center gap-3 pl-1">
              <span className="text-[12px] font-bold text-muted-foreground">From</span>
              <select value={quietFrom} onChange={(e) => setQuietFrom(e.target.value)}
                className="h-10 bg-background border border-border rounded-xl px-3 text-[12px] font-bold text-foreground focus:outline-none focus:border-primary appearance-none">
                {["8:00 PM", "9:00 PM", "10:00 PM", "11:00 PM", "12:00 AM"].map((t) => (<option key={t} value={t}>{t}</option>))}
              </select>
              <span className="text-[12px] font-bold text-muted-foreground">To</span>
              <select value={quietTo} onChange={(e) => setQuietTo(e.target.value)}
                className="h-10 bg-background border border-border rounded-xl px-3 text-[12px] font-bold text-foreground focus:outline-none focus:border-primary appearance-none">
                {["5:00 AM", "6:00 AM", "7:00 AM", "8:00 AM", "9:00 AM"].map((t) => (<option key={t} value={t}>{t}</option>))}
              </select>
            </div>
          )}
          <ToggleRow label="Weekend Notifications" desc="Only receive critical alerts on weekends" on={weekendNotifs} onChange={setWeekendNotifs} />
        </div>
      </div>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8">
        ...
        <p className="text-[12px] font-bold text-muted-foreground italic mt-3">Critical notifications (salary, security) cannot be unsubscribed</p>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8 mb-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { key: "light" as const, icon: Sun, label: "Light", desc: "Default — clean and bright", preview: "white" },
            { key: "dark" as const, icon: Moon, label: "Dark", desc: "Easy on the eyes at night", preview: "#0F172A" },
            { key: "system" as const, icon: Monitor, label: "System", desc: "Follows your device setting", preview: "linear-gradient(90deg, white 50%, #0F172A 50%)" },
          ].map((t) => {
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="h-14 rounded-xl w-full overflow-hidden border" style={{ borderColor: "var(--border)", background: t.preview }}>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-[14px] font-black text-foreground">{t.label}</p>
                    <p className="text-[11px] font-bold text-muted-foreground">{t.desc}</p>
                  </div>
                </div>
              </div>
            );
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8 mb-6">
        ...
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-[13px] font-black text-primary">Medium ({fontSize}px)</span>
        </div>
        <input type="range" min="12" max="18" step="2" value={fontSize} onChange={(e) => setFontSize(Number(e.target.value))}
          className="w-full h-2 rounded-full appearance-none cursor-pointer accent-primary" style={{ backgroundColor: "var(--border)" }} />
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8">
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <p className="text-[15px] font-black text-foreground">NexusHR Green</p>
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 text-amber-500 text-[11px] font-black mt-1 border border-amber-500/20">
              <AlertTriangle size={12} /> Accent color is set by your organization
            </span>
          </div>
        </div>
      </div>
...
function LanguageRegion({ onModal }: { onModal: (m: string | null) => void }) {
  ...
  const [firstDay, setFirstDay] = useState<"sunday" | "monday" | "saturday">("monday");
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8 mb-6">
        ...
        <div className="max-w-md relative">
          <select value={lang} onChange={(e) => setLang(e.target.value)}
            ...
            {["English", "Hindi", "Tamil", "Telugu", "Kannada", "Other"].map((o) => (<option key={o} value={o}>{o}</option>))}
          </select>
          ...
          <div>
            ...
            <div className="relative">
              <select value={tz} onChange={(e) => setTz(e.target.value)}
                ...
                {["IST (UTC+5:30)", "UTC (UTC+0:00)", "EST (UTC-5:00)", "PST (UTC-8:00)"].map((o) => (<option key={o} value={o}>{o}</option>))}
              </select>
              <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-muted-foreground">
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2.5 4.5L6 8L9.5 4.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
              </div>
          ...
          <div>
            ...
            <div className="relative">
              <select value={currency} onChange={(e) => setCurrency(e.target.value)}
                className="w-full h-12 bg-background border border-border rounded-xl px-4 text-[14px] font-bold text-foreground focus:outline-none focus:border-primary appearance-none">
                {["₹ INR", "$ USD", "£ GBP", "€ EUR"].map((o) => (<option key={o} value={o}>{o}</option>))}
              </select>
              <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-muted-foreground">
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2.5 4.5L6 8L9.5 4.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
              </div>
...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
function ConnectedDevices({ onModal }: { onModal: (m: string | null) => void }) {
  return (
    <div>
      <Breadcrumb active="Connected Devices" />
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-[22px] font-black text-foreground">Connected Devices & Sessions</h2>
        <button className="px-5 py-2.5 rounded-xl bg-rose-500 text-white text-[12px] font-black hover:bg-rose-600 transition-all active:scale-95"
      ...
      <div className="bg-primary/5 border border-primary/20 rounded-2xl p-8 mb-6 relative overflow-hidden">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[16px] font-black text-foreground">MacBook Pro 14</p>
        ...
        <p className="text-[13px] font-bold text-muted-foreground">Chennai, India · Apr 6, 2026, 9:02 AM</p>
        <p className="text-[13px] font-black text-primary mt-1 flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-primary animate-pulse" /> Your current active session
        </p>
      </div>

      <Label>OTHER ACTIVE SESSIONS</Label>
      {[
        { icon: Smartphone, bg: "#E0F2FE", color: "#0EA5E9", name: "iPhone 14 Pro", meta: "NexusHR App · iOS 17", location: "Chennai, India · Apr 5, 2026, 8:45 PM" },
        { icon: Laptop, bg: "#EDE9FE", color: "#8B5CF6", name: "Windows 11 PC", meta: "Chrome 123 · Windows 11", location: "Mumbai, India · Mar 28, 2026, 2:30 PM" },
      ].map((s, i) => {
        const Icon = s.icon;
        return (
          <div key={i} className="bg-card rounded-2xl border border-border shadow-sm p-6 mb-4 flex items-start justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[15px] font-black text-foreground">{s.name}</p>
                <p className="text-[12px] font-bold text-muted-foreground">{s.meta}</p>
                <p className="text-[12px] font-bold text-muted-foreground/60">{s.location}</p>
              </div>
            </div>
            <button className="px-4 py-2 rounded-xl border border-rose-500 text-rose-500 text-[11px] font-black hover:bg-rose-500/10 transition-all shrink-0"
      ...
      <div className="p-6 rounded-2xl border border-amber-500/20 bg-amber-500/5 mb-6">
        ...
        <div className="flex gap-3">
          ...
          <button className="px-5 py-2.5 rounded-xl bg-rose-500 text-white text-[12px] font-black hover:bg-rose-600 transition-all"
            onClick={() => showToast("Account Secured", "success", "All sessions revoked. Password has been reset.")}>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[15px] font-black text-foreground">NexusHR Mobile App</p>
            <p className="text-[13px] font-black text-primary">Connected — Last sync: Today 9:05 AM</p>
          </div>
        </div>
        <button className="px-5 py-2.5 rounded-xl border border-border text-muted-foreground text-[12px] font-black hover:bg-secondary transition-all"
...
function DataDownloads({ onModal }: { onModal: (m: string | null) => void }) {
  const cards = [
    { icon: FileText, bg: "var(--secondary)", color: "var(--primary)", title: "My Payslips", desc: "All salary slips — FY 2022 to 2026", last: "Never", btn: "Download ZIP", primary: true },
    { icon: Calendar, bg: "#E0F2FE", color: "#0EA5E9", title: "My Attendance Records", desc: "Check-in/out history — Current FY", last: "Mar 1, 2026", btn: "Download CSV", primary: false },
    { icon: Clock, bg: "#FEF3C7", color: "#F59E0B", title: "My Leave History", desc: "All leave requests and approvals", last: "Never", btn: "Download CSV", primary: false },
    { icon: Receipt, bg: "#FEF3C7", color: "#F59E0B", title: "My Expense Claims", desc: "All expense submissions — All time", last: "Never", btn: "Download CSV", primary: false },
    { icon: Target, bg: "var(--secondary)", color: "var(--primary)", title: "My Goals & Performance", desc: "Performance reviews, ratings and goals", last: "Never", btn: "Download PDF", primary: false },
    { icon: Folder, bg: "#E0F2FE", color: "#0EA5E9", title: "My Documents", desc: "All uploaded personal documents", last: "Never", btn: "Download ZIP", primary: false },
    { icon: Star, bg: "#FEF3C7", color: "#F59E0B", title: "My Training Records", desc: "Course completions and certifications", last: "Never", btn: "Download PDF", primary: false },
  ];
    ...
    <div>
      ...
      <div className="p-6 rounded-2xl bg-blue-500/5 border border-blue-500/20 flex items-start gap-4 mb-6">
        ...
        <p className="text-[14px] font-bold text-muted-foreground leading-relaxed">
          Your data is securely stored by NexusHR. You can download or request deletion of your personal data at any time.
        </p>
      </div>
          ...
          return (
            <div key={i} className="bg-card rounded-2xl border border-border shadow-sm p-6">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div className="flex-1">
                  <p className="text-[15px] font-black text-foreground">{card.title}</p>
                  <p className="text-[12px] font-bold text-muted-foreground">{card.desc}</p>
                  <p className="text-[11px] font-bold text-muted-foreground/60 mt-1">Last: {card.last}</p>
                </div>
              </div>
              <button
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8 mb-6">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <p className="text-[16px] font-black text-foreground">Download All My Data</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-rose-500/5 border border-rose-500/20 rounded-2xl p-6">
          ...
          <button className="mt-4 px-5 py-2.5 rounded-xl border border-rose-500 text-rose-500 text-[12px] font-black hover:bg-rose-500/10 transition-all"
...
function HelpFAQ({ navigate }: { navigate: (p: string) => void }) {
  ...
  const faqs = [
    { q: "How do I apply for leave?", a: "Go to My Leaves → click 'Apply Leave' button to submit a new request.", link: "/leave", linkLabel: "Apply Now →" },
    { q: "How do I download my payslip?", a: "Go to My Payslips → click the Download button against any payslip entry.", link: "/payslips", linkLabel: "Go to Payslips →" },
    { q: "How do I update my bank account details?", a: "Please contact HR or the Finance team to update your bank details." },
    { q: "Why is my attendance showing incorrect?", a: "You can submit a regularization request from the Attendance page.", link: "/attendance", linkLabel: "Go to Attendance →" },
    { q: "How do I check my leave balance?", a: "Your leave balance is displayed on the My Leaves page and the dashboard." },
    { q: "How do I submit an expense claim?", a: "Go to My Expenses → click 'New Claim' to submit an expense." },
    { q: "How do I change my password?", a: "Go to Settings → Account & Security to update your password." },
    { q: "How do I view my performance review?", a: "Go to My Performance → Reviews section to view your reviews." },
    { q: "What is my Employee ID?", a: "Your Employee ID is EMP-0142, visible on your profile." },
    { q: "How do I enroll for health insurance?", a: "Contact HR during the open enrollment period or within 30 days of joining." },
  ];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { icon: Calendar, label: "Attendance & Leave", count: "12 articles", bg: "var(--secondary)", color: "var(--primary)" },
          { icon: FileText, label: "Payroll & Salary", count: "8 articles", bg: "#EDE9FE", color: "#8B5CF6" },
          { icon: User, label: "Documents & Profile", count: "6 articles", bg: "var(--secondary)", color: "var(--primary)" },
          { icon: Receipt, label: "Expenses", count: "5 articles", bg: "#FEF3C7", color: "#F59E0B" },
          { icon: Target, label: "Goals & Performance", count: "7 articles", bg: "var(--secondary)", color: "var(--primary)" },
          { icon: Lock, label: "Account & Security", count: "9 articles", bg: "#FEE2E2", color: "#EF4444" },
        ].map((cat, i) => {
          const Icon = cat.icon;
          return (
        ...
        {filtered.map((faq, i) => {
          ...
          return (
            <div key={i} className="border-b border-border/50 last:border-b-0">
              <button onClick={() => setExpandedFaq(expanded ? null : i)}
                className="w-full flex items-center justify-between px-6 py-4 text-left hover:bg-secondary/50 transition-colors">
                <span className="text-[14px] font-black text-foreground flex-1 pr-4">{faq.q}</span>
                <span className="text-muted-foreground text-[14px] shrink-0">{expanded ? "▾" : "▸"}</span>
              </button>
              {expanded && (
                <div className="px-6 pb-5">
                  <p className="text-[14px] font-bold text-muted-foreground leading-relaxed">{faq.a}</p>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-8 text-center">
        ...
        {helpful === null ? (
          <div className="flex justify-center gap-4">
            <button onClick={() => setHelpful("yes")} className="px-6 py-2.5 rounded-xl border border-border text-[13px] font-black text-foreground hover:bg-secondary transition-all">👍 Yes</button>
            <button onClick={() => setHelpful("no")} className="px-6 py-2.5 rounded-xl border border-border text-[13px] font-black text-foreground hover:bg-secondary transition-all">👎 No</button>
          </div>
        ) : helpful === "yes" ? (
          <p className="text-[16px] font-black text-primary">Thank you for your feedback! 🙏</p>
        ) : (
...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
function ContactSupport({ onModal, navigate }: { onModal: (m: string | null) => void; navigate: (p: string) => void }) {
  return (
    <div>
      <Breadcrumb active="Contact HR / Support" />
      <h2 className="text-[22px] font-black text-foreground mb-5">Contact HR & Support</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {[
          { icon: Headphones, bg: "#E0F2FE", color: "#0EA5E9", title: "Raise a Support Ticket", desc: "For IT issues, HR requests, payroll queries", btn: "Create Ticket", onClick: () => navigate("/support") },
          { icon: Users, bg: "var(--secondary)", color: "var(--primary)", title: "Contact HR Team", desc: "For policy questions, personal matters, HR issues", email: "hr@nexushr.com", phone: "+91 80 1234 5678" },
          { icon: Monitor, bg: "#EDE9FE", color: "#8B5CF6", title: "IT Helpdesk", desc: "For laptop, software, VPN, access issues", email: "it@nexushr.com", onClick: () => navigate("/support") },
          { icon: AlertCircle, bg: "#FEE2E2", color: "#EF4444", title: "Emergency", desc: "For urgent matters requiring immediate attention", phone: "+91 98765 00000", emergency: true },
        ].map((card, i) => {
          const Icon = card.icon;
          return (
            <div key={i} className="bg-card rounded-2xl border border-border shadow-sm p-6">
              ...
              <p className="text-[16px] font-black text-foreground mb-1">{card.title}</p>
              <p className="text-[13px] font-bold text-muted-foreground mb-4">{card.desc}</p>
              {card.email && (
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[13px] font-black text-primary">{card.email}</span>
                  <button onClick={() => { navigator.clipboard.writeText(card.email!); showToast("Copied", "success", "Email copied!"); }}
                    className="p-1.5 hover:bg-secondary rounded-lg transition-all">
                    <Copy size={14} className="text-muted-foreground" />
                  </button>
                </div>
              )}
              {card.phone && !card.emergency && <p className="text-[13px] font-bold text-muted-foreground mb-3">{card.phone}</p>}
              {card.emergency && (
                <>
                  <p className="text-[15px] font-black text-rose-500">{card.phone}</p>
                  <p className="text-[12px] font-bold text-muted-foreground italic mt-1">Only for genuine emergencies</p>
                </>
              )}
              {card.btn && (
        ...
        {[
          ...
          <div key={i} className="flex items-center justify-between px-6 py-4 border-b border-border/50 last:border-b-0 hover:bg-[#00B87C]/[0.08] transition-colors">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[14px] font-black text-foreground">{contact.name}</p>
                <p className="text-[12px] font-bold text-muted-foreground">{contact.role}</p>
                <p className="text-[12px] font-black text-primary">{contact.email}</p>
              </div>
            </div>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
        <div className="space-y-3 mb-5">
          {[
            ...
            { label: "IT Support", hours: "Monday–Saturday, 8:00 AM – 8:00 PM IST" },
            { label: "Emergency", hours: "24/7" },
          ].map((item, i) => (
            <div key={i} className="flex items-center justify-between">
              <span className="text-[14px] font-bold text-muted-foreground">{item.label}</span>
              <span className="text-[14px] font-black text-foreground">{item.hours}</span>
            </div>
          ))}
        </div>
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-primary animate-pulse" />
          <span className="text-[13px] font-black text-primary">HR Team is currently available</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-center justify-between py-1.5 group">
      <div className="flex-1 min-w-0 pr-4">
        <p className={`text-[14px] font-black ${disabled ? "text-muted-foreground/60" : "text-foreground"} group-hover:text-primary transition-colors`}>{label}</p>
        {desc && <p className="text-[12px] font-bold text-muted-foreground mt-0.5">{desc}</p>}
      </div>
