    ...
    <div className="w-full px-4 md:px-8 py-6 space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[13px] text-[#6B7280]">
              Access and manage your digital repository
            </p>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 px-6 py-4 rounded-2xl border bg-muted/5 border-border shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 rounded bg-amber-500/10 text-amber-600 text-[9px] font-black uppercase">
            Expiring
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 rounded bg-rose-500/10 text-rose-600 text-[9px] font-black uppercase">
            Required
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {categories.map((category) => (
          <div
            key={category.id}
            className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm hover:border-[#00B87C]/30 transition-all flex flex-col h-full"
          >
            <div className="p-6 border-b border-border flex items-center justify-between bg-muted/10">
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-xl ${category.iconBg} flex items-center justify-center ${category.iconColor}`}
                >
              ...
              <span
                className={`px-2.5 py-1 rounded-lg ${category.chipColor} text-[11px] font-bold uppercase tracking-widest`}
              >
              ...
              {category.files.map((file, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <FileText
                      size={16}
                      className="text-muted-foreground/40 group-hover:text-[#00B87C] transition-all"
                    />
                    <span className="text-[14px] font-bold text-foreground">
                      {file.name}
                    </span>
                    {file.status === "Expiring" && (
                      <span className="px-2 py-0.5 rounded bg-amber-500/10 text-amber-600 text-[9px] font-black uppercase tracking-tight">
                        Expires {file.expiryDate}
                      </span>
                    )}
                    {file.status === "Not Uploaded" && (
                      <span className="px-2 py-0.5 rounded bg-rose-500/10 text-rose-600 text-[9px] font-black uppercase tracking-tight">
                        Not Uploaded
                      </span>
                    )}
                  </div>
                    ...
                    file.status === "Expiring" ? (
                      <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all">
                        ...
                        {file.canUpdate && (
                          <button
                            onClick={() =>
                              handleOpenUpload(category.id, file.name)
                            }
                ...
                <button
                  onClick={() => {
                    ...
                    showToast(
                      "Downloading",
                      "success",
                      "Your file download started successfully.",
                    );
                  }}
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const firstFile =
                    categories.find((c) => c.id === e.target.value)?.files[0]
                      ?.name || "";
            ...
            <div className="space-y-2">
              ...
              {preSelected ? (
                ...
                >
                  ...
                  <option value="Other Custom Document">
