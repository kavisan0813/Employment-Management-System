...
function StatusChip({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    "Awaiting Finance Clearance":
      "bg-[#FFFBEB] text-[#D97706] border-[#FBBF24]/20",
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-[11px] font-semibold uppercase tracking-wider border flex items-center w-fit gap-1.5 ${styles[status] || "bg-[#FFFBEB] text-[#D97706]"}`}
    >
...
export function FinanceSettlements() {
  ...
  const __initialState = {
    exits: readExits() as ExitEmployee[],
    selectedExit: null as ExitEmployee | null,
    salary: 0,
    gratuity: 0,
    leaveEncashment: 0,
    reimbursements: 0,
  ...
  const setExits = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        exits: typeof val === "function" ? val(prev.exits) : val,
      })),
    [],
  );
  const setSelectedExit = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        selectedExit: typeof val === "function" ? val(prev.selectedExit) : val,
      })),
    [],
  );
  const setSalary = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        salary: typeof val === "function" ? val(prev.salary) : val,
      })),
    [],
  );
  const setGratuity = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        gratuity: typeof val === "function" ? val(prev.gratuity) : val,
      })),
    [],
  );
  const setLeaveEncashment = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        leaveEncashment:
          typeof val === "function" ? val(prev.leaveEncashment) : val,
      })),
    [],
  );
  const setReimbursements = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        reimbursements:
          typeof val === "function" ? val(prev.reimbursements) : val,
      })),
    [],
  );
  const setBonus = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        bonus: typeof val === "function" ? val(prev.bonus) : val,
      })),
    [],
  );
  const setIncentives = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        incentives: typeof val === "function" ? val(prev.incentives) : val,
      })),
    [],
  );
  const setOtherEarnings = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        otherEarnings:
          typeof val === "function" ? val(prev.otherEarnings) : val,
      })),
    [],
  );
  const setNoticePeriodRecovery = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        noticePeriodRecovery:
          typeof val === "function" ? val(prev.noticePeriodRecovery) : val,
      })),
    [],
  );
  const setLoanRecovery = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        loanRecovery: typeof val === "function" ? val(prev.loanRecovery) : val,
      })),
    [],
  );
  const setAssetRecovery = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        assetRecovery:
          typeof val === "function" ? val(prev.assetRecovery) : val,
      })),
    [],
  );
  const setTaxDeduction = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        taxDeduction: typeof val === "function" ? val(prev.taxDeduction) : val,
      })),
    [],
  );
  const setPfEsiAdjustment = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        pfEsiAdjustment:
          typeof val === "function" ? val(prev.pfEsiAdjustment) : val,
      })),
    [],
  );
  const setOtherDeductions = useCallback(
    (val: any) =>
      __updateState((prev: any) => ({
        otherDeductions:
          typeof val === "function" ? val(prev.otherDeductions) : val,
      })),
    [],
  );
  ...
  const handleSave = (status: string) => {
    ...
    const updatedExits = exits.map((exit) => {
      ...
      const clearances = clearance.filter(
        (item) => item.status === "cleared",
      ).length;
      const returnedAssets = exit.assets.filter(
  ...
  const pendingExits = exits.filter(
    (e) =>
      e.ffStatus === "Pending" ||
      e.ffStatus === "Awaiting Finance Clearance" ||
      e.ffStatus === "Draft",
  );
  const approvedExits = exits.filter(
    (e) => e.ffStatus === "Approved & Processed",
  );
    ...
    const csvContent =
      ...
      exits
        .map((e) => {
          const gross =
            e.totalEarnings ||
            e.salary + e.gratuity + e.leaveEncashment + e.reimbursements;
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[13px] text-muted-foreground font-semibold">
              Calculate earnings, deductions, and finalize F&F Settlements for
              exiting employees
            </p>
          </div>
        </div>
              ...
              {exits.map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`text-[13px] font-bold ${ded > 0 ? "text-[#EF4444]" : "text-muted-foreground"}`}
                      >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[12px] font-bold text-muted-foreground">
                      {selectedExit.name} · {selectedExit.designation} (
                      {selectedExit.department})
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        disabled={
                          !canManage ||
                          selectedExit.ffStatus === "Approved & Processed"
                        }
                        value={salary}
                        onChange={(e) =>
                          setSalary(
                ...
                <div className="col-span-1 md:col-span-2 bg-[#F0FDF4] dark:bg-[#00B87C]/5 p-6 rounded-3xl border border-[#00B87C]/20 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[26px] font-black text-[#00B87C] tracking-tight">
