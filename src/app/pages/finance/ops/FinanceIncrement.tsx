...
export function FinanceIncrement() {
  ...
  const [approvingEmployee, setApprovingEmployee] =
    useState<AppraisalEmployee | null>(() => {
      if (location.state?.employeeId) {
        const found = MOCK_APPRAISALS.find(
  ...
  const [rejectingEmployee, setRejectingEmployee] =
    useState<AppraisalEmployee | null>(null);
  ...
  const handleApprove = () => {
    if (approvingEmployee) {
      showToast(
        "Increment Approved",
        "success",
        `Increment approved for ${approvingEmployee.name}. Payroll updated.`,
      );
  ...
  const handleReject = () => {
    if (rejectingEmployee) {
      showToast(
        "Increment Rejected",
        "success",
        `Increment for ${rejectingEmployee.name} has been rejected.`,
      );
  ...
  const executeExport = () => {
    ...
    setTimeout(() => {
      const headers = [
        "Employee ID,Name,Department,Current Salary,Increment %,Revised Salary,Status",
      ];
      const rows = MOCK_APPRAISALS.map(
        (emp) =>
      ...
      showToast(
        "Export Successful",
        "success",
        "Increment & Appraisal report has been downloaded.",
      );
    }, 1500);
  };
  ...
  const filteredAppraisals = MOCK_APPRAISALS.filter((emp) => {
    ...
    const matchesIncrement =
      incrementFilter === "Any Increment" ||
      (incrementFilter === "> 10%" && emp.recommendedIncrement > 10) ||
      (incrementFilter === "5% - 10%" &&
        emp.recommendedIncrement >= 5 &&
        emp.recommendedIncrement <= 10) ||
      (incrementFilter === "< 5%" && emp.recommendedIncrement < 5);
    const matchesPerformance =
      performanceFilter === "Any Rating" ||
      (performanceFilter === "> 4.5" && emp.managerRating > 4.5) ||
      (performanceFilter === "4.0 - 4.5" &&
        emp.managerRating >= 4.0 &&
        emp.managerRating <= 4.5) ||
  ...
  return (
    <div className="w-full px-4 md:px-8 py-6 pb-10 space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[13px] text-[#6B7280]">
              Manage salary increments and performance appraisals
            </p>
          </div>
        </div>
        <button
      ...
      <div className="bg-[#F8FAFC] dark:bg-muted/10 border border-border/50 rounded-2xl px-6 py-3 flex flex-wrap items-center gap-x-8 gap-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[13px] font-bold text-foreground">
            9 employees eligible for increment
          </span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-amber-500" />
          <span className="text-[13px] font-bold text-foreground">
            2 employees need manual review (&lt; 85%)
          </span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-purple-500" />
          <span className="text-[13px] font-bold text-foreground">
            Estimated payroll increase: ₹1.0L this cycle
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 bg-card border border-border p-4 rounded-2xl shadow-sm">
        ...
        <button
          onClick={handleExport}
          className="ml-auto flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#00B87C] text-white font-black text-[12px] uppercase tracking-widest hover:opacity-90 transition-all shadow-lg shadow-emerald-500/20"
        >
          <Download size={16} />
          Export CSV
        </button>
        ...
        <div className="p-6 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] font-bold text-[#00B87C]">
              10 employees · Click row · Sort to reorder
            </span>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-border">
              {filteredAppraisals.map((emp, i) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`text-[13px] font-black ${emp.recommendedIncrement >= 10 ? "text-emerald-600" : "text-amber-600"}`}
                    >
                        ...
                        <button
                          onClick={(e) => {
                            ...
                            showToast(
                              "Redirecting",
                              "info",
                              "Opening Payroll records...",
                            );
                          }}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border text-[11px] font-semibold uppercase tracking-wider text-muted-foreground hover:bg-muted transition-all"
                        >
            ...
            >
              <div className="p-8 pb-0 flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-black text-foreground tracking-tight">
                  Approve Increment — {approvingEmployee.name}
                </h3>
                ...
                <div className="w-full mt-6 p-4 rounded-2xl bg-muted/30 border border-border flex items-center gap-4 text-left">
                  ...
                  <div>
                    <p className="text-[15px] font-black text-foreground leading-tight">
                      {approvingEmployee.name}
                    </p>
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">
                        {approvingEmployee.designation}
                      </p>
                      <span className="px-1.5 py-0.5 rounded-md bg-[#00B87C]/10 text-[#00B87C] text-[9px] font-black uppercase">
                        {approvingEmployee.department}
                      </span>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-3 gap-x-4">
                      <div>
                        ...
                        <p className="text-[15px] font-bold text-foreground">
                          ₹{approvingEmployee.currentSalary.toLocaleString()}/mo
                        </p>
                      </div>
                      <div>
                        <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-widest mb-1">
                        ...
                        <p className="text-[15px] font-black text-emerald-600">
                          +{approvingEmployee.recommendedIncrement}%
                        </p>
                      </div>
                      <div className="col-span-2 pt-3 border-t border-emerald-500/10 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xl font-black text-[#00B87C]">
                            ₹{approvingEmployee.revisedSalary.toLocaleString()}
                          </p>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="text-[13px] font-black text-foreground">
                            ₹
                            {(
                              (approvingEmployee.revisedSalary -
                                approvingEmployee.currentSalary) *
                              12
                            ).toLocaleString()}
                          </p>
                        </div>
              ...
              <div className="p-8 flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-3.5 rounded-2xl bg-[#00B87C] text-white font-black text-[12px] uppercase tracking-widest hover:opacity-90 transition-all shadow-lg shadow-emerald-500/20"
                >
                  Confirm & Approve
                </button>
              </div>
            ...
            >
              <div className="p-8 pb-0 flex flex-col items-center text-center">
                ...
                <p className="text-[13px] font-bold text-muted-foreground mt-2 mb-6">
                  Are you sure you want to reject the increment for{" "}
                  {rejectingEmployee.name}?
                </p>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between p-3 rounded-xl bg-secondary/50 border border-border">
                      ...
                      <span className="text-[12px] font-black text-foreground flex items-center gap-1">
                        <Star
                          size={12}
                          className="text-amber-500 fill-amber-500"
                        />{" "}
                        {detailedEmployee.managerRating}/5
                      </span>
                    </div>
                    <div className="flex justify-between p-3 rounded-xl bg-secondary/50 border border-border">
                      <span className="text-[12px] font-bold text-muted-foreground">
                      ...
                      <span className="text-[12px] font-black text-foreground">
                        {detailedEmployee.performanceScore}%
                      </span>
                    </div>
                    <div className="flex justify-between p-3 rounded-xl bg-secondary/50 border border-border">
                      <span className="text-[12px] font-bold text-muted-foreground">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between p-3 rounded-xl bg-secondary/50 border border-border">
                      ...
                      <span className="text-[12px] font-black text-foreground">
                        ₹{detailedEmployee.currentSalary.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between p-3 rounded-xl bg-secondary/50 border border-border">
                      <span className="text-[12px] font-bold text-muted-foreground">
                      ...
                      <span className="text-[12px] font-black text-[#00B87C]">
                        +{detailedEmployee.recommendedIncrement}%
                      </span>
                    </div>
                    <div className="flex justify-between p-3 rounded-xl bg-[#F0FDF4] dark:bg-emerald-500/5 border border-[#00B87C]/20">
                      ...
                      <span className="text-[12px] font-black text-[#00B87C]">
                        ₹{detailedEmployee.revisedSalary.toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
