...
const radarData = [
  { subject: 'Finance Knowledge', self: 90, manager: 85, peers: 80, fullMark: 150 },
          ...
          {activeTab === "My Goals" && (
            ...
            >
              ...
              <div className="bg-card border border-border rounded-[32px] overflow-hidden">
                <GoalRow title="Complete TDS automation workflow" category="Finance" progress={80} status="On track" />
                <GoalRow title="Reduce payroll errors to < 0.5%" category="Finance" progress={95} status="Completed" isComplete />
                <GoalRow title="Implement new salary band structure" category="Finance" progress={40} status="At risk" />
                <GoalRow title="Get certified in GST compliance" category="Compliance" progress={100} status="Completed" isComplete />
                <GoalRow title="Mentor junior finance analyst" category="Leadership" progress={60} status="On track" />
              </div>
            </motion.div>
          )}
                ...
                <div className="bg-card border border-border rounded-[32px] p-8 h-[400px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
                      ...
                      <Radar name="Self" dataKey="self" stroke="#00B87C" fill="#00B87C" fillOpacity={0.4} />
                      <Radar name="Manager" dataKey="manager" stroke="#3B82F6" fill="#3B82F6" fillOpacity={0.4} />
                      <Radar name="Peers" dataKey="peers" stroke="#8B5CF6" fill="#8B5CF6" fillOpacity={0.4} />
            ...
            >
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-border">
                  <HistoryRow period="FY 24-25" rating="4.1" band="B" reviewer="Rajesh Kumar" date="Apr 2025" />
                  <HistoryRow period="FY 23-24" rating="3.9" band="B-" reviewer="Rajesh Kumar" date="Apr 2024" />
                </tbody>
              </table>
            </motion.div>
          )}
...
function GoalRow({ title, category, progress, status, isComplete }: { title: string, category: string, progress: number, status: string, isComplete?: boolean }) {
  const statusColor = status === "On track" ? "text-emerald-600 bg-emerald-500/10" : 
