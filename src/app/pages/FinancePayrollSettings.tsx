        ...
        <div className="space-y-6">
          ...
          <section className="bg-card border border-border rounded-[32px] overflow-hidden shadow-sm">
            <div className="px-6 py-4 bg-emerald-500/5 border-b border-border flex items-center gap-3">
              ...
              <h2 className="text-[11px] font-black text-emerald-700 uppercase tracking-widest">Pay Cycle Configuration</h2>
            </div>
            ...
            <div className="p-8 space-y-8">
              <div className="bg-[#F0FDF4] dark:bg-emerald-500/5 rounded-2xl p-6 border border-emerald-500/10 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                ...
                <Field label="Pay Slip Generation" value="Auto on Pay Day" type="select" />
                <Field label="Bank Transfer Mode" value="NEFT/RTGS" type="select" />
              </div>
            </div>
          </section>
          ...
          <section className="bg-card border border-border rounded-[32px] overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-2 h-2 rounded-full bg-emerald-500" />
                <h2 className="text-[11px] font-black text-muted-foreground uppercase tracking-widest">Salary Components</h2>
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-border">
                  <ComponentRow name="Basic Salary" type="Fixed" calc="50% of CTC" taxable="Yes" pf="Yes" />
                  <ComponentRow name="HRA" type="Fixed" calc="40% of Basic" taxable="Partial" pf="No" />
                  <ComponentRow name="Conveyance" type="Fixed" calc="₹1,600/month" taxable="No" pf="No" />
                  <ComponentRow name="Medical Allowance" type="Fixed" calc="₹1,250/month" taxable="Partial" pf="No" />
                  <ComponentRow name="Performance Bonus" type="Variable" calc="% of Basic" taxable="Yes" pf="No" />
                  <ComponentRow name="Special Allowance" type="Variable" calc="Balance of CTC" taxable="Yes" pf="No" />
                </tbody>
              </table>
            </div>
          </section>
          ...
          <section className="bg-card border border-border rounded-[32px] overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-border flex items-center gap-3">
              <div className="w-2 h-2 rounded-full bg-emerald-500" />
              <h2 className="text-[11px] font-black text-muted-foreground uppercase tracking-widest">Statutory Deductions</h2>
            </div>
  ...
  return (
    <div className="space-y-1">
      ...
      {items.map((item) => (
        ...
        >
          {item.label}
          {activeId === item.id && <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />}
        </button>
      ))}
    </div>
  );
...
function Field({ label, value, type }: { label: string, value: string, type: 'select' | 'number' }) {
  return (
    <div className="space-y-2 group">
      <label className="text-[11px] font-black text-muted-foreground uppercase tracking-widest group-focus-within:text-emerald-600 transition-colors">{label}</label>
      <div className="relative">
        ...
        />
        {type === 'select' && <ChevronDown size={14} className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground" />}
      </div>
    </div>
  );
}

function ComponentRow({ name, type, calc, taxable, pf }: { name: string, type: 'Fixed' | 'Variable', calc: string, taxable: string, pf: string }) {
  return (
    <tr className="hover:bg-muted/10 transition-colors group">
      ...
      <td className="px-6 py-4 text-right">
        <button className="text-[11px] font-black text-emerald-600 uppercase tracking-widest hover:underline">Edit</button>
      </td>
    </tr>
  );
}
...
function ToggleRow({ label, desc, enabled, onToggle }: { label: string, desc: string, enabled: boolean, onToggle: () => void }) {
  return (
    <div className="flex items-center justify-between py-5 group">
      <div>
        <p className="text-[14px] font-black text-foreground tracking-tight group-hover:text-emerald-600 transition-colors">{label}</p>
        <p className="text-[12px] font-medium text-muted-foreground opacity-80">{desc}</p>
      </div>
      <button 
        onClick={onToggle}
        className={`w-12 h-6 rounded-full transition-all duration-300 flex items-center px-1 ${
          enabled ? "bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.3)]" : "bg-slate-200 dark:bg-slate-800"
        }`}
      >
