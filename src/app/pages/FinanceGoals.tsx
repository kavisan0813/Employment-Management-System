...
export function FinanceGoals() {
  ...
  const filteredGoals = goals.filter(goal => activeTab === "All" || goal.status === activeTab);
  ...
  const overallProgress = goals.length > 0 ? Math.round(goals.reduce((acc, g) => acc + g.progress, 0) / goals.length) : 0;
  ...
  const handleAddGoal = (newGoalData: {
    ...
    showToast("Goal Added", "success", `"${newGoalData.title}" has been successfully added.`);
  };
  ...
  const handleCheckIn = (goalId: string) => {
    setGoals(prev => prev.map(g => {
      ...
      const newStatus: GoalStatus = newProgress === 100 ? "Completed" : g.status === "At Risk" && newProgress >= 50 ? "In Progress" : g.status;
  ...
  return (
    <div className="w-full px-4 md:px-8 py-6 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="GOALS SET" value={String(goalsSetCount)} color="default" />
        <KPICard label="COMPLETED" value={String(completedCount)} color="green" />
        <KPICard label="IN PROGRESS" value={String(inProgressCount)} color="teal" />
      ...
      <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[15px] font-black text-foreground tracking-tight">Overall Progress: {overallProgress}%</span>
          </div>
          <span className="text-[12px] font-bold text-muted-foreground">{goalsOnTrack} / {goals.length} goals on track</span>
        </div>
          ...
          <GoalDetailsModal
            ...
            onUpdateProgress={(id, progress) => {
              setGoals(prev => prev.map(g => {
                ...
                const newStatus = isCompleted ? "Completed" : g.status === "Completed" ? "In Progress" : g.status;
                showToast(
                  "Goal Updated",
                  "success",
                  `Progress for "${g.title}" updated to ${newProgress}%.`
              ...
              setSelectedGoal(prev => {
                ...
                const newProgress = Math.min(progress, 100);
                const isCompleted = newProgress === 100;
                const newStatus = isCompleted ? "Completed" : prev.status === "Completed" ? "In Progress" : prev.status;
                return {
                  ...prev,
                  progress: newProgress,
                  status: newStatus,
...
const GoalItem = forwardRef<HTMLDivElement, { goal: Goal; onCheckIn: (id: string) => void; onViewDetails: (goal: Goal) => void }>(({ goal, onCheckIn, onViewDetails }, ref) => {
      ...
      >
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={onClose} className="p-2 rounded-xl hover:bg-muted text-muted-foreground"><X size={20} /></button>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {(["Finance", "Compliance", "Leadership", "Personal", "Technical"] as Category[]).map(cat => (
                  <button 
                    key={cat} 
                    type="button"
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 py-3 rounded-2xl border border-border text-[12px] font-bold uppercase tracking-widest hover:bg-muted transition-all text-foreground bg-transparent">Cancel</button>
            <button type="submit" className="flex-2 px-8 py-3 rounded-2xl bg-[#00B87C] text-white text-[12px] font-bold uppercase tracking-widest hover:bg-[#009966] transition-all shadow-lg shadow-[#00B87C]/20 border-0">Add Goal</button>
          </div>
        </form>
      </motion.div>
    </div>
