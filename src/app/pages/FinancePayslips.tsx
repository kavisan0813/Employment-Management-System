...
const payslipsData: Payslip[] = [
  { id: "1", month: "March", year: "2026", gross: "₹1,00,000", deductions: "₹17,600", net: "₹82,400", status: "Credited", isLatest: true },
  { id: "2", month: "February", year: "2026", gross: "₹1,00,000", deductions: "₹17,600", net: "₹82,400", status: "Credited" },
  { id: "3", month: "January", year: "2026", gross: "₹1,00,000", deductions: "₹17,600", net: "₹82,400", status: "Credited" },
  { id: "4", month: "December", year: "2025", gross: "₹1,00,000", deductions: "₹17,600", net: "₹82,400", status: "Credited" },
  { id: "5", month: "November", year: "2025", gross: "₹1,00,000", deductions: "₹17,600", net: "₹82,400", status: "Credited" },
  { id: "6", month: "October", year: "2025", gross: "₹1,00,000", deductions: "₹17,600", net: "₹82,400", status: "Credited" },
];
  ...
  return (
    <div className="w-full px-4 md:px-8 py-6 space-y-6 animate-in fade-in duration-500 overflow-hidden relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <KPICard label="CURRENT CTC" value="₹12,00,000" subValue="Annual" color="purple" />
        <KPICard 
          ...
          chip="Credited" 
        />
        <KPICard label="YTD EARNINGS" value="₹8.2L" subValue="FY 2025-26" color="default" />
      </div>

      {/* TABLE SECTION */}
      <div className="bg-card border border-border rounded-[32px] overflow-hidden shadow-sm">
        <div className="p-6 border-b border-border flex items-center justify-between bg-muted/20">
          <h3 className="text-[14px] font-black text-foreground uppercase tracking-widest">PAYSLIPS — FY 2025-26</h3>
                ...
                <tr key={payslip.id} className="h-14 hover:bg-[#F0FDF4] dark:hover:bg-emerald-500/5 transition-all group">
                  <td className="px-8 text-[14px] font-bold text-foreground">
                    <div className="flex items-center gap-3">
                      ...
                      {payslip.isLatest && (
                        <span className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-[#00B87C] text-[9px] font-black uppercase">Latest</span>
                      )}
                    </div>
                  </td>
                  <td className="px-8 text-[14px] font-bold text-muted-foreground">{payslip.gross}</td>
          ...
          <>
            ...
            >
              <div className="p-6 border-b border-border flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-black text-foreground tracking-tight">Salary Slip</h3>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-8">
                  ...
                  <InfoItem label="Pay Period" value={`${selectedPayslip.month} ${selectedPayslip.year}`} />
                  <InfoItem label="Pay Date" value="Apr 1, 2026" />
                </div>

                {/* EARNINGS TABLE */}
                <div className="space-y-4">
                  <h5 className="text-[11px] font-black text-muted-foreground uppercase tracking-widest">EARNINGS</h5>
                  <div className="space-y-3 bg-muted/10 p-4 rounded-2xl border border-border/50">
                    ...
                    <div className="pt-3 border-t border-border flex justify-between items-center">
                      <span className="text-xs font-black text-foreground uppercase">Gross Earnings</span>
                      <span className="text-[15px] font-black text-foreground">₹1,00,000</span>
                    </div>
                  </div>
                </div>
                ...
                {/* DEDUCTIONS TABLE */}
                <div className="space-y-4">
                  <h5 className="text-[11px] font-black text-muted-foreground uppercase tracking-widest">DEDUCTIONS</h5>
                  <div className="space-y-3 bg-muted/10 p-4 rounded-2xl border border-border/50">
                    ...
                    <div className="pt-3 border-t border-border flex justify-between items-center">
                      <span className="text-xs font-black text-rose-600 uppercase">Total Deductions</span>
                      <span className="text-[15px] font-black text-rose-600">₹17,600</span>
                    </div>
                  </div>
                </div>
...
function KPICard({ label, value, subValue, color, chip }: { label: string, value: string, subValue: string, color: 'purple' | 'green' | 'default', chip?: string }) {
  const colorClasses = {
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-6 shadow-sm group hover:border-[#00B87C]/30 transition-all">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[11px] font-bold text-muted-foreground tracking-tight">{subValue}</span>
          {chip && (
            <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-600 text-[9px] font-black uppercase">{chip}</span>
          )}
        </div>
      </div>
    </div>
