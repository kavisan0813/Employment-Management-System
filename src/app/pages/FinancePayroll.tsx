...
export function FinancePayroll() {
  ...
  const filteredRecords = MOCK_RECORDS.filter((rec) => {
    ...
    const matchesDept =
      ...
      rec.department === departmentFilter;
    const matchesStatus =
  ...
  return (
    <div className="w-full px-4 md:px-8 py-6 pb-10 space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[13px] text-[#6B7280]">
              Manage salary disbursements and slips
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3 relative">
          ...
          {exportDropdownOpen && (
            <div className="absolute top-full right-40 mt-2 w-56 bg-card border border-border rounded-xl shadow-xl z-50 overflow-hidden">
              <button
                onClick={() => handleExport("Full Payroll")}
                className="w-full text-left px-4 py-3 text-[13px] font-bold text-foreground hover:bg-muted transition-colors border-b border-border"
              >
                Export Full Payroll CSV
              </button>
              <button
              ...
              <button
                onClick={() => handleExport("Tax Withholdings")}
                className="w-full text-left px-4 py-3 text-[13px] font-bold text-foreground hover:bg-muted transition-colors border-b border-border"
              >
                Export Tax Withholdings CSV
              </button>
              <button
                onClick={() => handleExport("Bank Transfer Format")}
                className="w-full text-left px-4 py-3 text-[13px] font-bold text-foreground hover:bg-muted transition-colors"
              >
                Export Bank Transfer Format
              </button>
            </div>
          )}
          <button
      ...
      <div className="w-full py-3 px-6 rounded-2xl bg-[#DCFCE7]/40 dark:bg-emerald-500/5 border border-emerald-100/50 dark:border-emerald-500/10 flex flex-wrap items-center gap-y-3 gap-x-8 shadow-sm backdrop-blur-sm">
        ...
        <div
          className="flex items-center gap-2 md:border-l border-emerald-200/50 dark:border-emerald-800/50 md:pl-8 cursor-pointer hover:opacity-80 transition-opacity"
          onClick={() => setGenericModalTitle("Payroll Lock Details")}
        >
          <div className="w-2 h-2 rounded-full bg-amber-500" />
          <span className="text-[12px] font-bold text-foreground/80 tracking-tight">
            Lock Date: <span className="text-foreground">Apr 20</span>
          </span>
        </div>
        <div
          className="flex items-center gap-2 md:border-l border-emerald-200/50 dark:border-emerald-800/50 md:pl-8 cursor-pointer hover:opacity-80 transition-opacity"
          onClick={() => setGenericModalTitle("Disbursement Schedule")}
        >
          <div className="w-2 h-2 rounded-full bg-rose-500" />
          <span className="text-[12px] font-bold text-foreground/80 tracking-tight">
            Payment: <span className="text-foreground">Apr 28</span>
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div
          onClick={() => setGenericModalTitle("Processed Employees Details")}
        >
          <KPICard
      ...
      <div className="flex flex-wrap items-center gap-3 bg-muted/10 p-4 rounded-2xl border border-border/50">
        ...
        <button
          className="ml-auto flex items-center gap-2 px-5 py-2.5 bg-[#00B87C]/10 text-[#00B87C] font-black text-[12px] uppercase tracking-widest rounded-xl hover:bg-[#00B87C]/20 transition-all border border-[#00B87C]/20"
            ...
            <tbody className="divide-y divide-border">
              {filteredRecords.map((rec, i) => (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="overflow-hidden">
                        ...
                        <p className="text-[12px] font-semibold text-muted-foreground truncate">
                          {rec.role}
                        </p>
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-4 text-[14px] font-semibold text-[#374151] dark:text-gray-300">
                    ₹{rec.basic.toLocaleString()}
                  </td>
                  <td className="px-6 py-4 text-[14px] font-semibold text-[#374151] dark:text-gray-300">
                    ₹{rec.hra.toLocaleString()}
                  </td>
                  <td className="px-6 py-4 text-[14px] font-semibold text-[#374151] dark:text-gray-300">
                    ₹{rec.allowances.toLocaleString()}
                  </td>
                  <td className="px-6 py-4 text-[14px] font-semibold text-[#374151] dark:text-gray-300">
                    ₹{rec.gross.toLocaleString()}
                  </td>
                  <td className="px-6 py-4 text-[14px] text-[#EF4444] font-black">
                    ₹{rec.deductions.toLocaleString()}
                  </td>
                  <td className="px-6 py-4 text-[14px] text-[#00B87C] font-black">
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8">
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      ...
                      <TableItem
                        ...
                        value={selectedEmployee.allowances - 1600}
                      />
                      <div className="pt-4 border-t border-dashed border-border flex justify-between items-center">
                        <span className="text-[14px] font-black text-foreground uppercase tracking-tight">
                          Gross Earnings
                        </span>
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="pt-4 border-t border-dashed border-border flex justify-between items-center">
                        ...
                        <span className="text-[16px] font-black text-[#EF4444]">
                          ₹27,600
                        </span>
                      </div>
                    </div>
                ...
                <div className="p-8 rounded-[32px] bg-gradient-to-br from-[#F0FDF4] to-[#DCFCE7] dark:from-emerald-500/10 dark:to-emerald-500/5 border border-[#00B87C]/20 text-center shadow-xl shadow-emerald-500/5">
                  <p className="text-[11px] font-black text-[#00B87C] uppercase tracking-[2px] mb-2">
                    Net Pay Amount
                  </p>
                  <h2 className="text-[44px] font-black text-[#00B87C] tracking-tighter">
                    ₹{selectedEmployee.net.toLocaleString()}
                  </h2>
                  <p className="text-[11px] font-bold text-emerald-600/60 mt-2 italic">
                    Twelve Lakh Twenty Four Thousand Rupees Only
                  </p>
                </div>
              </div>
                ...
                {runModalStep === 1 ? (
                  <>
                    ...
                    <div className="space-y-3.5 mb-8">
                      ...
                      />
                      <ChecklistItem
                        label="Leave data verified for all employees"
                        status="success"
                      />
                      <ChecklistItem
                    ...
                    <div className="border border-border rounded-xl overflow-hidden mb-6">
                      <table className="w-full text-left">
                        <thead className="bg-muted">
                          <tr>
                            <th className="px-4 py-2 text-[11px] font-bold text-muted-foreground uppercase">
                              Category
                            </th>
                            <th className="px-4 py-2 text-[11px] font-bold text-muted-foreground uppercase text-right">
                              Amount
                            </th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-border">
                          ...
                          <tr className="bg-emerald-500/5">
                            <td className="px-4 py-3 text-[14px] font-black text-foreground">
                              Net Disbursement
                            </td>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-end gap-2">
        ...
        {subValue && (
          <span className="text-[12px] font-bold text-muted-foreground mb-1.5">
            {subValue}
          </span>
        )}
      </div>
...
function InfoItem({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-[1.5px] mb-1.5">
        {label}
      </p>
...
function TableItem({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex justify-between items-center group/item">
      <span className="text-[14px] text-muted-foreground font-bold group-hover/item:text-foreground transition-colors">
        {label}
      </span>
...
function ChecklistItem({
  ...
  return (
    <div className="flex items-center gap-4 bg-muted/20 p-3.5 rounded-2xl border border-border/50">
      ...
      <span
        className={`text-[13px] font-bold ${status === "success" ? "text-foreground" : status === "warning" ? "text-amber-700" : "text-teal-700"}`}
      >
        {label}
      </span>
      {status === "success" && (
        <div className="ml-auto w-5 h-5 rounded-full bg-emerald-500 flex items-center justify-center text-white text-[11px] font-semibold">
          ✓
        </div>
      )}
    </div>
