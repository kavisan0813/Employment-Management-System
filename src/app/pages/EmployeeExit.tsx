...
const EXIT_TASKS: ExitTask[] = [
  { id: "t1", label: "Submit Resignation Letter", description: "Formal resignation submitted to manager", responsible: "You", responsibleChip: "You", status: "done", actionLabel: "View", actionType: "link", completedDate: "Mar 28" },
  { id: "t2", label: "Resignation Accepted", description: "Manager has accepted your resignation", responsible: "HR", responsibleChip: "HR", status: "done", completedDate: "Mar 30" },
  { id: "t3", label: "Upload Clearance Documents", description: "Submit ID card copy, NOC, and other clearance docs", responsible: "You", responsibleChip: "You", status: "in_progress", actionLabel: "Upload", actionType: "btn" },
  { id: "t4", label: "Return Company Laptop", description: "Submit laptop to IT desk with charger and accessories", responsible: "IT Dept", responsibleChip: "IT", status: "pending", actionLabel: "Confirm Drop-off", actionType: "btn" },
  { id: "t5", label: "Return iPhone 14 Pro", description: "Return company phone with charger and box", responsible: "IT", responsibleChip: "IT", status: "pending", actionLabel: "Confirm Drop-off", actionType: "btn" },
  { id: "t6", label: "Complete Exit Interview", description: "Schedule and complete exit interview with HR", responsible: "HR", responsibleChip: "HR", status: "pending", actionLabel: "Schedule", actionType: "schedule" },
  { id: "t7", label: "Return Access Card", description: "Submit access card to Admin on last working day", responsible: "Admin", responsibleChip: "Admin", status: "pending" },
  { id: "t8", label: "Sign NDA Acknowledgment", description: "Digitally sign the non-disclosure agreement", responsible: "You", responsibleChip: "You", status: "pending", actionLabel: "Sign Digitally", actionType: "sign" },
  { id: "t9", label: "Collect Experience Letter", description: "Experience letter will be issued after last working day", responsible: "HR", responsibleChip: "HR", status: "pending" },
  { id: "t10", label: "F&F Settlement", description: "Final settlement will be processed after all clearances", responsible: "Finance", responsibleChip: "Finance", status: "pending" },
];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[13px] font-semibold text-muted-foreground">Complete all tasks before your last working day</p>
          </div>
        </div>
        <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-card border border-border shadow-sm">
          <div className="flex items-center gap-2">
            <div className="w-20 h-1.5 bg-muted rounded-full overflow-hidden">
              <div className="h-full bg-[#00B87C] rounded-full" style={{ width: `${progress}%` }} />
            </div>
            <span className="text-[12px] font-black text-foreground">{doneCount}/{totalCount} tasks complete</span>
          </div>
        </div>
      </div>
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="relative bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
            ...
            <div className="p-6 pl-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-4">
                  <div className="text-[36px] font-black text-[#EF4444] tracking-tight leading-none">4</div>
                  <div>
                    <p className="text-[13px] font-bold text-foreground">Days Remaining</p>
                    <p className="text-[11px] font-medium text-muted-foreground">Notice Period: Apr 1 – Apr 10, 2026</p>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-[#FEF3C7] border border-[#FBBF24]/30">
                ...
                <span className="text-[12px] font-bold text-[#D97706]">LWD: Apr 10, 2026</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-border bg-secondary/10 flex items-center justify-between">
              ...
              <span className="text-[11px] font-black text-[#00B87C]">{doneCount}/{totalCount} completed</span>
            </div>
              ...
              {tasks.map((task) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] font-medium text-muted-foreground">{task.description}</p>
                  </div>
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-3">
                <div className="flex items-center justify-between py-1.5">
                  <span className="text-[12px] font-medium text-muted-foreground">Last month salary</span>
                  <span className="text-[13px] font-bold text-foreground">{formatCurrency(87000)}</span>
                </div>
                <div className="border-t border-border/50" />
                <div className="flex items-center justify-between py-1.5">
                  <span className="text-[12px] font-medium text-muted-foreground">Gratuity</span>
                  <span className="text-[13px] font-bold text-foreground">{formatCurrency(54000)}</span>
                </div>
                <div className="border-t border-border/50" />
                <div className="flex items-center justify-between py-1.5">
                  <span className="text-[12px] font-medium text-muted-foreground">Leave encashment</span>
                  <span className="text-[13px] font-bold text-foreground">{formatCurrency(28000)}</span>
                </div>
                <div className="border-t border-border/50" />
                <div className="flex items-center justify-between py-1.5">
                  <span className="text-[12px] font-medium text-muted-foreground">Reimbursements</span>
                  <span className="text-[13px] font-bold text-foreground">{formatCurrency(4200)}</span>
                </div>
              </div>
            ...
            >
              <div className="px-6 py-5 border-b border-border flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-[15px] font-bold text-foreground tracking-tight">Upload Clearance Documents</h3>
                </div>
                <button onClick={() => setShowUploadModal(false)} className="w-8 h-8 rounded-lg hover:bg-muted flex items-center justify-center transition-all">
            ...
            >
              <div className="px-6 py-5 border-b border-border flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-[15px] font-bold text-foreground tracking-tight">NDA Acknowledgment</h3>
                </div>
                <button onClick={() => setShowSignatureModal(false)} className="w-8 h-8 rounded-lg hover:bg-muted flex items-center justify-center transition-all">
                  <X size={16} className="text-muted-foreground" />
                </button>
              </div>
              ...
              <div className="px-6 py-5 space-y-5">
                <div className="px-4 py-3 rounded-xl bg-[#F0FDF4] border border-[#00B87C]/20 flex items-start gap-3">
                  <ShieldCheck size={18} className="text-[#00B87C] shrink-0 mt-0.5" />
                  <p className="text-[12px] font-medium text-foreground">
                    By signing below, you acknowledge that you have read and agree to the terms of the Non-Disclosure Agreement. You confirm that you will not disclose any confidential company information after your employment ends.
                  </p>
                </div>
                  ...
                  <div className="border-2 border-dashed border-border rounded-2xl h-24 flex items-center justify-center bg-background cursor-pointer group hover:border-[#00B87C] transition-all">
                    {signatureComplete ? (
                      ...
                      <div className="flex flex-col items-center gap-1">
                        <Pen size={20} className="text-muted-foreground group-hover:text-[#00B87C] transition-colors" />
                        <span className="text-[11px] font-bold text-muted-foreground group-hover:text-foreground transition-colors">Click to sign</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <label className="flex items-center gap-2.5 cursor-pointer">
                  <input type="checkbox" className="w-4 h-4 rounded border-border text-[#00B87C] focus:ring-[#00B87C]" />
                  <span className="text-[12px] font-bold text-foreground">I agree to the terms of the NDA</span>
                </label>
              </div>
            ...
            >
              <div className="px-6 py-5 border-b border-border flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-[15px] font-bold text-foreground tracking-tight">Schedule Exit Interview</h3>
                </div>
                <button onClick={() => setShowScheduleModal(false)} className="w-8 h-8 rounded-lg hover:bg-muted flex items-center justify-center transition-all">
                  <X size={16} className="text-muted-foreground" />
                </button>
              </div>
