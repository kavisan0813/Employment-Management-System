...
export const ShiftSchedule: React.FC = () => {
  ...
  const [renameTemplate, setRenameTemplate] = useState<ShiftTemplate | null>(
    null,
  );
  ...
  const [advancedApplyTemplate, setAdvancedApplyTemplate] =
    useState<ShiftTemplate | null>(null);
  ...
  const [applyConflictHandling, setApplyConflictHandling] = useState<
    "replace" | "fill" | "skip"
  >("replace");
  ...
  const [swaps, setSwaps] = useState<SwapItem[]>([
    {
      ...
      detail: "Mon Apr 7 Morning ↔ Tue Apr 8 Evening",
      p1Color: "#059669",
      p2Color: "#2563EB",
    ...
    },
    {
      ...
      p2Init: "SP",
      reason: "Family event",
      detail: "Wed Apr 9 Night ↔ Fri Apr 11 Night",
      p1Color: "#0D9488",
      p2Color: "#7C3AED",
    ...
    },
    {
      ...
      detail: "Thu Apr 10 Evening ↔ Fri Apr 11 Morning",
      p1Color: "#D97706",
      p2Color: "#DB2777",
      shiftTypes: "Evening ↔ Morning",
    },
  ]);
  const [fixingOT, setFixingOT] = useState(false);
  const [generatingReport, setGeneratingReport] = useState(false);
  const [selectedSwapDetails, setSelectedSwapDetails] =
    useState<SwapItem | null>(null);
    ...
    <div className="w-full px-4 md:px-8 py-6 pb-10">
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-3xl font-extrabold tracking-tight text-teal-600 group-hover:scale-105 transition-transform">
                8
              </p>
              <button
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex gap-4">
            {[
              ...
              >
                <div
                  className={`w-7 h-7 rounded-full ${type.color} flex items-center justify-center text-white text-[9px] font-black shadow-sm`}
                >
                  {type.label}
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-20">
        ...
        >
          ...
          <div
            ref={swapListRef}
            className="flex-1 p-4 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-700 scrollbar-track-transparent scroll-smooth"
          >
            {swaps.map((swap) => (
              <div
                key={swap.id}
                className="pb-4 border-b last:border-b-0 border-border/50 last:pb-0 group"
              >
            ...
            <div className="bg-amber-50 dark:bg-amber-900/10 p-3 rounded-xl border border-amber-100 dark:border-amber-900/30 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[11px] text-amber-700 dark:text-amber-400 font-bold">
                  3 employees exceed the 15hr limit
                </p>
              </div>
              <button
      ...
      <div className="mb-12">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-xl font-extrabold tracking-tight text-slate-900 dark:text-slate-50">
              Shift Templates
            </h3>
            <p className="text-xs font-medium text-muted-foreground mt-1">
              Deploy pre-configured schedule routines across organizational
              groups
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {templates.map((tmpl) => (
            ...
            >
              ...
              <div className="absolute top-4 right-4">
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    setShowTemplateMenu(
                      showTemplateMenu === tmpl.id ? null : tmpl.id,
                    );
                  }}
                >
                  <MoreIcon size={16} />
              ...
              <div>
                <h4 className="text-base font-extrabold text-slate-900 dark:text-slate-100 mb-2">
                  {tmpl.name}
                </h4>

                {/* Shift Legend Compact Chips */}
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {Array.from(new Set(Object.values(tmpl.weeklySchedule))).map(
                    (shiftName) => {
                      ...
                      > = {
                        ...
                        Night: {
                          bg: "bg-[#F3E8FF] dark:bg-violet-900/20",
                          text: "text-[#7C3AED]",
                          dot: "#7C3AED",
                        },
                        "Off Day": {
                      ...
                      const colors = colorMap[shiftName] || {
                        bg: "bg-neutral-100",
                ...
                <div className="space-y-1.5 mb-5">
                  ...
                  <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground">
                    ...
                    <span>
                      Department:{" "}
                      <span className="text-foreground">{tmpl.department}</span>
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground">
                    <Clock size={14} />
          ...
          {applyStep === "form" ? (
              ...
              <div className="px-6 py-4 border-b border-border bg-neutral-50 dark:bg-zinc-800/40 flex items-center justify-between flex-shrink-0">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Assign this template to employees for the selected week.
                  </p>
                </div>
                <button
                  onClick={() => setAdvancedApplyTemplate(null)}
                  className="p-1.5 rounded-lg hover:bg-neutral-200 dark:hover:bg-zinc-700 text-muted-foreground"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6 overflow-y-auto flex-1">
                ...
                <div className="bg-[#F4FBF7] dark:bg-emerald-950/10 border border-[#00B87C]/20 p-4 rounded-2xl flex flex-wrap items-center justify-between gap-4">
                  <div>
                    <p className="text-base font-black text-slate-900 dark:text-slate-100">
                      {advancedApplyTemplate.name}
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {advancedApplyTemplate.department} •{" "}
                      {advancedApplyTemplate.employeesCount} Utilizing • Last
                      Applied: {advancedApplyTemplate.lastApplied}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {Array.from(
                      new Set(
                        Object.values(advancedApplyTemplate.weeklySchedule),
                      ),
                    ).map((shiftName) => {
                      ...
                      return (
                        <span
                          key={shiftName}
                          className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${colorMap[shiftName] || "bg-neutral-100 text-neutral-600"}`}
                        >
                          {shiftName}
                        </span>
                      );
                        ...
                        <button
                          ...
                          onClick={() => {
                            const allDeptEmps = globalEmployees
                              ...
                              .map((emp) => emp.id);
                            setSelectedEmps(
                              selectedEmps.length === allDeptEmps.length
                                ? []
                                : allDeptEmps,
                            );
                          }}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setApplyConflictHandling(
                              e.target.value as "replace" | "fill" | "skip",
                            )
                          }
                        >
                ...
                {selectedEmps.length > 0 && (
                  <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 rounded-xl p-3 flex items-center gap-2 text-xs font-bold text-amber-700 dark:text-amber-400">
                    ...
                    <span>
                      Warning: 2 shifts already assigned to selected staff. 1
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mb-5">
                ...
                routine to {selectedEmps.length} employees for {applyDateRange}?
              </p>
      ...
      {selectedTemplate && (
        ...
        >
          ...
          >
            ...
            <div className="p-6">
              <p className="text-xs font-extrabold text-slate-400 tracking-widest uppercase mb-3">
              ...
              <div className="divide-y divide-border">
                {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map(
                  (day) => {
                    const shift =
                      selectedTemplate.weeklySchedule[day] || "Off Day";
                    const colorMap: Record<string, string> = {
                      Morning: "bg-[#E6F4EA] text-[#00B87C]",
                      Evening: "bg-[#FFFBEB] text-[#F59E0B]",
                      Night: "bg-[#F3E8FF] text-[#7C3AED]",
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          className={`px-3 py-1 text-xs font-black uppercase tracking-wide rounded-full ${colorMap[shift] || "bg-slate-400 text-white"}`}
                        >
                          {shift}
                        </span>
                      </div>
        ...
        >
          ...
          >
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <label className="block text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-wide mb-3">
                  Weekly Shift Selector
                </label>
                  ...
                  {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map(
                    (day) => (
                      <div key={day}>
                        ...
                        >
                          {["Morning", "Evening", "Night", "Off Day"].map(
                            (s) => (
      ...
      {showAddModal && (
        ...
        >
          <div
            className="w-full max-w-md rounded-2xl shadow-xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200"
            style={{
          ...
          <div className="bg-card rounded-[32px] shadow-2xl border border-border w-full max-w-md overflow-hidden transform transition-all p-6">
            ...
            <div className="flex items-center justify-center gap-12 py-6 bg-[#F8FCFA] dark:bg-zinc-800/30 rounded-2xl mb-6 relative">
              <div className="flex flex-col items-center">
                ...
                <span className="text-sm font-extrabold mt-2 text-foreground">
                  {selectedSwapDetails.p1}
                </span>
              </div>
            ...
            <div className="space-y-4 mb-8">
              <div>
                ...
                <p className="text-sm font-extrabold text-foreground mt-1">
                  {selectedSwapDetails.shiftTypes || "Morning ↔ Evening"}
                </p>
              </div>
            ...
            <div className="flex items-center justify-center gap-3 pt-2">
              <button
                onClick={() => {
                  setSwaps((prev) =>
                    prev.filter((s) => s.id !== selectedSwapDetails.id),
                  );
