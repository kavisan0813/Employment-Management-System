...
const MOCK_RESULTS = {
  ...
  attendance: [
    {
      id: "a1",
      title: "Highest Attendance March",
      info: "Sarah Connor (100%)",
      type: "Metric",
    },
    {
      id: "a2",
      title: "Team Attendance Average",
      info: "94.5%",
      type: "Metric",
    },
  ],
  payroll: [
    {
      id: "p1",
      title: "Payroll Summary March",
      info: "Total: ₹45,20,000",
      type: "Report",
    },
  ],
  leaves: [
...
const SmartSearch = () => {
  ...
  const getFilteredResults = (searchTerm: string) => {
    ...
    const isPayrollQuery =
      q.includes("payroll") || q.includes("salary") || q.includes("payslip");
    const isLeaveQuery =
      q.includes("leave") || q.includes("vacation") || q.includes("off");

    const filteredEmployees = MOCK_RESULTS.employees.filter(
      (emp) =>
    ...
    const filteredAttendance = MOCK_RESULTS.attendance.filter(
      (att) =>
    ...
    const filteredPayroll = MOCK_RESULTS.payroll.filter(
      (pay) =>
    ...
    const filteredLeaves = MOCK_RESULTS.leaves.filter(
      (leave) =>
    ...
    if (
      ...
      if (q.includes("payroll"))
        return {
          employees: [],
          attendance: [],
          payroll: MOCK_RESULTS.payroll,
          leaves: [],
        };
      if (q.includes("attendance"))
        return {
          employees: [],
          attendance: MOCK_RESULTS.attendance,
          payroll: [],
          leaves: [],
        };
      if (q.includes("leave"))
        return {
          employees: MOCK_RESULTS.employees.filter(
            (e) => e.status === "On Leave",
          ),
          attendance: [],
          payroll: [],
          leaves: MOCK_RESULTS.leaves,
        };
      if (q.includes("performer") || q.includes("performance"))
        return {
          employees: MOCK_RESULTS.employees,
          attendance: MOCK_RESULTS.attendance,
          payroll: [],
          leaves: [],
        };
      ...
      <span>
        {parts.map((part: string, i: number) =>
          part.toLowerCase() === highlight.toLowerCase() ? (
            <span
              key={i}
              className="bg-emerald-100 text-emerald-900 font-semibold px-0.5 rounded"
            >
              {part}
            </span>
          ) : (
            ...
            {isSearching ? (
              ...
              >
                ...
                <div className="text-center">
                  <p className="text-[#064E3B] dark:text-[#A7F3D0] font-medium text-lg">
                    AI is thinking...
                  </p>
                  <p className="text-sm text-[#064E3B]/60 dark:text-[#A7F3D0]/60">
                    Analyzing workforce data and reports
                  </p>
                </div>
              </motion.div>
            ) : hasSearched ? (
              results ? (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                            ...
                            >
                              ...
                              <div className="px-4 py-2 bg-emerald-50/50 dark:bg-emerald-900/10 flex gap-2 border-t border-emerald-50 dark:border-emerald-900/20 opacity-0 group-hover:opacity-100 transition-opacity">
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  className="h-7 text-[11px] text-[#10B981] hover:bg-emerald-100"
                                  onClick={(e) => {
                                    e.stopPropagation();
                            ...
                            >
                              <CardContent className="p-4 flex gap-4">
                                ...
                                <div>
                                  ...
                                  <h4 className="font-semibold text-[#022C22] dark:text-[#ECFDF5] group-hover:text-amber-700 transition-colors">
                                    {att.title}
                                  </h4>
                                  <p className="text-sm text-[#064E3B]/70 dark:text-[#A7F3D0]/70 font-medium mt-1">
                                    {att.info}
                                  </p>
                                </div>
                              </CardContent>
                            ...
                            >
                              <CardContent className="p-4 flex gap-4">
                                ...
                                <div className="flex-1">
                                  ...
                                  <h4 className="font-semibold text-[#022C22] dark:text-[#ECFDF5] group-hover:text-blue-700 transition-colors">
                                    {pay.title}
                                  </h4>
                                  <p className="text-sm text-[#064E3B]/70 dark:text-[#A7F3D0]/70 font-medium mt-1">
                                    {pay.info}
                                ...
                                <Button
                                  size="icon"
                                  variant="ghost"
                                  className="text-blue-500 group-hover:translate-x-1 transition-transform border-none hover:bg-blue-50"
                                >
                                  <ArrowRight size={18} />
                                </Button>
                              </CardContent>
                    ...
                    {results.leaves.length > 0 && (
                      <div
                        className={`${results.employees.length === 0 ? "md:col-span-2" : "md:col-start-1 md:col-end-3"} space-y-4`}
                      >
                        <h3 className="flex items-center gap-2 text-[#022C22] dark:text-[#ECFDF5] font-bold text-xl px-2">
                          ...
                          {results.leaves
                            ...
                            .map((leave) => (
                              ...
                              >
                                <CardContent className="p-4 flex items-center gap-4">
                                  <div className="flex-1">
                                    <h4 className="font-bold text-[#022C22] dark:text-[#ECFDF5] group-hover:text-amber-700 transition-colors">
                                      {leave.name}
                                    </h4>
                                    <p className="text-xs text-[#064E3B]/60 dark:text-[#A7F3D0]/60">
                ...
                >
                  ...
                  <p className="text-[#064E3B]/60 dark:text-[#A7F3D0]/60 max-w-xs mx-auto mt-2">
                    We couldn't find anything matching your query. Try
                    rephrasing or search for something else.
                  </p>
                </motion.div>
              )
            ) : (
              ...
              >
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl font-bold text-[#022C22] dark:text-[#ECFDF5]">
                    Ready to help?
                  </h3>
                  <p className="text-[#064E3B]/60 dark:text-[#A7F3D0]/60 max-w-md">
                    Ask questions in natural language like "Show me designers in
                    Delhi" or "Average performance score last month".
                  </p>
                </div>
              </motion.div>
            )}
