    ...
    <div className="w-full px-4 md:px-8 py-6 pb-10 space-y-6">
      ...
      <div className="w-full py-3 px-6 rounded-2xl bg-emerald-50/50 border border-emerald-100 flex flex-wrap items-center gap-8 shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-bold text-slate-700 tracking-tight">12 pending leave requests</span>
        </div>
        <div className="flex items-center gap-2 border-l border-emerald-200/50 pl-8">
          <div className="w-2 h-2 rounded-full bg-emerald-500" />
          <span className="text-[12px] font-bold text-slate-700 tracking-tight">8 new applications today</span>
        </div>
        <div className="flex items-center gap-2 border-l border-emerald-200/50 pl-8">
          <div className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" />
          <span className="text-[12px] font-bold text-slate-700 tracking-tight">Payroll process starts in 2 days</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: Users, label: "TOTAL EMPLOYEES", value: "1,284", sub: "Active count", color: "#10B981", bg: "#DCFCE7" },
          { icon: UserCheck, label: "NEW HIRES", value: "24", sub: "Last 30 days", color: "#8B5CF6", bg: "#EDE9FE" },
          { icon: UserX, label: "ATTRITION RATE", value: "1.2%", sub: "Last month: 1.4%", color: "#EF4444", bg: "#FEE2E2" },
          { icon: Briefcase, label: "OPEN POSITIONS", value: "18", sub: "5 high priority", color: "#F59E0B", bg: "#FEF3C7" },
        ].map((kpi, i) => (
          <motion.div 
            key={i}
        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {LEAVE_REQUESTS.map((req, i) => (
              <div key={i} className="p-4 flex items-center gap-4 hover:bg-secondary/30 transition-colors group cursor-pointer">
          ...
          <div className="p-4 text-center border-t border-border">
            <button className="text-[12px] font-black text-primary uppercase tracking-widest hover:underline">Process All Leaves</button>
          </div>
        </div>
        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
          ...
          <div className="p-4 space-y-4">
            {RECENT_HIRES.map((hire, i) => (
              <div key={i} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-secondary/50 transition-colors border border-transparent hover:border-border">
          ...
          <div className="p-4 text-center border-t border-border">
            <button className="text-[12px] font-black text-primary uppercase tracking-widest hover:underline">Complete Onboarding (4)</button>
          </div>
        </div>
        ...
        <div className="bg-card p-6 rounded-2xl border border-border shadow-sm flex flex-col">
          ...
          <div className="mt-8 p-4 rounded-2xl bg-amber-50 border border-amber-100">
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-[13px] font-bold text-amber-900 leading-tight">Retention Alert</p>
                <p className="text-[11px] font-semibold text-amber-700 mt-1">High attrition risk detected in Sales department for Mid-level roles.</p>
              </div>
            </div>
          </div>
        </div>
